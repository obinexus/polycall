...
# Execution modes
readonly DRY_RUN="${1:-true}"
readonly FEATURE_NAME="${2:-}"
readonly FEATURE_DESCRIPTION="${3:-}"

# Architecture hierarchy (Sinphasé single-pass dependency flow)
readonly CORE_PATH="core"
...
generate_python_binding() {
    local feature="$1"
    local binding_dir="bindings/python/pypolycall"
...
generate_nodejs_binding() {
    local feature="$1"
    local binding_dir="bindings/nodejs/node-polycall/lib"
...
generate_go_binding() {
    local feature="$1"
    local binding_dir="bindings/go/pkg/polycall"
...
from pypolycall.${feature} import cli_${feature}_handler
...
execute_nodejs_${feature}() {
    ...
    node -e "
const { cli${feature^}Handler } = require('./bindings/nodejs/node-polycall/lib/${feature}');
...
execute_go_${feature}() {
    ...
    echo "Use: go run -tags cgo bindings/go/cmd/${feature}/main.go --dry-run=\$dry_run"
    exit 1
}

detect_available_binding() {
    # Priority order: Python, Node.js, Go
    if command -v python3 &>/dev/null && [[ -f "bindings/python/pypolycall/${feature}.py" ]]; then
        echo "python"
    elif command -v node &>/dev/null && [[ -f "bindings/nodejs/node-polycall/lib/${feature}.js" ]]; then
        echo "node"
    elif command -v go &>/dev/null && [[ -f "bindings/go/pkg/polycall/${feature}.go" ]]; then
        echo "go"
    else
...
main() {
    ...
    echo "   Bindings: bindings/*/${FEATURE_NAME}.* (hotwire layer)"
    ...
    echo "   3. Test bindings: python3 bindings/python/pypolycall/${FEATURE_NAME}.py"
    ...
    if [[ "$DRY_RUN" == "true" ]]; then
        ...
        echo "   Run with 'false' as first argument to generate actual files"
    fi
