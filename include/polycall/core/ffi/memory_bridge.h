...
typedef enum {
  ...
  POLYCALL_MEMORY_CONFIG_STRICT_OWNERSHIP =
      (1 << 0),                                 // Strict ownership checking
...
typedef enum {
  ...
  POLYCALL_MEMORY_FLAG_MARKED_FOR_COLLECTION =
      (1 << 1),                             // Marked for collection
 ...
 */
typedef void (*gc_notification_callback_t)(polycall_core_context_t *ctx,
                                           const char *language, void *ptr,
...
polycall_core_error_t polycall_memory_track_reference(
    polycall_core_context_t *ctx, polycall_ffi_context_t *ffi_ctx,
    memory_manager_t *memory_mgr, void *ptr, size_t size, const char *language);
