...
static polycall_dop_error_t dop_utils_copy_string_safe(
    char* dest, 
    size_t dest_size, 
    const char* src
);
...
polycall_dop_value_t polycall_dop_value_create_uint32(uint32_t uint_value) {
    polycall_dop_value_t value = {
        .type = POLYCALL_DOP_VALUE_UINT32,
...
polycall_dop_value_t polycall_dop_value_create_uint64(uint64_t uint_value) {
    polycall_dop_value_t value = {
        .type = POLYCALL_DOP_VALUE_UINT64,
...
polycall_dop_value_t polycall_dop_value_create_float32(float float_value) {
    polycall_dop_value_t value = {
        .type = POLYCALL_DOP_VALUE_FLOAT32,
...
polycall_dop_value_t polycall_dop_value_create_float64(double double_value) {
    polycall_dop_value_t value = {
        .type = POLYCALL_DOP_VALUE_FLOAT64,
 ...
 */
polycall_dop_error_t polycall_dop_value_create_string(
    ...
    char* string_copy = (char*)polycall_memory_allocate(core_ctx, string_length + 1);
    if (!string_copy) {
 ...
 */
polycall_dop_error_t polycall_dop_value_create_bytes(
    polycall_core_context_t* core_ctx,
 ...
 */
polycall_dop_error_t polycall_dop_value_destroy(
    polycall_core_context_t* core_ctx,
    polycall_dop_value_t* value
) {
    if (!core_ctx || !value) {
        ...
        case POLYCALL_DOP_VALUE_ARRAY:
            if (value->value.array_val.elements) {
                ...
                for (size_t i = 0; i < value->value.array_val.count; i++) {
                    polycall_dop_value_destroy(core_ctx, &value->value.array_val.elements[i]);
                }
 ...
 */
polycall_dop_error_t polycall_dop_value_copy(
    polycall_core_context_t* core_ctx,
    ...
    switch (source->type) {
        ...
        case POLYCALL_DOP_VALUE_STRING:
            return polycall_dop_value_create_string(core_ctx, source->value.string_val.data, dest);
            
        case POLYCALL_DOP_VALUE_BYTES:
            return polycall_dop_value_create_bytes(core_ctx, source->value.bytes_val.data,
                                                  source->value.bytes_val.size, dest);
            
        case POLYCALL_DOP_VALUE_ARRAY:
            // Deep copy array
            if (source->value.array_val.count > 0) {
                dest->value.array_val.elements = (polycall_dop_value_t*)polycall_memory_allocate(
                    core_ctx, sizeof(polycall_dop_value_t) * source->value.array_val.count);
                ...
                for (size_t i = 0; i < source->value.array_val.count; i++) {
                    polycall_dop_error_t result = polycall_dop_value_copy(
                        core_ctx, &source->value.array_val.elements[i], &dest->value.array_val.elements[i]);
                    if (result != POLYCALL_DOP_SUCCESS) {
                        // Cleanup partial copy
                        for (size_t j = 0; j < i; j++) {
                            polycall_dop_value_destroy(core_ctx, &dest->value.array_val.elements[j]);
                        }
...
polycall_dop_error_t polycall_dop_component_config_validate_extended(
    ...
    polycall_dop_error_t basic_result = polycall_dop_component_config_validate(config);
    ...
    if (config->security_policy.allowed_permissions & config->security_policy.denied_permissions) {
    ...
    for (size_t i = 0; i < config->method_count; i++) {
        polycall_dop_error_t method_result = dop_utils_validate_method_signature(&config->methods[i]);
...
polycall_dop_error_t polycall_dop_component_config_create_default(
    ...
    polycall_dop_error_t result = dop_utils_copy_string_safe(
        (char*)config->component_id, POLYCALL_DOP_MAX_COMPONENT_ID_LENGTH, component_id);
    if (result != POLYCALL_DOP_SUCCESS) {
        return result;
    }
    
    result = dop_utils_copy_string_safe(
        (char*)config->component_name, POLYCALL_DOP_MAX_COMPONENT_NAME_LENGTH, component_name);
    if (result != POLYCALL_DOP_SUCCESS) {
        return result;
    }
    
    result = dop_utils_copy_string_safe(
        (char*)config->version, POLYCALL_DOP_MAX_VERSION_LENGTH, "1.0.0");
    if (result != POLYCALL_DOP_SUCCESS) {
        return result;
    }
    ...
    result = polycall_dop_security_policy_create_default(default_isolation, &config->security_policy);
    if (result != POLYCALL_DOP_SUCCESS) {
        return result;
    }
 ...
 */
polycall_dop_error_t polycall_dop_component_debug_print(
    ...
    int written = snprintf(buffer, buffer_size,
        ...
        component->total_execution_time_ns,
...
static bool dop_utils_is_valid_version_string(const char* version) {
    ...
    // Simple version validation: should match pattern like "1.2.3" or "1.0.0-beta"
...
static polycall_dop_error_t dop_utils_validate_method_signature(
    ...
    if (!signature->method_name || !dop_utils_is_valid_identifier(signature->method_name)) {
        return POLYCALL_DOP_ERROR_CONFIGURATION_INVALID;
    }
        ...
        case POLYCALL_DOP_VALUE_ARRAY:
            {
                ...
                for (size_t i = 0; i < value->value.array_val.count; i++) {
                    total_size += dop_utils_calculate_value_size(&value->value.array_val.elements[i]);
                }
                return total_size;
            }
        case POLYCALL_DOP_VALUE_COMPONENT_REF:
