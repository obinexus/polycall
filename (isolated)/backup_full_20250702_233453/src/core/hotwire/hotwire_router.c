...
static polycall_core_error_t hotwire_validate_route_config(
    const char* source_protocol,
    const char* target_protocol,
    const char* config_path
);

static polycall_core_error_t hotwire_audit_route_access(
...
static polycall_core_error_t hotwire_v1_compatibility_check(
...
polycall_core_error_t hotwire_router_init(
    ...
    // Allocate context
    g_hotwire_ctx = polycall_core_alloc(core_ctx, sizeof(hotwire_router_context_t));
    if (!g_hotwire_ctx) {
    ...
    polycall_core_error_t map_result = hotwire_protocol_map_register(protocol_ctx);
...
void hotwire_router_cleanup(void) {
    ...
    if (g_hotwire_ctx->telemetry_ctx) {
        telemetry_commands_cleanup(g_hotwire_ctx->core_ctx, g_hotwire_ctx->telemetry_ctx);
    }

    // Free any private route data
    for (size_t i = 0; i < g_hotwire_ctx->route_count; i++) {
        if (g_hotwire_ctx->routes[i].private_data) {
            polycall_core_free(g_hotwire_ctx->core_ctx, g_hotwire_ctx->routes[i].private_data);
        }
    }
...
polycall_core_error_t hotwire_route_register(
    ...
    polycall_core_error_t compat_result = hotwire_v1_compatibility_check(source_protocol);
    if (compat_result != POLYCALL_CORE_SUCCESS) {
        // Log compatibility violation for audit
        if (g_hotwire_ctx->flags & HOTWIRE_AUDIT_ENABLED) {
            hotwire_audit_route_access(g_hotwire_ctx, source_protocol, target_protocol, "COMPAT_VIOLATION");
        }
        return compat_result;
    }

    // Create new route entry
    hotwire_route_entry_t* route = &g_hotwire_ctx->routes[g_hotwire_ctx->route_count];
    strncpy(route->source_protocol, source_protocol, sizeof(route->source_protocol) - 1);
    strncpy(route->target_protocol, target_protocol, sizeof(route->target_protocol) - 1);
    ...
    if (g_hotwire_ctx->flags & HOTWIRE_AUDIT_ENABLED) {
        hotwire_audit_route_access(g_hotwire_ctx, source_protocol, target_protocol, "REGISTER");
    }
...
polycall_core_error_t hotwire_route_execute(
    ...
    if (!g_hotwire_ctx || !source_protocol || !target_protocol || !request || !response) {
        return POLYCALL_CORE_ERROR_INVALID_PARAMETER;
    }
    ...
    for (size_t i = 0; i < g_hotwire_ctx->route_count; i++) {
        if (strcmp(g_hotwire_ctx->routes[i].source_protocol, source_protocol) == 0 &&
            strcmp(g_hotwire_ctx->routes[i].target_protocol, target_protocol) == 0) {
    ...
    if (g_hotwire_ctx->flags & HOTWIRE_AUDIT_ENABLED) {
        hotwire_audit_route_access(g_hotwire_ctx, source_protocol, target_protocol, "EXECUTE");
    }

    // Delegate to protocol layer with stateless fallback
    polycall_core_error_t exec_result = hotwire_protocol_map_execute(
        g_hotwire_ctx->protocol_ctx,
        source_protocol,
        target_protocol,
        request,
        response
    ...
    if (exec_result != POLYCALL_CORE_SUCCESS && (g_hotwire_ctx->flags & HOTWIRE_V1_COMPAT_FLAG)) {
...
static polycall_core_error_t hotwire_validate_route_config(
    ...
    if (config_path && strlen(config_path) > 0) {
        polycall_core_error_t config_result = config_commands_validate_path(
            g_hotwire_ctx->core_ctx,
            config_path
        );
...
static polycall_core_error_t hotwire_audit_route_access(
    ...
    snprintf(audit_message, sizeof(audit_message),
        "HOTWIRE_ROUTE: %s -> %s [%s] at %llu",
        source_protocol, target_protocol, operation,
 ...
 */
polycall_core_error_t hotwire_router_get_stats(
    hotwire_router_stats_t* stats
) {
