 ...
 *
 * This file implements the JavaScript language bridge for LibPolyCall FFI,
providing
...
polycall_js_bridge_init(polycall_core_context_t *ctx,
  ...
  polycall_js_bridge_t *new_bridge =
      polycall_core_malloc(ctx, sizeof(polycall_js_bridge_t));
  if (!new_bridge) {
    POLYCALL_ERROR_SET(
        ctx, POLYCALL_ERROR_SOURCE_FFI, POLYCALL_CORE_ERROR_OUT_OF_MEMORY,
        POLYCALL_ERROR_SEVERITY_ERROR, "Failed to allocate JavaScript bridge");
  ...
  new_bridge->enable_exception_translation =
      config->enable_exception_translation;
  ...
  polycall_core_error_t result =
      init_js_function_registry(ctx, &new_bridge->function_registry, 256);
  ...
  if (new_bridge->adapter.initialize_runtime &&
      !new_bridge->adapter.initialize_runtime(new_bridge->runtime_handle)) {
    POLYCALL_ERROR_SET(ctx, POLYCALL_ERROR_SOURCE_FFI,
                       POLYCALL_CORE_ERROR_INITIALIZATION_FAILED,
                       POLYCALL_ERROR_SEVERITY_ERROR,
 ...
 */
void polycall_js_bridge_cleanup(polycall_core_context_t *ctx,
                                polycall_ffi_context_t *ffi_ctx,
                                polycall_js_bridge_t *js_bridge) {
  if (!ctx || !js_bridge) {
    return;
...
polycall_core_error_t polycall_js_bridge_register_function(
    ...
    polycall_js_bridge_t *js_bridge, const char *function_name,
    void *js_function, ffi_signature_t *signature, uint32_t flags) {
  if (!ctx || !ffi_ctx || !js_bridge || !function_name || !js_function ||
      !signature) {
    return POLYCALL_CORE_ERROR_INVALID_PARAMETERS;
  }
  ...
  bool is_function =
      js_bridge->adapter.is_function(js_bridge->runtime_handle, js_function);
  ...
  if (find_js_function(&js_bridge->function_registry, function_name)) {
    ...
    POLYCALL_ERROR_SET(
        ctx, POLYCALL_ERROR_SOURCE_FFI, POLYCALL_CORE_ERROR_ALREADY_INITIALIZED,
  ...
  if (js_bridge->function_registry.count >=
      js_bridge->function_registry.capacity) {
    pthread_mutex_unlock(&js_bridge->function_registry.mutex);
  ...
  js_function_t *func = &js_bridge->function_registry
                             .functions[js_bridge->function_registry.count];
  ...
  if (result != POLYCALL_CORE_SUCCESS) {
    ...
    POLYCALL_ERROR_SET(ctx, POLYCALL_ERROR_SOURCE_FFI, result,
                       POLYCALL_ERROR_SEVERITY_ERROR,
...
polycall_core_error_t polycall_js_bridge_call_function(
    polycall_core_context_t *ctx, polycall_ffi_context_t *ffi_ctx,
    polycall_js_bridge_t *js_bridge, const char *function_name,
    ffi_value_t *args, size_t arg_count, ffi_value_t *result) {
  if (!ctx || !ffi_ctx || !js_bridge || !function_name ||
  ...
  pthread_mutex_lock(&js_bridge->function_registry.mutex);
  js_function_t *func =
      find_js_function(&js_bridge->function_registry, function_name);
  ...
  if (arg_count > 0) {
    ...
    for (size_t i = 0; i < arg_count; i++) {
      polycall_core_error_t error =
          convert_ffi_to_js_value(ctx, js_bridge, &args[i], &js_args[i]);
      if (error != POLYCALL_CORE_SUCCESS) {
        // Clean up already converted arguments
        for (size_t j = 0; j < i; j++) {
          pthread_mutex_lock(&js_bridge->runtime_mutex);
          js_bridge->adapter.release_value(js_bridge->runtime_handle,
                                           js_args[j]);
          pthread_mutex_unlock(&js_bridge->runtime_mutex);
        }
        ...
        POLYCALL_ERROR_SET(
            ctx, POLYCALL_ERROR_SOURCE_FFI, error,
            POLYCALL_ERROR_SEVERITY_ERROR,
  ...
  bool has_exception =
      js_bridge->adapter.has_exception(js_bridge->runtime_handle);
  void *exception = NULL;
  ...
  if (!js_result) {
    ...
    if (error != POLYCALL_CORE_SUCCESS) {
      POLYCALL_ERROR_SET(ctx, POLYCALL_ERROR_SOURCE_FFI, error,
                         POLYCALL_ERROR_SEVERITY_ERROR,
 ...
 */
polycall_core_error_t
polycall_js_bridge_to_js_value(polycall_core_context_t *ctx,
                               polycall_ffi_context_t *ffi_ctx,
                               polycall_js_bridge_t *js_bridge,
                               const ffi_value_t *ffi_value, void **js_value) {
...
polycall_core_error_t polycall_js_bridge_from_js_value(
    polycall_core_context_t *ctx, polycall_ffi_context_t *ffi_ctx,
    polycall_js_bridge_t *js_bridge, void *js_value,
    polycall_ffi_type_t expected_type, ffi_value_t *ffi_value) {
  if (!ctx || !ffi_ctx || !js_bridge || !js_value || !ffi_value) {
    return POLYCALL_CORE_ERROR_INVALID_PARAMETERS;
  }

  return convert_js_to_ffi_value(ctx, js_bridge, js_value, expected_type,
                                 ffi_value);
}
...
polycall_core_error_t polycall_js_bridge_setup_promise(
    polycall_core_context_t *ctx, polycall_ffi_context_t *ffi_ctx,
    polycall_js_bridge_t *js_bridge, const char *async_function_name,
  ...
  if (find_promise_callback(&js_bridge->promise_registry,
                            async_function_name)) {
    ...
    POLYCALL_ERROR_SET(ctx, POLYCALL_ERROR_SOURCE_FFI,
                       ...
                       POLYCALL_ERROR_SEVERITY_WARNING,
                       "Promise callback for function %s already registered",
                       async_function_name);
    return POLYCALL_CORE_ERROR_ALREADY_INITIALIZED;
  }

  // Check if registry is full
  if (js_bridge->promise_registry.count >=
      js_bridge->promise_registry.capacity) {
    pthread_mutex_unlock(&js_bridge->promise_registry.mutex);
  ...
  promise_callback_t *callback =
      &js_bridge->promise_registry.callbacks[js_bridge->promise_registry.count];

  // Duplicate function name
  callback->function_name =
      polycall_core_malloc(ctx, strlen(async_function_name) + 1);
...
polycall_core_error_t polycall_js_bridge_handle_exception(
    polycall_core_context_t *ctx, polycall_ffi_context_t *ffi_ctx,
    polycall_js_bridge_t *js_bridge, void *js_exception, char *error_message,
    size_t message_size) {
  if (!ctx || !ffi_ctx || !js_bridge || !js_exception || !error_message ||
      message_size == 0) {
    return POLYCALL_CORE_ERROR_INVALID_PARAMETERS;
  }
  ...
  if (exception_msg && msg_len > 0) {
    size_t copy_len =
        msg_len < (message_size - 1) ? msg_len : (message_size - 1);
    strncpy(error_message, exception_msg, copy_len);
    error_message[copy_len] = '\0';
...
polycall_core_error_t polycall_js_bridge_get_interface(
    polycall_core_context_t *ctx, polycall_ffi_context_t *ffi_ctx,
    polycall_js_bridge_t *js_bridge, language_bridge_t *bridge) {
...
init_js_function_registry(polycall_core_context_t *ctx,
                          js_function_registry_t *registry, size_t capacity) {
  // Allocate function array
  registry->functions =
      polycall_core_malloc(ctx, capacity * sizeof(js_function_t));
 ...
 */
static void cleanup_js_function_registry(polycall_core_context_t *ctx,
 ...
 */
static polycall_core_error_t init_promise_registry(polycall_core_context_t *ctx,
                                                   promise_registry_t *registry,
                                                   size_t capacity) {
  // Allocate callback array
  registry->callbacks =
      polycall_core_malloc(ctx, capacity * sizeof(promise_callback_t));
 ...
 */
static void cleanup_promise_registry(polycall_core_context_t *ctx,
                                     promise_registry_t *registry) {
  if (!registry->callbacks) {
    return;
  }
 ...
 */
static js_function_t *find_js_function(js_function_registry_t *registry,
                                       const char *name) {
  for (size_t i = 0; i < registry->count; i++) {
 ...
 */
static promise_callback_t *find_promise_callback(promise_registry_t *registry,
                                                 const char *name) {
  for (size_t i = 0; i < registry->count; i++) {
 ...
 */
static polycall_core_error_t
setup_runtime_adapter(polycall_js_bridge_t *js_bridge) {
  ...
  default:
    POLYCALL_ERROR_SET(
        js_bridge->core_ctx, POLYCALL_ERROR_SOURCE_FFI,
 ...
 */
static polycall_core_error_t
convert_ffi_to_js_value(polycall_core_context_t *ctx,
                        polycall_js_bridge_t *js_bridge,
                        const ffi_value_t *ffi_value, void **js_value) {
  ...
  case POLYCALL_FFI_TYPE_BOOL:
    *js_value = js_bridge->adapter.create_boolean(js_bridge->runtime_handle,
                                                  ffi_value->value.bool_value);
    break;
  ...
  case POLYCALL_FFI_TYPE_DOUBLE:
    ...
    switch (ffi_value->type) {
    case POLYCALL_FFI_TYPE_CHAR:
      number_value = (double)ffi_value->value.char_value;
      break;
    case POLYCALL_FFI_TYPE_INT8:
      number_value = (double)ffi_value->value.int8_value;
      break;
    case POLYCALL_FFI_TYPE_UINT8:
      number_value = (double)ffi_value->value.uint8_value;
      break;
    case POLYCALL_FFI_TYPE_INT16:
      number_value = (double)ffi_value->value.int16_value;
      break;
    case POLYCALL_FFI_TYPE_UINT16:
      number_value = (double)ffi_value->value.uint16_value;
      break;
    case POLYCALL_FFI_TYPE_INT32:
      number_value = (double)ffi_value->value.int32_value;
      break;
    case POLYCALL_FFI_TYPE_UINT32:
      number_value = (double)ffi_value->value.uint32_value;
      break;
    case POLYCALL_FFI_TYPE_INT64:
      number_value = (double)ffi_value->value.int64_value;
      break;
    case POLYCALL_FFI_TYPE_UINT64:
      number_value = (double)ffi_value->value.uint64_value;
      break;
    case POLYCALL_FFI_TYPE_FLOAT:
      number_value = (double)ffi_value->value.float_value;
      break;
    case POLYCALL_FFI_TYPE_DOUBLE:
      number_value = ffi_value->value.double_value;
      break;
    default:
      break; // Should never reach here
    }

    *js_value = js_bridge->adapter.create_number(js_bridge->runtime_handle,
                                                 number_value);
    break;
  ...
  case POLYCALL_FFI_TYPE_STRING:
    if (ffi_value->value.string_value) {
      ...
      if (js_bridge->max_string_length > 0 &&
  ...
  case POLYCALL_FFI_TYPE_POINTER:
    // Pointers require special handling - we wrap them in an object
    {
      ...
      // on the specific JavaScript runtime adapter's object property
      // manipulation API For now, we'll assume we have a function to set object
      // properties
  ...
  case POLYCALL_FFI_TYPE_ARRAY:
    ...
    if (ffi_value->value.array_value && ffi_value->type_info) {
      ...
      size_t array_length =
          ffi_value->type_info->details.array_info.element_count;
      *js_value = js_bridge->adapter.create_array(js_bridge->runtime_handle,
                                                  array_length);
    } else {
  ...
  case POLYCALL_FFI_TYPE_CALLBACK:
    // For callbacks, create a JavaScript function that will invoke the native
    // callback
    if (js_bridge->enable_callback_conversion &&
        ffi_value->value.callback_value) {
  ...
  default:
    ...
    *js_value = js_bridge->adapter.create_null(js_bridge->runtime_handle);
    ...
    POLYCALL_ERROR_SET(ctx, POLYCALL_ERROR_SOURCE_FFI,
                       POLYCALL_CORE_ERROR_UNSUPPORTED_OPERATION,
                       POLYCALL_ERROR_SEVERITY_ERROR,
                       "Unsupported FFI type for conversion to JavaScript: %d",
                       ffi_value->type);
    return POLYCALL_CORE_ERROR_UNSUPPORTED_OPERATION;
  }
...
static polycall_core_error_t convert_js_to_ffi_value(
    polycall_core_context_t *ctx, polycall_js_bridge_t *js_bridge,
    void *js_value, polycall_ffi_type_t expected_type, ffi_value_t *ffi_value) {
  ...
  bool is_number =
      js_bridge->adapter.is_number(js_bridge->runtime_handle, js_value);
  bool is_string =
      js_bridge->adapter.is_string(js_bridge->runtime_handle, js_value);
  bool is_boolean =
      js_bridge->adapter.is_boolean(js_bridge->runtime_handle, js_value);
  bool is_object =
      js_bridge->adapter.is_object(js_bridge->runtime_handle, js_value);
  bool is_null =
      js_bridge->adapter.is_null(js_bridge->runtime_handle, js_value);
  bool is_undefined =
      js_bridge->adapter.is_undefined(js_bridge->runtime_handle, js_value);
  bool is_array =
      js_bridge->adapter.is_array(js_bridge->runtime_handle, js_value);
  bool is_function =
      js_bridge->adapter.is_function(js_bridge->runtime_handle, js_value);
  ...
  if (expected_type == POLYCALL_FFI_TYPE_VOID) {
    ...
    if (is_boolean) {
      ...
      ffi_value->value.bool_value =
          js_bridge->adapter.get_boolean(js_bridge->runtime_handle, js_value);
    } else if (is_number) {
      ffi_value->type = POLYCALL_FFI_TYPE_DOUBLE;
      ffi_value->value.double_value =
          js_bridge->adapter.get_number(js_bridge->runtime_handle, js_value);
    } else if (is_string) {
      size_t str_len = 0;
      char *str_value = js_bridge->adapter.get_string(js_bridge->runtime_handle,
                                                      js_value, &str_len);
      ...
      ffi_value->value.array_value =
          NULL; // Actual implementation would create array
    } else if (is_function) {
    ...
    case POLYCALL_FFI_TYPE_BOOL:
      ...
      if (is_boolean) {
        ...
        size_t str_len = 0;
        js_bridge->adapter.get_string(js_bridge->runtime_handle, js_value,
                                      &str_len);
    ...
    case POLYCALL_FFI_TYPE_DOUBLE:
      ...
      if (is_number) {
        ...
        size_t str_len = 0;
        char *str_value = js_bridge->adapter.get_string(
            js_bridge->runtime_handle, js_value, &str_len);
      ...
      switch (expected_type) {
      case POLYCALL_FFI_TYPE_CHAR:
        ffi_value->value.char_value = (char)num_val;
        break;
      case POLYCALL_FFI_TYPE_INT8:
        ffi_value->value.int8_value = (int8_t)num_val;
        break;
      case POLYCALL_FFI_TYPE_UINT8:
        ffi_value->value.uint8_value = (uint8_t)num_val;
        break;
      case POLYCALL_FFI_TYPE_INT16:
        ffi_value->value.int16_value = (int16_t)num_val;
        break;
      case POLYCALL_FFI_TYPE_UINT16:
        ffi_value->value.uint16_value = (uint16_t)num_val;
        break;
      case POLYCALL_FFI_TYPE_INT32:
        ffi_value->value.int32_value = (int32_t)num_val;
        break;
      case POLYCALL_FFI_TYPE_UINT32:
        ffi_value->value.uint32_value = (uint32_t)num_val;
        break;
      case POLYCALL_FFI_TYPE_INT64:
        ffi_value->value.int64_value = (int64_t)num_val;
        break;
      case POLYCALL_FFI_TYPE_UINT64:
        ffi_value->value.uint64_value = (uint64_t)num_val;
        break;
      case POLYCALL_FFI_TYPE_FLOAT:
        ffi_value->value.float_value = (float)num_val;
        break;
      case POLYCALL_FFI_TYPE_DOUBLE:
        ffi_value->value.double_value = num_val;
        break;
      default:
        break;
      }
    ...
    case POLYCALL_FFI_TYPE_STRING:
      ...
      if (is_string) {
        ...
        // Allocate a buffer for the number string (20 chars should be enough
        // for most numbers)
    ...
    default:
      ...
      POLYCALL_ERROR_SET(
          ...
          POLYCALL_CORE_ERROR_UNSUPPORTED_OPERATION,
          POLYCALL_ERROR_SEVERITY_ERROR,
          "Unsupported FFI type for conversion from JavaScript: %d",
          expected_type);
      return POLYCALL_CORE_ERROR_UNSUPPORTED_OPERATION;
    }
  }
 ...
 */
static polycall_core_error_t
setup_node_adapter(polycall_js_bridge_t *js_bridge) {
 ...
 */
static polycall_core_error_t
setup_webkit_adapter(polycall_js_bridge_t *js_bridge) {
 ...
 */
static polycall_core_error_t
setup_spidermonkey_adapter(polycall_js_bridge_t *js_bridge) {
 ...
 */
static polycall_core_error_t
setup_quickjs_adapter(polycall_js_bridge_t *js_bridge) {
 ...
 */
static polycall_core_error_t
setup_custom_adapter(polycall_js_bridge_t *js_bridge) {
  // Custom adapter implementation
  // This assumes that the runtime_handle is already initialized with custom
  // adapter functions and just needs to be assigned to the bridge adapter
 ...
 */
static polycall_core_error_t js_convert_to_native(polycall_core_context_t *ctx,
                                                  const ffi_value_t *src,
                                                  void *dest,
                                                  ffi_type_info_t *dest_type) {
  // This function is called by the FFI system to convert an FFI value to a
  // native value In the case of JavaScript, this means converting to a
  // JavaScript engine-specific value
  ...
  POLYCALL_ERROR_SET(
      ctx, POLYCALL_ERROR_SOURCE_FFI, POLYCALL_CORE_ERROR_UNSUPPORTED_OPERATION,
      POLYCALL_ERROR_SEVERITY_ERROR,
 ...
 */
static polycall_core_error_t
js_convert_from_native(polycall_core_context_t *ctx, const void *src,
                       ffi_type_info_t *src_type, ffi_value_t *dest) {
  // This function is called by the FFI system to convert a native value to an
  // FFI value In the case of JavaScript, this means converting from a
  // JavaScript engine-specific value
 ...
 */
static polycall_core_error_t js_register_function(polycall_core_context_t *ctx,
                                                  const char *function_name,
                                                  void *function_ptr,
                                                  ffi_signature_t *signature,
                                                  uint32_t flags) {
  // This is called when another language wants to register a function for
  // JavaScript to call

  // Get the JavaScript bridge from the user_data field
  polycall_js_bridge_t *js_bridge = NULL;

  // In reality, we'd need to find the js_bridge instance, perhaps from a global
  // registry or from ctx/function_ptr (depends on how the FFI system is
  // implemented)
  ...
  // Implementation would create a JavaScript function that wraps the native
  // function Then register that JavaScript function in the JavaScript runtime
 ...
 */
static polycall_core_error_t
js_call_function(polycall_core_context_t *ctx, const char *function_name,
                 ffi_value_t *args, size_t arg_count, ffi_value_t *result) {
  ...
  // In reality, we'd need to find the js_bridge instance, perhaps from a global
  // registry or from ctx/function_name (depends on how the FFI system is
  // implemented)
  ...
  return polycall_js_bridge_call_function(ctx, js_bridge->ffi_ctx, js_bridge,
                                          function_name, args, arg_count,
                                          result);
}
 ...
 */
static polycall_core_error_t js_acquire_memory(polycall_core_context_t *ctx,
                                               void *ptr, size_t size) {
  // This is called when JavaScript needs to acquire memory allocated by another
  // language
  ...
  // In reality, we'd need to find the js_bridge instance, perhaps from a global
  // registry or from ctx/ptr (depends on how the FFI system is implemented)
 ...
 */
static polycall_core_error_t js_release_memory(polycall_core_context_t *ctx,
                                               void *ptr) {
  // This is called when JavaScript needs to release memory it acquired from
  // another language
  ...
  // For JavaScript, we would need to release any ArrayBuffer that's viewing the
  // native memory
 ...
 * @brief Handle JavaScript exception
 */
static polycall_core_error_t js_handle_exception(polycall_core_context_t *ctx,
  ...
  // In reality, we'd need to find the js_bridge instance, perhaps from a global
  // registry or from ctx (depends on how the FFI system is implemented)
  ...
  return polycall_js_bridge_handle_exception(ctx, js_bridge->ffi_ctx, js_bridge,
                                             exception, message, message_size);
}

/**
 * @brief Get JavaScript environment for current thread
 */
static polycall_core_error_t js_get_env(polycall_core_context_t *ctx,
                                        void **js_env) {
  if (!ctx || !js_env) {
    return POLYCALL_CORE_ERROR_INVALID_PARAMETERS;
  }
  ...
  *js_env = js_bridge->runtime_handle;
  ...
  if (!*js_env) {
    POLYCALL_ERROR_SET(
        ctx, POLYCALL_ERROR_SOURCE_FFI, POLYCALL_CORE_ERROR_INVALID_STATE,
        POLYCALL_ERROR_SEVERITY_ERROR, "JavaScript environment handle is NULL");
    return POLYCALL_CORE_ERROR_INVALID_STATE;
  }
