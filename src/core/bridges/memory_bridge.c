...
ownership_registry_find(ownership_registry_t *registry, void *ptr) {
  if (!registry || !ptr) {
    return NULL;
  }
...
ownership_registry_init(polycall_core_context_t *ctx,
  ...
  ownership_registry_t *new_registry =
      polycall_core_malloc(ctx, sizeof(ownership_registry_t));
  if (!new_registry) {
  ...
  if (initial_capacity > 0) {
    new_registry->regions = polycall_core_malloc(
        ctx, initial_capacity * sizeof(memory_region_descriptor_t *));
 ...
 */
static void ownership_registry_cleanup(polycall_core_context_t *ctx,
                                       ownership_registry_t *registry) {
  if (!ctx || !registry) {
    return;
  }
 ...
 */
static bool ownership_registry_add(ownership_registry_t *registry,
 ...
 */
static bool ownership_registry_remove(ownership_registry_t *registry,
                                      memory_region_descriptor_t *region_desc) {
  if (!registry || !region_desc) {
    return false;
  }
 ...
 */
static polycall_core_error_t
reference_counter_init(polycall_core_context_t *ctx,
                       reference_counter_t **counter, size_t initial_capacity) {
  if (!ctx || !counter) {
    return POLYCALL_CORE_ERROR_INVALID_PARAMETERS;
  }

  reference_counter_t *new_counter =
      polycall_core_malloc(ctx, sizeof(reference_counter_t));
  if (!new_counter) {
    return POLYCALL_CORE_ERROR_OUT_OF_MEMORY;
  }
  ...
  if (initial_capacity > 0) {
    new_counter->ptrs =
        polycall_core_malloc(ctx, initial_capacity * sizeof(void *));
    if (!new_counter->ptrs) {
    ...
    new_counter->counts =
        polycall_core_malloc(ctx, initial_capacity * sizeof(uint32_t));
 ...
 */
static void reference_counter_cleanup(polycall_core_context_t *ctx,
 ...
 */
static bool reference_counter_increment(polycall_core_context_t *ctx,
                                        reference_counter_t *counter,
                                        void *ptr) {
  if (!ctx || !counter || !ptr) {
    return false;
  ...
  if (counter->count >= counter->capacity) {
    ...
    void **new_ptrs = polycall_core_realloc(ctx, counter->ptrs,
    ...
    uint32_t *new_counts = polycall_core_realloc(
        ctx, counter->counts, new_capacity * sizeof(uint32_t));
    if (!new_counts) {
 ...
 */
static uint32_t reference_counter_decrement(polycall_core_context_t *ctx,
                                            reference_counter_t *counter,
                                            void *ptr) {
  if (!ctx || !counter || !ptr) {
    return 0;
 ...
 */
static uint32_t reference_counter_get_count(reference_counter_t *counter,
                                            void *ptr) {
  if (!counter || !ptr) {
    return 0;
  }
...
polycall_core_error_t polycall_memory_bridge_init(
    ...
    memory_manager_t **memory_mgr, const memory_bridge_config_t *config) {
  ...
  if (!mgr) {
    POLYCALL_ERROR_SET(
        ctx, POLYCALL_ERROR_SOURCE_MEMORY, POLYCALL_CORE_ERROR_OUT_OF_MEMORY,
        POLYCALL_ERROR_SEVERITY_ERROR, "Failed to allocate memory manager");
    return POLYCALL_CORE_ERROR_OUT_OF_MEMORY;
  }
  ...
  mgr->config.flags =
      POLYCALL_MEMORY_CONFIG_THREAD_SAFE; // Default: thread safe
  ...
  if (config->shared_pool_size > 0) {
    ...
    if (pool_result != POLYCALL_CORE_SUCCESS) {
      ...
      polycall_core_free(ctx, mgr);
      POLYCALL_ERROR_SET(ctx, POLYCALL_ERROR_SOURCE_MEMORY, pool_result,
                         POLYCALL_ERROR_SEVERITY_ERROR,
  ...
  polycall_core_error_t owner_result =
      ownership_registry_init(ctx, &mgr->ownership, config->ownership_capacity);
  ...
  polycall_core_error_t ref_result =
      reference_counter_init(ctx, &mgr->ref_counts, config->reference_capacity);
  ...
  if (config->enable_gc_notification) {
    ...
    mgr->gc_callbacks = polycall_core_malloc(
        ctx, mgr->gc_callback_capacity * sizeof(gc_callback_entry_t));
 ...
 */
void polycall_memory_bridge_cleanup(polycall_core_context_t *ctx,
 ...
 */
void *polycall_memory_alloc_shared(polycall_core_context_t *ctx,
                                   polycall_ffi_context_t *ffi_ctx,
                                   memory_manager_t *memory_mgr, size_t size,
  ...
  void *ptr = NULL;
  if (memory_mgr->shared_pool) {
    ptr = polycall_memory_alloc(ctx, memory_mgr->shared_pool, size,
  ...
  memory_region_descriptor_t *desc =
      polycall_core_malloc(ctx, sizeof(memory_region_descriptor_t));
  if (!desc) {
    if (memory_mgr->shared_pool) {
...
polycall_core_error_t polycall_memory_free_shared(
    polycall_core_context_t *ctx, polycall_ffi_context_t *ffi_ctx,
    memory_manager_t *memory_mgr, void *ptr, const char *language) {
  ...
  memory_region_descriptor_t *desc =
      ownership_registry_find(memory_mgr->ownership, ptr);
  if (!desc) {
    POLYCALL_ERROR_SET(ctx, POLYCALL_ERROR_SOURCE_MEMORY,
  ...
  if (strcmp(desc->owner, language) != 0) {
    POLYCALL_ERROR_SET(
        ctx, POLYCALL_ERROR_SOURCE_MEMORY,
        POLYCALL_CORE_ERROR_PERMISSION_DENIED, POLYCALL_ERROR_SEVERITY_ERROR,
        "Cannot free memory owned by '%s' from '%s'", desc->owner, language);
    return POLYCALL_CORE_ERROR_PERMISSION_DENIED;
  }

  // Decrement reference count
  uint32_t new_count =
      reference_counter_decrement(ctx, memory_mgr->ref_counts, ptr);
...
polycall_memory_share(polycall_core_context_t *ctx,
                      polycall_ffi_context_t *ffi_ctx,
                      memory_manager_t *memory_mgr, void *ptr, size_t size,
  ...
  memory_region_descriptor_t *existing_desc =
      ownership_registry_find(memory_mgr->ownership, ptr);
  if (existing_desc) {
  ...
  memory_region_descriptor_t *new_desc =
      polycall_core_malloc(ctx, sizeof(memory_region_descriptor_t));
  if (!new_desc) {
    POLYCALL_ERROR_SET(ctx, POLYCALL_ERROR_SOURCE_MEMORY,
                       POLYCALL_CORE_ERROR_OUT_OF_MEMORY,
 ...
 */
polycall_core_error_t
polycall_memory_track_reference(polycall_core_context_t *ctx,
                                polycall_ffi_context_t *ffi_ctx,
                                memory_manager_t *memory_mgr, void *ptr,
                                size_t size, const char *language) {
 ...
 */
polycall_core_error_t polycall_memory_release(polycall_core_context_t *ctx,
                                              polycall_ffi_context_t *ffi_ctx,
                                              memory_manager_t *memory_mgr,
                                              void *ptr, const char *language) {
  if (!ctx || !ffi_ctx || !memory_mgr || !ptr || !language) {
    return POLYCALL_CORE_ERROR_INVALID_PARAMETERS;
  }
  ...
  if (new_count == 0 && (desc->flags & POLYCALL_MEMORY_FLAG_AUTO_FREE)) {
    ...
    if (strcmp(desc->owner, language) != 0) {
      POLYCALL_ERROR_SET(
          ...
          POLYCALL_CORE_ERROR_PERMISSION_DENIED,
          POLYCALL_ERROR_SEVERITY_WARNING,
          "Non-owner '%s' cannot fully release memory owned by '%s'", language,
          desc->owner);
      return POLYCALL_CORE_ERROR_PERMISSION_DENIED;
    }
 ...
 */
polycall_core_error_t
polycall_memory_synchronize(polycall_core_context_t *ctx,
                            polycall_ffi_context_t *ffi_ctx,
                            memory_manager_t *memory_mgr, void *ptr,
                            size_t size, const char *source_language) {
  if (!ctx || !ffi_ctx || !memory_mgr || !ptr || size == 0 ||
      !source_language) {
    return POLYCALL_CORE_ERROR_INVALID_PARAMETERS;
  }
  ...
  if (memory_mgr->config.flags & POLYCALL_MEMORY_CONFIG_STRICT_OWNERSHIP) {
    if (strcmp(desc->owner, source_language) != 0) {
      // The requester is not the owner
      if (!memory_mgr->config.is_compatible_language ||
          !memory_mgr->config.is_compatible_language(desc->owner,
                                                     source_language)) {
        POLYCALL_ERROR_SET(ctx, POLYCALL_ERROR_SOURCE_MEMORY,
                           POLYCALL_CORE_ERROR_INCOMPATIBLE_LANGUAGE,
                           POLYCALL_ERROR_SEVERITY_ERROR,
  ...
  // This is architecture-dependent, so we'll assume a memory barrier is
  // sufficient
  __sync_synchronize();
...
polycall_core_error_t polycall_memory_register_gc_callback(
    polycall_core_context_t *ctx, polycall_ffi_context_t *ffi_ctx,
    memory_manager_t *memory_mgr, const char *language,
    gc_notification_callback_t callback, void *user_data) {
  ...
  if (memory_mgr->gc_callback_count >= memory_mgr->gc_callback_capacity) {
    ...
    gc_callback_entry_t *new_callbacks =
        polycall_core_realloc(ctx, memory_mgr->gc_callbacks,
 ...
 */
polycall_core_error_t polycall_memory_notify_gc(polycall_core_context_t *ctx,
                                                polycall_ffi_context_t *ffi_ctx,
                                                memory_manager_t *memory_mgr,
                                                const char *language) {
 ...
 */
polycall_core_error_t
polycall_memory_get_region_info(polycall_core_context_t *ctx,
                                polycall_ffi_context_t *ffi_ctx,
                                memory_manager_t *memory_mgr, void *ptr,
                                memory_region_descriptor_t **region_desc) {
...
polycall_core_error_t polycall_memory_create_snapshot(
    polycall_core_context_t *ctx, polycall_ffi_context_t *ffi_ctx,
    memory_manager_t *memory_mgr, const char *language, uint32_t *snapshot_id) {
  ...
  if (!g_snapshot_registry.snapshots) {
    ...
    g_snapshot_registry.snapshots = polycall_core_malloc(
        ctx, g_snapshot_registry.capacity * sizeof(memory_snapshot_t));
  ...
  if (g_snapshot_registry.count >= g_snapshot_registry.capacity) {
    ...
    memory_snapshot_t *new_snapshots =
        polycall_core_realloc(ctx, g_snapshot_registry.snapshots,
                              new_capacity * sizeof(memory_snapshot_t));

    if (!new_snapshots) {
      POLYCALL_ERROR_SET(
          ctx, POLYCALL_ERROR_SOURCE_MEMORY, POLYCALL_CORE_ERROR_OUT_OF_MEMORY,
          POLYCALL_ERROR_SEVERITY_ERROR, "Failed to expand snapshot registry");
      return POLYCALL_CORE_ERROR_OUT_OF_MEMORY;
    }
  ...
  memory_snapshot_t *snapshot =
      &g_snapshot_registry.snapshots[g_snapshot_registry.count];
  ...
  snapshot->regions = polycall_core_malloc(
      ctx, snapshot->region_count * sizeof(memory_region_descriptor_t *));
  ...
  for (size_t i = 0; i < memory_mgr->ownership->count; i++) {
    ...
    memory_region_descriptor_t *copy =
        polycall_core_malloc(ctx, sizeof(memory_region_descriptor_t));
...
polycall_core_error_t polycall_memory_restore_snapshot(
    polycall_core_context_t *ctx, polycall_ffi_context_t *ffi_ctx,
    memory_manager_t *memory_mgr, uint32_t snapshot_id, const char *language) {
  if (!ctx || !ffi_ctx || !memory_mgr || !language) {
    return POLYCALL_CORE_ERROR_INVALID_PARAMETERS;
  }
  ...
  for (size_t i = 0; i < snapshot->region_count; i++) {
    ...
    void *ptr = NULL;
    if (memory_mgr->shared_pool) {
      ptr = polycall_memory_alloc(ctx, memory_mgr->shared_pool, src->size,
                                  POLYCALL_MEMORY_FLAG_NONE);
    } else {
