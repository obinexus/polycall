...
cobol_convert_to_native(polycall_core_context_t *ctx, const ffi_value_t *src,
                        void *dest, ffi_type_info_t *dest_type);

static polycall_core_error_t
cobol_convert_from_native(polycall_core_context_t *ctx, const void *src,
                          ffi_type_info_t *src_type, ffi_value_t *dest);

static polycall_core_error_t
cobol_register_function(polycall_core_context_t *ctx, const char *function_name,
                        void *function_ptr, ffi_signature_t *signature,
                        uint32_t flags);

static polycall_core_error_t
cobol_call_function(polycall_core_context_t *ctx, const char *function_name,
                    ffi_value_t *args, size_t arg_count, ffi_value_t *result);

static polycall_core_error_t cobol_acquire_memory(polycall_core_context_t *ctx,
                                                  void *ptr, size_t size);

static polycall_core_error_t cobol_release_memory(polycall_core_context_t *ctx,
                                                  void *ptr);

static polycall_core_error_t
cobol_handle_exception(polycall_core_context_t *ctx, void *exception,
                       char *message, size_t message_size);

static polycall_core_error_t cobol_initialize(polycall_core_context_t *ctx);

static void cobol_cleanup(polycall_core_context_t *ctx);

// Helper functions
static cobol_program_t *find_program(cobol_program_registry_t *registry,
                                     const char *name) {
...
static polycall_core_error_t
init_program_registry(polycall_core_context_t *ctx,
                      cobol_program_registry_t *registry, size_t capacity) {
  registry->programs =
      polycall_core_malloc(ctx, capacity * sizeof(cobol_program_t));
...
static void cleanup_program_registry(polycall_core_context_t *ctx,
...
static polycall_core_error_t init_type_registry(polycall_core_context_t *ctx,
                                                cobol_type_registry_t *registry,
                                                size_t capacity) {
  registry->mappings =
      polycall_core_malloc(ctx, capacity * sizeof(cobol_type_mapping_t));
...
static void cleanup_type_registry(polycall_core_context_t *ctx,
                                  cobol_type_registry_t *registry) {
  if (!registry->mappings) {
    return;
  }
...
static polycall_core_error_t
register_cobol_types(polycall_core_context_t *ctx,
                     polycall_ffi_context_t *ffi_ctx,
...
static polycall_core_error_t
initialize_cobol_runtime(polycall_cobol_bridge_t *cobol_bridge) {
...
polycall_cobol_bridge_init(polycall_core_context_t *ctx,
                           polycall_ffi_context_t *ffi_ctx,
  ...
  *cobol_bridge = polycall_core_malloc(ctx, sizeof(polycall_cobol_bridge_t));
  if (!*cobol_bridge) {
    POLYCALL_ERROR_SET(
        ...
        POLYCALL_ERROR_SEVERITY_ERROR, "Failed to allocate COBOL bridge");
    return POLYCALL_CORE_ERROR_OUT_OF_MEMORY;
  }
  ...
  (*cobol_bridge)->enable_copybook_integration =
      config->enable_copybook_integration;
  ...
  polycall_core_error_t result =
      init_program_registry(ctx, &(*cobol_bridge)->program_registry, 64);
  ...
  // Initialize type registry
  result = init_type_registry(ctx, &(*cobol_bridge)->type_registry, 128);
  ...
  (*cobol_bridge)->bridge_interface.convert_from_native =
      cobol_convert_from_native;
 ...
 */
void polycall_cobol_bridge_cleanup(polycall_core_context_t *ctx,
                                   polycall_ffi_context_t *ffi_ctx,
                                   polycall_cobol_bridge_t *cobol_bridge) {
...
polycall_core_error_t polycall_cobol_bridge_register_program(
    ...
    polycall_cobol_bridge_t *cobol_bridge, const char *function_name,
    const char *program_name, const char *linkage_section_desc,
    uint32_t flags) {
  ...
  if (cobol_bridge->program_registry.count >=
      cobol_bridge->program_registry.capacity) {
    pthread_mutex_unlock(&cobol_bridge->program_registry.mutex);

    POLYCALL_ERROR_SET(
        ctx, POLYCALL_ERROR_SOURCE_FFI, POLYCALL_CORE_ERROR_OUT_OF_MEMORY,
        POLYCALL_ERROR_SEVERITY_ERROR, "COBOL program registry full");
    return POLYCALL_CORE_ERROR_OUT_OF_MEMORY;
  }

  // Add program to registry
  cobol_program_t *prog =
      &cobol_bridge->program_registry
           .programs[cobol_bridge->program_registry.count++];
  ...
  polycall_core_error_t result =
      polycall_ffi_create_signature(ctx, ffi_ctx,
  ...
  result = polycall_ffi_expose_function(
      ctx, ffi_ctx, function_name,
      cobol_bridge, // Use bridge as function pointer
      signature, "cobol", flags);
  ...
  if (result != POLYCALL_CORE_SUCCESS) {
    ...
    POLYCALL_ERROR_SET(
        ctx, POLYCALL_ERROR_SOURCE_FFI, result, POLYCALL_ERROR_SEVERITY_ERROR,
...
polycall_core_error_t polycall_cobol_bridge_call_program(
  ...
  pthread_mutex_lock(&cobol_bridge->program_registry.mutex);
  cobol_program_t *prog =
      find_program(&cobol_bridge->program_registry, function_name);
  pthread_mutex_unlock(&cobol_bridge->program_registry.mutex);
...
polycall_core_error_t polycall_cobol_bridge_parse_copybook(
    polycall_core_context_t *ctx, polycall_ffi_context_t *ffi_ctx,
    polycall_cobol_bridge_t *cobol_bridge, const char *copybook_path,
    const char *record_name, ffi_type_info_t **type_info) {
  if (!ctx || !ffi_ctx || !cobol_bridge || !copybook_path || !record_name ||
      !type_info) {
    return POLYCALL_CORE_ERROR_INVALID_PARAMETERS;
  }
...
polycall_core_error_t polycall_cobol_bridge_get_interface(
    polycall_core_context_t *ctx, polycall_ffi_context_t *ffi_ctx,
    polycall_cobol_bridge_t *cobol_bridge, language_bridge_t *bridge) {
...
polycall_cobol_bridge_create_default_config(void) {
...
cobol_convert_to_native(polycall_core_context_t *ctx, const ffi_value_t *src,
  ...
  if (src->type != dest_type->type) {
    ...
    POLYCALL_ERROR_SET(
        ...
        POLYCALL_ERROR_SEVERITY_ERROR, "Type mismatch: source=%d, dest=%d",
  ...
  case POLYCALL_FFI_TYPE_STRUCT:
    ...
    if (src->value.struct_value && dest_type->details.struct_info.size > 0) {
      memcpy(dest, src->value.struct_value,
             dest_type->details.struct_info.size);
    }
    break;
  ...
  default:
    POLYCALL_ERROR_SET(ctx, POLYCALL_ERROR_SOURCE_FFI,
                       POLYCALL_CORE_ERROR_UNSUPPORTED_OPERATION,
                       POLYCALL_ERROR_SEVERITY_ERROR, "Unsupported type: %d",
                       src->type);
    return POLYCALL_CORE_ERROR_UNSUPPORTED_OPERATION;
  }
...
cobol_convert_from_native(polycall_core_context_t *ctx, const void *src,
  ...
  case POLYCALL_FFI_TYPE_STRUCT:
    ...
    if (src_type->details.struct_info.size > 0) {
      void *struct_value =
          polycall_core_malloc(ctx, src_type->details.struct_info.size);
      if (!struct_value) {
        POLYCALL_ERROR_SET(ctx, POLYCALL_ERROR_SOURCE_FFI,
...
cobol_register_function(polycall_core_context_t *ctx, const char *function_name,
  ...
  // For COBOL, functions are registered through
  // polycall_cobol_bridge_register_program So we don't need to implement this
  // function
...
cobol_call_function(polycall_core_context_t *ctx, const char *function_name,
  ...
  if (!cobol_bridge) {
    POLYCALL_ERROR_SET(
        ...
        POLYCALL_ERROR_SEVERITY_ERROR, "COBOL bridge not found in context");
    return POLYCALL_CORE_ERROR_INVALID_STATE;
  }

  // Delegate to the bridge's call function
  return polycall_cobol_bridge_call_program(ctx, cobol_bridge->ffi_ctx,
                                            cobol_bridge, function_name, args,
                                            arg_count, result);
}

static polycall_core_error_t cobol_acquire_memory(polycall_core_context_t *ctx,
                                                  void *ptr, size_t size) {
  // This function is called when memory is shared from another language to
  // COBOL For now, we don't need to do anything special
...
static polycall_core_error_t cobol_release_memory(polycall_core_context_t *ctx,
                                                  void *ptr) {
  // This function is called when memory shared from COBOL to another language
  // is released For now, we don't need to do anything special
