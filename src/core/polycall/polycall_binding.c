...
polycall_error_t polycall_register_binding(const char *name,
                                           binding_init_fn init,
                                           binding_cleanup_fn cleanup,
                                           binding_process_fn process) {
...
polycall_error_t polycall_binding_init(polycall_context_t *ctx,
  ...
  for (int i = 0; i < binding_count; i++) {
    if (strcmp(bindings[i].name, binding_name) == 0) {
      // Initialize binding
      polycall_error_t result =
          bindings[i].init(ctx, binding_config, &bindings[i].binding_data);
      return result;
    }
  }
...
polycall_error_t polycall_binding_cleanup(polycall_context_t *ctx,
                                          const char *binding_name) {
  if (!ctx || !binding_name) {
    return POLYCALL_ERROR_INVALID_PARAMETERS;
  }
  ...
  for (int i = 0; i < binding_count; i++) {
    if (strcmp(bindings[i].name, binding_name) == 0) {
      // Clean up binding
      polycall_error_t result =
          bindings[i].cleanup(ctx, bindings[i].binding_data);
...
polycall_error_t polycall_binding_process_message(
    polycall_context_t *ctx, const char *binding_name,
    polycall_message_t *message, polycall_message_t **response) {
  ...
  for (int i = 0; i < binding_count; i++) {
    if (strcmp(bindings[i].name, binding_name) == 0) {
      // Process message
      return bindings[i].process(ctx, message, response,
                                 bindings[i].binding_data);
    }
  }
