...
static polycall_context_ref_t *
find_context_by_type_internal(context_registry_t *registry,
...
static polycall_context_ref_t *
find_context_by_name_internal(context_registry_t *registry, const char *name) {
  if (!registry || !name)
    return NULL;
...
polycall_context_init(polycall_core_context_t *core_ctx,
  ...
  polycall_context_ref_t *new_ctx =
      polycall_core_malloc(core_ctx, sizeof(polycall_context_ref_t));
  if (!new_ctx) {
    pthread_mutex_unlock(&registry->registry_lock);
    return POLYCALL_CORE_ERROR_OUT_OF_MEMORY;
  }
  ...
  if (init->init_fn) {
    polycall_core_error_t result =
        init->init_fn(core_ctx, new_ctx->data, init->init_data);
...
polycall_error_t polycall_context_unshare(polycall_context_t *core_ctx,
                                          polycall_context_ref_t *ctx_ref) {
  if (!core_ctx || !ctx_ref) {
...
polycall_context_find_by_type(polycall_core_context_t *core_ctx,
  ...
  pthread_mutex_lock(&registry->registry_lock);
  polycall_context_ref_t *result =
      find_context_by_type_internal(registry, type);
  pthread_mutex_unlock(&registry->registry_lock);
...
polycall_context_find_by_name(polycall_core_context_t *core_ctx,
  ...
  pthread_mutex_lock(&registry->registry_lock);
  polycall_context_ref_t *result =
      find_context_by_name_internal(registry, name);
  pthread_mutex_unlock(&registry->registry_lock);
