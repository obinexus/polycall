...
static const polycall_type_info_t type_registry[] = {
    ...
    {POLYCALL_CONTEXT_TYPE_PROTOCOL, "protocol",
     sizeof(polycall_protocol_context_t)},
    {POLYCALL_CONTEXT_TYPE_NETWORK, "network", sizeof(NetworkEndpoint)},
    {POLYCALL_CONTEXT_TYPE_MICRO, "micro",
     sizeof(void *)},                                     /* Placeholder size */
    {POLYCALL_CONTEXT_TYPE_EDGE, "edge", sizeof(void *)}, /* Placeholder size */
    {POLYCALL_CONTEXT_TYPE_PARSER, "parser",
     sizeof(void *)}, /* Placeholder size */
};
...
const char *polycall_log_level_to_string(polycall_log_level_t level) {
  switch (level) {
  case POLYCALL_LOG_DEBUG:
    return "DEBUG";
  case POLYCALL_LOG_INFO:
    return "INFO";
  case POLYCALL_LOG_WARNING:
    return "WARNING";
  case POLYCALL_LOG_ERROR:
    return "ERROR";
  case POLYCALL_LOG_FATAL:
    return "FATAL";
  default:
    return "UNKNOWN";
  }
...
polycall_log_level_t polycall_log_level_from_string(const char *str) {
  if (!str)
    return POLYCALL_LOG_INFO;

  if (strcasecmp(str, "debug") == 0)
    return POLYCALL_LOG_DEBUG;
  if (strcasecmp(str, "info") == 0)
    return POLYCALL_LOG_INFO;
  if (strcasecmp(str, "warning") == 0)
    return POLYCALL_LOG_WARNING;
  if (strcasecmp(str, "error") == 0)
    return POLYCALL_LOG_ERROR;
  if (strcasecmp(str, "fatal") == 0)
    return POLYCALL_LOG_FATAL;
 ...
 */
const char *
polycall_config_section_to_string(polycall_config_section_t section) {
  switch (section) {
  case POLYCALL_CONFIG_SECTION_CORE:
    return "core";
  case POLYCALL_CONFIG_SECTION_SECURITY:
    return "security";
  case POLYCALL_CONFIG_SECTION_MEMORY:
    return "memory";
  case POLYCALL_CONFIG_SECTION_TYPE:
    return "type";
  case POLYCALL_CONFIG_SECTION_PERFORMANCE:
    return "performance";
  case POLYCALL_CONFIG_SECTION_PROTOCOL:
    return "protocol";
  case POLYCALL_CONFIG_SECTION_C:
    return "c";
  case POLYCALL_CONFIG_SECTION_JVM:
    return "jvm";
  case POLYCALL_CONFIG_SECTION_JS:
    return "js";
  case POLYCALL_CONFIG_SECTION_PYTHON:
    return "python";
  default:
 ...
 */
const char *
polycall_config_value_type_to_string(polycall_config_value_type_t type) {
  switch (type) {
...
polycall_context_flags_combine(polycall_context_flags_t flags1,
 ...
 */
polycall_context_flags_t
polycall_context_flags_remove(polycall_context_flags_t flags,
...
struct polycall_config_value *polycall_config_value_create_boolean(int value) {
  struct polycall_config_value *val =
      calloc(1, sizeof(struct polycall_config_value));
  if (val) {
 ...
 */
struct polycall_config_value *
polycall_config_value_create_integer(int64_t value) {
  struct polycall_config_value *val =
      calloc(1, sizeof(struct polycall_config_value));
  if (val) {
 ...
 */
struct polycall_config_value *
polycall_config_value_create_string(const char *value) {
  if (!value)
    return NULL;
...
void polycall_config_value_free(struct polycall_config_value *value) {
  ...
  } else if (value->type == POLYCALL_CONFIG_VALUE_OBJECT &&
