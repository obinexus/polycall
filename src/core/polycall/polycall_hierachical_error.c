 ...
 */
static component_error_handler_t *
find_handler(polycall_hierarchical_error_context_t *error_ctx,
...
polycall_core_error_t polycall_hierarchical_error_init(
  ...
  // Allocate context
  polycall_hierarchical_error_context_t *new_ctx = polycall_core_malloc(
      core_ctx, sizeof(polycall_hierarchical_error_context_t));
  if (!new_ctx) {
...
polycall_core_error_t polycall_hierarchical_error_register_handler(
  ...
  if (config->parent_component[0] &&
      !find_handler(error_ctx, config->parent_component)) {
  ...
  component_error_handler_t *handler =
      polycall_core_malloc(core_ctx, sizeof(component_error_handler_t));
  if (!handler) {
    return POLYCALL_CORE_ERROR_OUT_OF_MEMORY;
  }
  ...
  strncpy(handler->component_name, config->component_name,
  ...
  strncpy(handler->parent_component, config->parent_component,
          POLYCALL_MAX_COMPONENT_NAME_LENGTH - 1);
 ...
 */
static void propagate_error(polycall_core_context_t *core_ctx,
                            polycall_hierarchical_error_context_t *error_ctx,
                            component_error_handler_t *handler,
  ...
  if (handler->propagation_mode == POLYCALL_ERROR_PROPAGATE_UPWARD ||
    ...
    if (handler->parent_component[0]) {
      component_error_handler_t *parent =
          find_handler(error_ctx, handler->parent_component);
      if (parent && parent->handler) {
        // Call parent handler
        parent->handler(core_ctx, handler->component_name, record->source,
                        record->code, record->severity, record->message,
                        parent->user_data);
  ...
  if (handler->propagation_mode == POLYCALL_ERROR_PROPAGATE_DOWNWARD ||
    ...
    component_error_handler_t *current = error_ctx->handlers;
    while (current) {
      if (strcmp(current->parent_component, handler->component_name) == 0 &&
          current->handler) {
        // Call child handler
...
polycall_core_error_t polycall_hierarchical_error_get_parent(
  ...
  if (!core_ctx || !error_ctx || !component_name || !parent_buffer ||
      buffer_size == 0) {
    return POLYCALL_CORE_ERROR_INVALID_PARAMETERS;
  }
