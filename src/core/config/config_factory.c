 ...
 * a consistent approach for component configuration creation, loading, and
validation
...
polycall_config_factory_init(polycall_core_context_t *ctx,
  ...
  polycall_config_factory_t *new_factory =
      polycall_core_malloc(ctx, sizeof(polycall_config_factory_t));
  if (!new_factory) {
  ...
  memcpy(&new_factory->options, options,
         sizeof(polycall_config_factory_options_t));
  ...
  polycall_core_error_t result =
      polycall_config_parser_init(ctx, &new_factory->parser, &parser_options);
  ...
  if (options->validate_configs) {
    result = polycall_schema_context_create(ctx, &new_factory->schema_ctx,
 ...
 */
void polycall_config_factory_cleanup(polycall_core_context_t *ctx,
                                     polycall_config_factory_t *factory) {
  if (!ctx || !factory) {
    return;
  }
...
polycall_core_error_t polycall_config_factory_create_component(
    polycall_core_context_t *ctx, polycall_config_factory_t *factory,
  ...
  case POLYCALL_COMPONENT_TYPE_EDGE: {
    polycall_edge_component_config_t *edge_config =
        polycall_core_malloc(ctx, sizeof(polycall_edge_component_config_t));
  ...
  case POLYCALL_COMPONENT_TYPE_NETWORK: {
    ...
    polycall_core_error_t result =
        polycall_network_config_create(ctx, &network_config, NULL);
  ...
  case POLYCALL_COMPONENT_TYPE_PROTOCOL: {
    protocol_config_t *protocol_config =
        polycall_core_malloc(ctx, sizeof(protocol_config_t));
  ...
  case POLYCALL_COMPONENT_TYPE_FFI: {
    polycall_ffi_config_options_t *ffi_config =
        polycall_core_malloc(ctx, sizeof(polycall_ffi_config_options_t));
  ...
  case POLYCALL_COMPONENT_TYPE_TELEMETRY: {
    polycall_telemetry_config_t *telemetry_config =
        polycall_core_malloc(ctx, sizeof(polycall_telemetry_config_t));
...
polycall_core_error_t polycall_config_factory_load_from_file(
    polycall_core_context_t *ctx, polycall_config_factory_t *factory,
    const char *file_path, polycall_config_load_flags_t flags) {
  ...
  result = polycall_config_parser_parse_file(factory->parser, resolved_path,
                                             &config_root);
...
polycall_core_error_t polycall_config_factory_extract_component(
  ...
  default:
    ...
    polycall_config_factory_free_component(ctx, factory, component_type,
                                           *config_out);
    *config_out = NULL;
  ...
  polycall_config_node_t *components_section =
      polycall_config_find_node(factory->global_config, section_path);
  ...
  if (component_name[0] != '\0' &&
    ...
    snprintf(component_path, sizeof(component_path), "%s.%s", section_path,
             component_name);

    polycall_config_node_t *component_section =
        polycall_config_find_node(factory->global_config, component_path);
...
void polycall_config_factory_free_component(
    polycall_core_context_t *ctx, polycall_config_factory_t *factory,
    polycall_component_type_t component_type, void *component_config) {
  ...
  case POLYCALL_COMPONENT_TYPE_PROTOCOL: {
    ...
    protocol_config_t *protocol_config = (protocol_config_t *)component_config;
 ...
 */
const char *
polycall_config_factory_get_error(polycall_core_context_t *ctx,
                                  polycall_config_factory_t *factory) {
 ...
 */
void polycall_config_factory_clear_error(polycall_core_context_t *ctx,
                                         polycall_config_factory_t *factory) {
  if (!ctx || !factory) {
    return;
  }
