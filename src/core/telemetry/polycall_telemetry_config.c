 ...
 */
typedef struct {
  void (*callback)(const polycall_telemetry_config_t *new_config,
                   void *user_data);
  void *user_data;
 ...
 */
static bool validate_config_context(polycall_telemetry_config_context_t *ctx) {
 ...
 */
static void
notify_config_callbacks(polycall_telemetry_config_context_t *config_ctx) {
  ...
  for (size_t i = 0; i < config_ctx->callback_count; i++) {
    if (config_ctx->callbacks[i].callback) {
      config_ctx->callbacks[i].callback(&config_ctx->config,
...
polycall_telemetry_config_init(polycall_core_context_t *core_ctx,
  ...
  polycall_telemetry_config_context_t *new_ctx = polycall_core_malloc(
      core_ctx, sizeof(polycall_telemetry_config_context_t));
 ...
 */
polycall_core_error_t
polycall_telemetry_config_load(polycall_telemetry_config_context_t *config_ctx,
  ...
  strncpy(config_ctx->config_file_path, file_path,
          sizeof(config_ctx->config_file_path) - 1);
 ...
 */
polycall_core_error_t
polycall_telemetry_config_save(polycall_telemetry_config_context_t *config_ctx,
                               const char *file_path) {
  if (!validate_config_context(config_ctx)) {
    return POLYCALL_CORE_ERROR_INVALID_PARAMETERS;
  }
  ...
  if (!config_ctx->has_config_file && file_path) {
    strncpy(config_ctx->config_file_path, file_path,
            sizeof(config_ctx->config_file_path) - 1);
    config_ctx->config_file_path[sizeof(config_ctx->config_file_path) - 1] =
        '\0';
    config_ctx->has_config_file = true;
  }
 ...
 */
polycall_core_error_t
polycall_telemetry_config_apply(polycall_telemetry_config_context_t *config_ctx,
  ...
  telemetry_config.log_file_path[sizeof(telemetry_config.log_file_path) - 1] =
      '\0';
...
polycall_core_error_t polycall_telemetry_config_update_param(
    polycall_telemetry_config_context_t *config_ctx, const char *param_name,
  ...
  if (strcmp(param_name, "enable_telemetry") == 0) {
    ...
    config_ctx->config.min_severity =
        *((polycall_telemetry_severity_t *)param_value);
    param_updated = true;
    ...
    config_ctx->config.destination =
        *((polycall_telemetry_destination_t *)param_value);
    param_updated = true;
    ...
    config_ctx->config.output_path[sizeof(config_ctx->config.output_path) - 1] =
        '\0';
    param_updated = true;
  }
 ...
 */
polycall_core_error_t
polycall_telemetry_config_get(polycall_telemetry_config_context_t *config_ctx,
...
bool polycall_telemetry_config_validate(
    const polycall_telemetry_config_t *config, char *error_message,
    size_t buffer_size) {
  if (!config) {
  ...
  if (config->max_event_queue_size == 0) {
    if (error_message && buffer_size > 0) {
      strncpy(error_message, "max_event_queue_size must be greater than 0",
              buffer_size - 1);
      error_message[buffer_size - 1] = '\0';
    }
    return false;
  ...
  if (config->sampling_mode != TELEMETRY_SAMPLING_NONE &&
    ...
    if (error_message && buffer_size > 0) {
      strncpy(error_message, "sampling_rate must be between 0.0 and 1.0",
              buffer_size - 1);
      error_message[buffer_size - 1] = '\0';
    }
    return false;
  ...
  if (config->destination == TELEMETRY_DEST_FILE &&
    ...
    if (error_message && buffer_size > 0) {
      strncpy(error_message, "output_path required for file destination",
              buffer_size - 1);
      error_message[buffer_size - 1] = '\0';
    }
    return false;
