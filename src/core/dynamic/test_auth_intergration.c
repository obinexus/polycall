...
static bool test_setup(void) {
  ...
  auth_config.token_signing_secret =
  ...
  if (polycall_auth_register_identity(core_ctx, auth_ctx, "test_identity",
                                      &attributes,
                                      TEST_PASSWORD) != POLYCALL_CORE_SUCCESS) {
  ...
  if (polycall_auth_add_role(core_ctx, auth_ctx, &role) !=
      POLYCALL_CORE_SUCCESS) {
  ...
  if (polycall_auth_assign_role(core_ctx, auth_ctx, "test_identity",
                                "test_role") != POLYCALL_CORE_SUCCESS) {
  ...
  if (polycall_auth_add_policy(core_ctx, auth_ctx, &policy) !=
      POLYCALL_CORE_SUCCESS) {
  ...
  if (polycall_auth_attach_policy(core_ctx, auth_ctx, "test_role",
                                  "test_policy") != POLYCALL_CORE_SUCCESS) {
...
static bool test_authentication(void) {
  ...
  polycall_core_error_t result =
      polycall_auth_authenticate(core_ctx, auth_ctx, TEST_USERNAME,
                                 TEST_PASSWORD, &access_token, &refresh_token);
  ...
  result = polycall_auth_authenticate(core_ctx, auth_ctx, TEST_USERNAME,
                                      "wrong_password", &access_token,
                                      &refresh_token);
  ...
  result =
      polycall_auth_authenticate(core_ctx, auth_ctx, "nonexistent_user",
                                 TEST_PASSWORD, &access_token, &refresh_token);
...
static bool test_token_validation(void) {
  ...
  result = polycall_auth_validate_token(core_ctx, auth_ctx, invalid_token,
                                        &identity_id);
...
static bool test_token_refresh(void) {
  ...
  result = polycall_auth_refresh_token(core_ctx, auth_ctx, access_token,
                                       &new_access_token);
...
static bool test_permission_checking(void) {
  ...
  result = polycall_auth_check_permission(core_ctx, auth_ctx, identity_id,
                                          TEST_RESOURCE, TEST_ACTION, &allowed);
  ...
  allowed = false;
  result = polycall_auth_check_permission(core_ctx, auth_ctx, identity_id,
                                          TEST_RESOURCE, "unauthorized_action",
                                          &allowed);
...
static bool test_zero_trust_security(void) {
  ...
  polycall_core_error_t result =
      polycall_auth_apply_zero_trust_constraints(core_ctx, &unsafe_config);
...
static bool test_config_loading(void) {
  ...
  base_config.token_signing_secret =
      "base_signing_secret_with_sufficient_length";
  base_config.token_validity_period_sec = 1800; // 30 minutes

  polycall_auth_config_t override_config =
      polycall_auth_create_default_config();
  override_config.token_validity_period_sec = 900; // 15 minutes
  override_config.token_signing_secret =
      "override_signing_secret_with_sufficient_length";
  ...
  if (merged_config.token_validity_period_sec !=
      override_config.token_validity_period_sec) {
  ...
  if (strcmp(merged_config.token_signing_secret,
             override_config.token_signing_secret) != 0) {
    fprintf(stderr,
            "Merged config did not properly override token signing secret\n");
    polycall_auth_cleanup_config(core_ctx, &base_config);
    polycall_auth_cleanup_config(core_ctx, &override_config);
    polycall_auth_cleanup_config(core_ctx, &merged_config);
    return false;
...
static bool test_concurrent_validation(void) {
  ...
  for (int i = 0; i < THREAD_COUNT; i++) {
    ...
    if (pthread_create(&threads[i], NULL, validation_thread, &contexts[i]) !=
        0) {
  ...
  return total_success == THREAD_COUNT * ITERATIONS_PER_THREAD &&
         total_failure == 0;
}
...
int main(void) {
  ...
  // Add tests
  test_case_t tests[] = {{"Authentication", test_authentication},
