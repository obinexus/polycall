...
int main(void) {
  ...
  POLYCALL_TEST_ASSERT(test_ctx->telemetry_ctx != NULL,
                       "telemetry context initialized");

  // Generate some telemetry events
  polycall_telemetry_record_operation(
      test_ctx->core_ctx, test_ctx->telemetry_ctx, "test_operation", 100);
  polycall_telemetry_record_operation(
      test_ctx->core_ctx, test_ctx->telemetry_ctx, "test_operation", 150);
