...
static polycall_core_error_t test_message_handler(polycall_core_context_t *ctx,
...
static int test_set_payload(void) {
  ...
  result =
      polycall_message_set_payload(test_ctx, message, new_payload, new_size);
...
static int test_set_flags(void) {
  ...
  polycall_message_flags_t flags =
      POLYCALL_MESSAGE_FLAG_ENCRYPTED | POLYCALL_MESSAGE_FLAG_REQUIRES_ACK;

  polycall_core_error_t result =
      polycall_message_set_flags(test_ctx, message, flags);
  ...
  // Get flags
  polycall_message_flags_t retrieved_flags =
      polycall_message_get_flags(message);
...
static int test_serialization(void) {
  ...
  polycall_message_set_payload(test_ctx, message, test_payload,
                               strlen(test_payload) + 1);
  polycall_message_set_metadata(test_ctx, message, test_metadata,
                                strlen(test_metadata) + 1);
  polycall_message_set_flags(test_ctx, message,
                             POLYCALL_MESSAGE_FLAG_REQUIRES_ACK);
  ...
  polycall_core_error_t result =
      polycall_message_serialize(test_ctx, message, &buffer, &buffer_size);
  ...
  result = polycall_message_deserialize(test_ctx, buffer, buffer_size,
                                        &deserialized);
  ...
  ASSERT_EQUAL_INT(message->header.payload_size,
                   deserialized->header.payload_size);
  ASSERT_EQUAL_INT(message->header.metadata_size,
                   deserialized->header.metadata_size);

  // Verify payload and metadata
  ASSERT_MEMORY_EQUAL(test_payload, deserialized->payload,
                      strlen(test_payload) + 1);
  ASSERT_MEMORY_EQUAL(test_metadata, deserialized->metadata,
                      strlen(test_metadata) + 1);
...
static int test_string_payload(void) {
  ...
  polycall_core_error_t result =
      polycall_message_set_string_payload(test_ctx, message, test_string);
...
static int test_message_handlers(void) {
  ...
  polycall_core_error_t result = polycall_message_register_handler(
      test_ctx, POLYCALL_MESSAGE_TYPE_HANDSHAKE, test_message_handler,
      &handler_counter);
