...
START_TEST(test_protocol_message_get_content) {
  ...
  polycall_protocol_message_t *message = NULL;
  polycall_protocol_create_message(core_ctx, protocol_ctx, test_data,
                                   strlen(test_data), &message);
  assert_non_null(message);
  ...
  polycall_core_error_t result =
      polycall_protocol_get_message_content(message, &content, &content_size);
