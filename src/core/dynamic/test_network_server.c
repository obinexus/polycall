 ...
 *
 * This file contains unit tests for the server-side networking interface in
 * LibPolyCall.
 */
...
static void test_connection_callback(polycall_network_server_t *server,
...
static void test_error_callback(polycall_network_server_t *server,
...
test_message_handler(polycall_core_context_t *ctx,
...
static void test_event_handler(polycall_network_server_t *server,
...
void test_server_create_cleanup(void) {
  ...
  polycall_core_error_t result =
      polycall_network_server_create(NULL, proto_ctx, &server, NULL);
...
void test_server_start_stop(void) {
  ...
  // (it would try to bind to a real port), we'll just test the parameter
  // validation.
...
void test_server_accept_disconnect(void) {
  ...
  polycall_core_error_t result =
      polycall_network_server_accept(NULL, server, &endpoint, 1000);
  assert(result == POLYCALL_CORE_ERROR_INVALID_PARAMETERS);
...
void test_server_send_receive(void) {
  ...
  polycall_core_error_t result =
      polycall_network_server_send(NULL, server, endpoint, packet, 1000);
  assert(result == POLYCALL_CORE_ERROR_INVALID_PARAMETERS);
  ...
  result = polycall_network_server_receive(NULL, server, endpoint, &recv_packet,
                                           1000);
  assert(result == POLYCALL_CORE_ERROR_INVALID_PARAMETERS);

  result = polycall_network_server_receive(core_ctx, NULL, endpoint,
                                           &recv_packet, 1000);
  assert(result == POLYCALL_CORE_ERROR_INVALID_PARAMETERS);

  result = polycall_network_server_receive(core_ctx, server, NULL, &recv_packet,
                                           1000);
  assert(result == POLYCALL_CORE_ERROR_INVALID_PARAMETERS);

  result =
      polycall_network_server_receive(core_ctx, server, endpoint, NULL, 1000);
  assert(result == POLYCALL_CORE_ERROR_INVALID_PARAMETERS);
...
void test_server_send_message(void) {
  ...
  result = polycall_network_server_send_message(core_ctx, server, proto_ctx,
...
void test_server_broadcast(void) {
  ...
  // Test with NULL parameters
  polycall_core_error_t result =
      polycall_network_server_broadcast(NULL, server, packet, 1000);
  assert(result == POLYCALL_CORE_ERROR_INVALID_PARAMETERS);
