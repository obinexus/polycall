...
static int test_add_states(void) {
  ...
  status = polycall_sm_add_state(test_sm, "active", test_state_callback,
                                 test_state_callback, false);
  ASSERT_EQUAL_INT(POLYCALL_SM_SUCCESS, status);
...
static int test_add_transitions(void) {
  ...
  // Add transitions
  status = polycall_sm_add_transition(test_sm, "to_handshake", "init",
                                      "handshake", NULL, NULL);
  ASSERT_EQUAL_INT(POLYCALL_SM_SUCCESS, status);

  status = polycall_sm_add_transition(test_sm, "to_auth", "handshake", "auth",
                                      NULL, NULL);
  ASSERT_EQUAL_INT(POLYCALL_SM_SUCCESS, status);

  status = polycall_sm_add_transition(test_sm, "to_ready", "auth", "ready",
                                      NULL, NULL);
  ASSERT_EQUAL_INT(POLYCALL_SM_SUCCESS, status);

  // Add transition with guard function
  int guard_value = 1;
  status = polycall_sm_add_transition(test_sm, "back_to_init", "ready", "init",
                                      test_guard_func, &guard_value);
  ASSERT_EQUAL_INT(POLYCALL_SM_SUCCESS, status);

  // Try to add transition between non-existent states - should fail
  status = polycall_sm_add_transition(test_sm, "invalid", "nonexistent",
                                      "ready", NULL, NULL);
...
static int test_execute_transitions(void) {
  ...
  // Add transitions
  polycall_sm_add_transition(test_sm, "to_handshake", "init", "handshake", NULL,
                             NULL);
  polycall_sm_add_transition(test_sm, "to_auth", "handshake", "auth", NULL,
                             NULL);
  polycall_sm_add_transition(test_sm, "to_ready", "auth", "ready", NULL, NULL);
  polycall_sm_add_transition(test_sm, "to_locked", "ready", "locked", NULL,
                             NULL);

  int guard_value = 1;
  polycall_sm_add_transition(test_sm, "back_to_init", "ready", "init",
                             test_guard_func, &guard_value);
...
static int test_get_state(void) {
  ...
  // Get state name
  polycall_sm_status_t status =
      polycall_sm_get_current_state(test_sm, state_name, sizeof(state_name));
  ASSERT_EQUAL_INT(POLYCALL_SM_SUCCESS, status);
...
static int test_lock_unlock_states(void) {
  ...
  // Add transition
  polycall_sm_add_transition(test_sm, "to_secure", "init", "secure", NULL,
                             NULL);
