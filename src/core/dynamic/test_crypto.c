...
static int test_key_exchange(void) {
  ...
  polycall_crypto_get_public_key(test_ctx, test_crypto_ctx, &local_pubkey,
                                 &local_pubkey_size);
  polycall_crypto_get_public_key(test_ctx, peer_ctx, &peer_pubkey,
                                 &peer_pubkey_size);
  ...
  result = polycall_crypto_set_remote_key(test_ctx, peer_ctx, local_pubkey,
                                          local_pubkey_size);
...
static int test_encryption_decryption(void) {
  ...
  polycall_crypto_set_remote_key(test_ctx, test_crypto_ctx, peer_pubkey,
                                 peer_pubkey_size);
  polycall_crypto_set_remote_key(test_ctx, peer_ctx, local_pubkey,
                                 local_pubkey_size);
  ...
  polycall_core_error_t result = polycall_crypto_encrypt(
      test_ctx, test_crypto_ctx, (const uint8_t *)plaintext, plaintext_len,
      (const uint8_t *)aad, aad_len, &ciphertext, &ciphertext_len);

  // Verify encryption result
  ASSERT_EQUAL_INT(POLYCALL_CORE_SUCCESS, result);
  ASSERT_NOT_NULL(ciphertext);
  ASSERT_TRUE(ciphertext_len >
              plaintext_len); // Ciphertext includes tag and nonce

  // Decrypt
  uint8_t *decrypted = NULL;
  size_t decrypted_len = 0;

  result = polycall_crypto_decrypt(test_ctx, peer_ctx, ciphertext,
                                   ciphertext_len, (const uint8_t *)aad,
                                   aad_len, &decrypted, &decrypted_len);
...
static int test_config_update(void) {
  ...
  polycall_core_error_t result =
      polycall_crypto_update_config(test_ctx, test_crypto_ctx, &new_config);
  ...
  result = polycall_crypto_encrypt(test_ctx, test_crypto_ctx,
