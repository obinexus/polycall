...
POLYCALL_TEST_FIXTURE(protocol_token, token_fixture) {
  ...
  POLYCALL_ASSERT_NOT_NULL(token_ctx,
                           "Token context should be created successfully");
...
POLYCALL_TEST_FIXTURE_END(protocol_token, token_fixture) {
  ...
  if (fixture_context) {
    polycall_token_context_t *token_ctx =
        (polycall_token_context_t *)fixture_context;
...
POLYCALL_TEST_CASE_WITH_FIXTURE(protocol_token, create_token, token_fixture) {
  ...
  polycall_token_error_t result =
      polycall_token_create(token_ctx, token_content, &token);
...
POLYCALL_TEST_CASE_WITH_FIXTURE(protocol_token, validate_token, token_fixture) {
  ...
  polycall_token_error_t create_result =
      polycall_token_create(token_ctx, valid_content, &token);
  ...
  polycall_token_error_t register_result =
      polycall_token_register(token_ctx, token);
  ...
  POLYCALL_ASSERT_TRUE(valid_result,
                       "Token validation should succeed with valid content");
  ...
  POLYCALL_ASSERT_FALSE(invalid_result,
                        "Token validation should fail with invalid content");
...
POLYCALL_TEST_CASE_WITH_FIXTURE(protocol_token, token_expiration,
                                token_fixture) {
  // Get the token context from fixture
  polycall_token_context_t *token_ctx =
      (polycall_token_context_t *)fixture_context;
  ...
  polycall_token_error_t exp_result =
      polycall_token_set_expiration(token, 100); // 100ms
  ...
  POLYCALL_ASSERT_FALSE(valid_after,
...
POLYCALL_TEST_SUITE_END(protocol_token) {
  ...
  if (global_context) {
    polycall_core_context_t *core_ctx =
        (polycall_core_context_t *)global_context;
...
POLYCALL_TEST_MAIN(protocol_token_create_token_register,
