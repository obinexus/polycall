...
#define TEST_SIGNING_SECRET                                                    \
  "test_signing_secret_with_sufficient_length_for_zero_trust"
...
static bool test_setup(void) {
  ...
  if (polycall_auth_init(core_ctx, &auth_ctx, &auth_config) !=
...
static bool test_token_issuance(void) {
  ...
  polycall_core_error_t result = polycall_auth_issue_token(
      core_ctx, auth_ctx, TEST_IDENTITY_ID, POLYCALL_TOKEN_TYPE_ACCESS, NULL, 0,
      NULL, &access_token);
  ...
  result = polycall_auth_issue_token(core_ctx, auth_ctx, TEST_IDENTITY_ID,
                                     POLYCALL_TOKEN_TYPE_REFRESH, NULL, 0, NULL,
                                     &refresh_token);
  ...
  char *api_key = NULL;
  result = polycall_auth_generate_api_key(core_ctx, auth_ctx, TEST_IDENTITY_ID,
                                          "test_key", scopes, 2, 30, &api_key);
...
static bool test_token_validation(void) {
  ...
  result = polycall_auth_validate_token_ex(core_ctx, auth_ctx, access_token,
                                           &validation_result);
  ...
  if (validation_result->claims->expires_at <=
      validation_result->claims->issued_at) {
  ...
  result = polycall_auth_validate_token_ex(core_ctx, auth_ctx, invalid_token,
                                           &invalid_result);
  ...
  if (invalid_result->is_valid) {
    fprintf(stderr,
            "Invalid token should be invalid but validation succeeded\n");
    polycall_core_free(core_ctx, access_token);
    polycall_auth_free_token_validation_result(core_ctx, validation_result);
...
static bool test_token_introspection(void) {
  ...
  const char *custom_claims_json =
      "{\"app_id\":\"test_app\",\"device\":\"test_device\"}";
  const char *scopes[] = {"read", "write"};

  char *access_token = NULL;
  polycall_core_error_t result = polycall_auth_issue_token(
      core_ctx, auth_ctx, TEST_IDENTITY_ID, POLYCALL_TOKEN_TYPE_ACCESS, scopes,
      2, custom_claims_json, &access_token);
  ...
  result =
      polycall_auth_introspect_token(core_ctx, auth_ctx, access_token, &claims);
  ...
  if (!claims->custom_claims ||
      strstr(claims->custom_claims, "app_id") == NULL) {
...
static bool test_token_revocation(void) {
  ...
  result = polycall_auth_validate_token(core_ctx, auth_ctx, access_token,
                                        &identity_id);
  ...
  // Revoke the token
  result = polycall_auth_revoke_token(core_ctx, auth_ctx, access_token);
  ...
  if (result == POLYCALL_CORE_SUCCESS && identity_id) {
    fprintf(stderr,
            "Token validation after revocation succeeded, but should fail\n");
    polycall_core_free(core_ctx, access_token);
    polycall_core_free(core_ctx, identity_id);
    return false;
  }
...
static bool test_token_expiration(void) {
  ...
  polycall_core_error_t (*issue_short_token)(
      polycall_core_context_t *, polycall_auth_context_t *, const char *,
      polycall_token_type_t, const char **, size_t, const char *, uint32_t,
      char **) = NULL;
  ...
  if (issue_short_token == NULL) {
    printf(
        "Skipping token expiration test - no support for custom expiration\n");
    return true;
  }
  ...
  if (result == POLYCALL_CORE_SUCCESS && identity_id) {
    fprintf(stderr,
            "Token validation after expiration succeeded, but should fail\n");
    polycall_core_free(core_ctx, access_token);
    polycall_core_free(core_ctx, identity_id);
    return false;
  }
...
static bool test_token_scopes(void) {
  ...
  for (size_t i = 0; i < claims->scope_count; i++) {
    ...
    } else if (claims->scopes[i] &&
               strcmp(claims->scopes[i], "write:data") == 0) {
  ...
  bool (*check_token_scope)(polycall_core_context_t *,
                            polycall_auth_context_t *, const char *,
  ...
  if (check_token_scope != NULL) {
    ...
    result = check_token_scope(core_ctx, auth_ctx, access_token, "read:data",
                               &has_scope);
    ...
    has_scope = false;
    result = check_token_scope(core_ctx, auth_ctx, access_token, "admin:data",
                               &has_scope);
...
static bool test_token_refresh_mechanism(void) {
  ...
  result = polycall_auth_issue_token(core_ctx, auth_ctx, TEST_IDENTITY_ID,
                                     POLYCALL_TOKEN_TYPE_REFRESH, test_scopes,
                                     2, NULL, &refresh_token);
  ...
  result = polycall_auth_introspect_token(core_ctx, auth_ctx, access_token,
                                          &initial_claims);
  ...
  result = polycall_auth_refresh_token(core_ctx, auth_ctx, refresh_token,
                                       &new_access_token);
  ...
  result = polycall_auth_introspect_token(core_ctx, auth_ctx, new_access_token,
                                          &new_claims);
  ...
  if (!new_claims->subject || !initial_claims->subject ||
  ...
  // Try to use the old token - should fail if token revocation on refresh is
  // implemented
  char *identity_id = NULL;
  result = polycall_auth_validate_token(core_ctx, auth_ctx, access_token,
                                        &identity_id);
  ...
  // But this is implementation-specific, so we don't strictly test for failure
  // here
  if (result == POLYCALL_CORE_SUCCESS && identity_id) {
    printf("Note: Old access token still valid after refresh "
...
static bool test_zero_trust_token_properties(void) {
  ...
  const char *device_info =
      "{\"device_id\":\"test_device\",\"platform\":\"test_platform\"}";
  const char *custom_claims =
      "{\"context\":{\"ip\":\"192.168.1.1\",\"user_agent\":\"Test Agent\"}}";

  char *access_token = NULL;
  polycall_core_error_t result = polycall_auth_issue_token(
      core_ctx, auth_ctx, TEST_IDENTITY_ID, POLYCALL_TOKEN_TYPE_ACCESS, NULL, 0,
      custom_claims, &access_token);
