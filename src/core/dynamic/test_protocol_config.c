...
static int test_apply_config(void) {
  ...
  polycall_core_error_t result =
      polycall_protocol_apply_config(test_ctx, test_proto_ctx, &config);

  // Verify result
  ASSERT_EQUAL_INT(POLYCALL_CORE_SUCCESS, result);

  // Verify the mock protocol context was updated
  ASSERT_EQUAL_INT(PROTOCOL_TRANSPORT_WEBSOCKET,
                   test_proto_ctx->transport_type);
...
static int test_validate_config(void) {
  ...
  valid = polycall_protocol_validate_config(test_ctx, &config, error_message,
...
static int test_merge_config(void) {
  ...
  polycall_core_error_t result =
      polycall_protocol_merge_config(test_ctx, &base_config, &override_config);
...
static int test_copy_config(void) {
  ...
  // Copy configuration
  polycall_core_error_t result =
      polycall_protocol_copy_config(test_ctx, &dest_config, &src_config);
  ...
  ASSERT_EQUAL_INT(src_config.core.max_retry_count,
                   dest_config.core.max_retry_count);
...
static int test_config_init(void) {
  ...
  polycall_core_error_t result =
      polycall_protocol_config_init(test_ctx, test_proto_ctx, &config);
