...
/**
 * @file test_security_qa.c
 * @brief UNIT_QA tests for core/security module
...
static void setup_test_fixture(void) {
  ...
  // Arrange: Initialize security context
  result =
      polycall_security_init(g_fixture.core_ctx, &g_fixture.security_ctx, NULL);
  assert(result == POLYCALL_CORE_SUCCESS);
  assert(g_fixture.security_ctx != NULL);
}
 ...
 */
int main(void) {
  printf("Starting UNIT_QA tests for core/security\n");
  printf("==============================================\n");
  ...
  printf("==============================================\n");
  printf("✅ All UNIT_QA tests passed for core/security\n");
