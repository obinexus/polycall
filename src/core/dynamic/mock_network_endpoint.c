 ...
 */
bool mock_network_endpoint_send(NetworkEndpoint *endpoint,
                                const NetworkPacket *packet, uint32_t flags) {
  ...
  /* Add to send queue */
  NetworkPacket *new_packet =
      &endpoint->send_queue.packets[endpoint->send_queue.count++];
 ...
 */
bool mock_network_endpoint_receive(NetworkEndpoint *endpoint,
                                   NetworkPacket *packet, uint32_t flags) {
  if (!endpoint || endpoint->magic != MOCK_ENDPOINT_MAGIC || !packet) {
    return false;
  }
  ...
  memset(&endpoint->recv_queue.packets[endpoint->recv_queue.count - 1], 0,
 ...
 */
bool mock_network_endpoint_add_test_packet(NetworkEndpoint *endpoint,
                                           const void *data, size_t size,
                                           uint32_t flags) {
  if (!endpoint || endpoint->magic != MOCK_ENDPOINT_MAGIC || !data ||
      size == 0) {
    return false;
  }
  ...
  NetworkPacket *new_packet =
      &endpoint->recv_queue.packets[endpoint->recv_queue.count++];
  new_packet->data = data_copy;
