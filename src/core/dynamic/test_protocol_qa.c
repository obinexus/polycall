...
/**
 * @file test_protocol_qa.c
 * @brief UNIT_QA tests for core/protocol module
...
static void setup_test_fixture(void) {
  ...
  // Arrange: Initialize protocol context
  result =
      polycall_protocol_init(g_fixture.core_ctx, &g_fixture.protocol_ctx, NULL);
  assert(result == POLYCALL_CORE_SUCCESS);
  assert(g_fixture.protocol_ctx != NULL);
}
 ...
 */
int main(void) {
  printf("Starting UNIT_QA tests for core/protocol\n");
  printf("==============================================\n");
  ...
  printf("==============================================\n");
  printf("✅ All UNIT_QA tests passed for core/protocol\n");
