...
static int test_memory_pool_creation() {
  ...
  polycall_core_error_t result =
      polycall_memory_create_pool(g_core_ctx, &g_test_pool,
                                  1024 * 1024 // 1MB pool
      );
...
static int test_memory_alloc_free() {
  ...
  void *ptr = polycall_memory_alloc(g_core_ctx, g_test_pool, alloc_size,
  ...
  // Pool statistics should be updated (implementation specific, but we can
  // verify basics)
...
static int test_memory_realloc() {
  ...
  void *ptr = polycall_memory_alloc(g_core_ctx, g_test_pool, initial_size,
                                    POLYCALL_MEMORY_FLAG_NONE);
  ...
  size_t new_size = 256;
  void *new_ptr =
      polycall_memory_realloc(g_core_ctx, g_test_pool, ptr, new_size);
...
static int test_memory_region() {
  ...
  polycall_memory_region_t *region = polycall_memory_create_region(
      g_core_ctx, g_test_pool, 1024,
      POLYCALL_MEMORY_PERM_READ | POLYCALL_MEMORY_PERM_WRITE,
      POLYCALL_MEMORY_FLAG_NONE, "TestOwner");
  ...
  ASSERT_TRUE(polycall_memory_verify_permissions(
      g_core_ctx, region, "TestOwner",
      POLYCALL_MEMORY_PERM_READ | POLYCALL_MEMORY_PERM_WRITE));

  // Verify permissions for unauthorized component
  ASSERT_FALSE(polycall_memory_verify_permissions(
      g_core_ctx, region, "UnauthorizedComponent", POLYCALL_MEMORY_PERM_READ));

  // Share the region
  polycall_core_error_t result =
      polycall_memory_share_region(g_core_ctx, region, "SharedComponent");
  ...
  ASSERT_TRUE(polycall_memory_verify_permissions(
      g_core_ctx, region, "SharedComponent",
      POLYCALL_MEMORY_PERM_READ | POLYCALL_MEMORY_PERM_WRITE));
  ...
  ASSERT_FALSE(polycall_memory_verify_permissions(
      g_core_ctx, region, "SharedComponent", POLYCALL_MEMORY_PERM_READ));
...
static int test_memory_stats() {
  ...
  void *ptr1 = polycall_memory_alloc(g_core_ctx, g_test_pool, 1024,
                                     POLYCALL_MEMORY_FLAG_NONE);
  void *ptr2 = polycall_memory_alloc(g_core_ctx, g_test_pool, 2048,
                                     POLYCALL_MEMORY_FLAG_NONE);
...
static int test_memory_reset() {
  ...
  // Allocate some memory
  void *ptr = polycall_memory_alloc(g_core_ctx, g_test_pool, 1024,
                                    POLYCALL_MEMORY_FLAG_NONE);

  // Reset the pool (note: this invalidates all allocations)
  polycall_core_error_t result =
      polycall_memory_reset_pool(g_core_ctx, g_test_pool);
  ...
  void *new_ptr = polycall_memory_alloc(g_core_ctx, g_test_pool, 1024,
                                        POLYCALL_MEMORY_FLAG_NONE);
...
static int test_custom_allocator() {
  // This test is a bit tricky as we can't directly modify the pool's custom
  // allocator Instead, we'll test the core memory functions which can use
  // custom allocators

  // Set custom memory functions on the core context
  polycall_core_error_t result = polycall_core_set_memory_functions(
      g_core_ctx, test_custom_malloc, test_custom_free, NULL);
