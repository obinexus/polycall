 ...
 * @brief Zero-Trust Security Model implementation for LibPolyCall configuration
validation
...
polycall_core_error_t polycall_security_validation_create(
  ...
  if (!port_usage_registry) {
    ...
    port_usage_registry =
        malloc(port_usage_capacity * sizeof(port_usage_info_t));
    if (!port_usage_registry) {
...
void polycall_security_validation_set_privileged(
    polycall_security_validation_context_t *security_ctx, bool is_privileged) {
  if (security_ctx) {
...
register_port_usage(polycall_component_type_t component_type,
                    const char *component_name, uint16_t port,
                    const char *purpose) {
  ...
  if (port_usage_count >= port_usage_capacity) {
    ...
    port_usage_info_t *new_registry =
        realloc(port_usage_registry, new_capacity * sizeof(port_usage_info_t));
  ...
  strncpy(info->component_name, component_name,
          sizeof(info->component_name) - 1);
...
polycall_core_error_t polycall_security_validate_port(
    polycall_security_validation_context_t *security_ctx,
    polycall_component_type_t component_type, const char *component_name,
    uint16_t port, const char *purpose, bool register_if_valid) {
  ...
  const size_t reserved_port_count =
      sizeof(reserved_ports) / sizeof(reserved_ports[0]);
  ...
  if (is_in_ephemeral_range(port)) {
    ...
    POLYCALL_LOG(
        security_ctx->core_ctx, POLYCALL_LOG_WARNING,
  ...
  for (size_t i = 0; i < port_usage_count; i++) {
    if (port_usage_registry[i].port == port) {
      snprintf(security_ctx->error_buffer, sizeof(security_ctx->error_buffer),
               "Port %u is already in use by component '%s' for %s", port,
               port_usage_registry[i].component_name,
  ...
  if (register_if_valid) {
    polycall_core_error_t result =
        register_port_usage(component_type, component_name, port, purpose);
...
polycall_core_error_t polycall_security_validate_authentication(
  ...
  if (security_ctx->enforcement_flags & ZERO_TRUST_ENFORCE_AUTH) {
    if (!requires_authentication) {
      snprintf(security_ctx->error_buffer, sizeof(security_ctx->error_buffer),
               "Zero-Trust violation: Component '%s' does not require "
               "authentication",
               component_name);
...
polycall_core_error_t polycall_security_validate_encryption(
  ...
  if (security_ctx->enforcement_flags & ZERO_TRUST_ENFORCE_ENCRYPTION) {
    if (!enables_encryption) {
      snprintf(
          security_ctx->error_buffer, sizeof(security_ctx->error_buffer),
...
polycall_core_error_t polycall_security_validate_isolation(
    polycall_security_validation_context_t *security_ctx,
    polycall_component_type_t component_type, const char *component_name,
    int isolation_level, int minimum_required_level) {
  if (!security_ctx || !component_name) {
    return POLYCALL_CORE_ERROR_INVALID_PARAMETERS;
  }
  ...
  if (security_ctx->enforcement_flags & ZERO_TRUST_ENFORCE_ISOLATION) {
    if (isolation_level < minimum_required_level) {
      const char *level_names[] = {"None", "Memory", "Resources", "Security",
                                   "Strict"};

      snprintf(security_ctx->error_buffer, sizeof(security_ctx->error_buffer),
               "Zero-Trust violation: Component '%s' has insufficient "
               "isolation level '%s', requires at least '%s'",
               component_name,
               isolation_level >= 0 && isolation_level < 5
                   ? level_names[isolation_level]
                   : "Unknown",
               minimum_required_level >= 0 && minimum_required_level < 5
                   ? level_names[minimum_required_level]
                   : "Unknown");
      return POLYCALL_CORE_ERROR_SECURITY_VIOLATION;
    }
  }
...
polycall_core_error_t polycall_security_validate_micro_component(
  ...
  result = polycall_security_validate_authentication(
      security_ctx, POLYCALL_COMPONENT_TYPE_MICRO, config->name,
  ...
  result = polycall_security_validate_audit(security_ctx,
                                            POLYCALL_COMPONENT_TYPE_MICRO,
                                            config->name, config->audit_access);
...
polycall_core_error_t polycall_security_validate_edge_component(
  ...
  result = polycall_security_validate_port(
      security_ctx, POLYCALL_COMPONENT_TYPE_EDGE, config->component_name,
      config->discovery_port, "discovery", true);
  ...
  result = polycall_security_validate_port(
      security_ctx, POLYCALL_COMPONENT_TYPE_EDGE, config->component_name,
      config->command_port, "command", true);
...
polycall_core_error_t polycall_security_validate_network_config(
  ...
  config_entry_t *entry =
      find_config_entry(config, "security", "enable_encryption");
  ...
  if (security_ctx->enforcement_flags & ZERO_TRUST_ENFORCE_ENCRYPTION) {
    if (!enable_encryption && !enable_tls) {
      snprintf(security_ctx->error_buffer, sizeof(security_ctx->error_buffer),
               "Zero-Trust violation: Network configuration does not enable "
               "encryption or TLS");
      return POLYCALL_CORE_ERROR_SECURITY_VIOLATION;
    }
  }
...
polycall_core_error_t polycall_security_validate_component(
    polycall_security_validation_context_t *security_ctx,
    polycall_component_type_t component_type, const void *component_config) {
  ...
  case POLYCALL_COMPONENT_TYPE_MICRO:
    return polycall_security_validate_micro_component(
        security_ctx, (const micro_component_config_t *)component_config);
  ...
  case POLYCALL_COMPONENT_TYPE_NETWORK:
    return polycall_security_validate_network_config(
        security_ctx, (const polycall_network_config_t *)component_config);

  case POLYCALL_COMPONENT_TYPE_PROTOCOL:
    return polycall_security_validate_protocol_config(
        security_ctx, (const protocol_config_t *)component_config);
