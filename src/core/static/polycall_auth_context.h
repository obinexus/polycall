 ...
 *
 * This header defines the authentication context that manages identity,
 * credentials, and authorization for LibPolyCall components.
 */
 ...
 * @param identity_id Pointer to receive the identity ID associated with the
 * token
 * @return Error code
 */
polycall_core_error_t
...
static identity_registry_t *init_identity_registry(polycall_core_context_t *ctx,
                                                   size_t capacity);
static void cleanup_identity_registry(polycall_core_context_t *ctx,
                                      identity_registry_t *registry);
static credential_store_t *init_credential_store(polycall_core_context_t *ctx,
                                                 bool enable_hashing);
static void cleanup_credential_store(polycall_core_context_t *ctx,
                                     credential_store_t *store);
static token_service_t *init_token_service(polycall_core_context_t *ctx,
                                           const char *signing_secret,
                                           uint32_t access_validity,
                                           uint32_t refresh_validity);
static void cleanup_token_service(polycall_core_context_t *ctx,
                                  token_service_t *service);
static auth_policy_manager_t *init_policy_manager(polycall_core_context_t *ctx);
static void cleanup_policy_manager(polycall_core_context_t *ctx,
                                   auth_policy_manager_t *manager);
static auth_audit_t *init_auth_audit(polycall_core_context_t *ctx,
                                     bool enable_logging);
static void cleanup_auth_audit(polycall_core_context_t *ctx,
                               auth_audit_t *audit);
static auth_integrator_t *init_auth_integrator(polycall_core_context_t *ctx);
static void cleanup_auth_integrator(polycall_core_context_t *ctx,
                                    auth_integrator_t *integrator);
static char *hash_password(credential_store_t *store, const char *password);
static bool verify_password(credential_store_t *store, const char *password,
                            const char *hash);
static char *generate_token(token_service_t *service, const char *identity_id,
                            polycall_token_type_t type, uint64_t expiry_time);
static token_validation_result_t *
validate_token_internal(token_service_t *service, const char *token);
