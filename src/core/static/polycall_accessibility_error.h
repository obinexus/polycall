 ...
 * @param core_ctx Core context
 */
void polycall_accessibility_error_cleanup(polycall_core_context_t *core_ctx);
...
bool polycall_accessibility_error_get_last(
    polycall_core_context_t *core_ctx, polycall_error_record_t *error_record);
...
void polycall_accessibility_error_set(
    polycall_core_context_t *core_ctx,
    polycall_hierarchical_error_context_t *hier_error_ctx,
    polycall_accessibility_error_t code, polycall_error_severity_t severity,
 ...
 */
const char *
polycall_accessibility_error_to_string(polycall_accessibility_error_t error);

/**
 * @brief Macro for setting accessibility error with file and line info
 */
#define POLYCALL_ACCESSIBILITY_ERROR_SET(ctx, hier_ctx, code, severity,        \
                                         message, ...)                         \
  polycall_accessibility_error_set(ctx, hier_ctx, code, severity, __FILE__,    \
                                   __LINE__, message, ##__VA_ARGS__)

/**
 * @brief Macro for checking accessibility error condition
 */
#define POLYCALL_ACCESSIBILITY_CHECK_ERROR(ctx, hier_ctx, expr, code,          \
                                           severity, message, ...)             \
  do {                                                                         \
    if (!(expr)) {                                                             \
      POLYCALL_ACCESSIBILITY_ERROR_SET(ctx, hier_ctx, code, severity, message, \
                                       ##__VA_ARGS__);                         \
      return code;                                                             \
    }                                                                          \
  } while (0)
