...
struct security_policy {
  ...
  audit_callback_entry_t
      audit_callbacks[MAX_AUDIT_CALLBACKS]; // Audit callbacks
...
struct command_security_attributes {
  ...
  const char
      *restricted_to_component; // Restrict to specific component (or NULL)
...
typedef void (*security_audit_callback_t)(
    ...
    const polycall_security_audit_entry_t *entry, void *user_data);
