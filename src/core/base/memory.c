...
static memory_block_header_t *create_block(void *addr, size_t size,
                                           bool is_free) {
...
// List operations
static void add_to_list(memory_block_header_t **list,
...
static void remove_from_list(memory_block_header_t **list,
                             memory_block_header_t *block) {
  if (block->prev) {
...
static void init_memory_pool(polycall_memory_pool_t *pool, void *base,
                             size_t size) {
  pool->base = base;
  pool->size = size;
  pool->used = 0;
...
polycall_core_error_t polycall_memory_create_pool(polycall_core_context_t *ctx,
  ...
  polycall_memory_pool_t *new_pool =
      polycall_core_malloc(ctx, sizeof(polycall_memory_pool_t));
  if (!new_pool) {
...
void polycall_memory_destroy_pool(polycall_core_context_t *ctx,
                                  polycall_memory_pool_t *pool) {
  if (!ctx || !pool)
    return;
  ...
  if (pool->used > 0) {
    POLYCALL_ERROR_SET(
        ctx, POLYCALL_ERROR_SOURCE_MEMORY, POLYCALL_CORE_ERROR_INVALID_STATE,
        POLYCALL_ERROR_SEVERITY_WARNING,
        "Memory pool destroyed with %zu bytes still allocated", pool->used);
  }
...
static memory_block_header_t *find_free_block(polycall_memory_pool_t *pool,
                                              size_t size) {
...
static void split_block(polycall_memory_pool_t *pool,
                        memory_block_header_t *block, size_t size) {
  ...
  if (remaining_size > MEMORY_HEADER_SIZE + MEMORY_MIN_BLOCK_SIZE) {
    ...
    memory_block_header_t *new_block =
        create_block(new_block_addr, remaining_size + MEMORY_HEADER_SIZE, true);
...
void *polycall_memory_alloc(polycall_core_context_t *ctx,
...
void polycall_memory_free(polycall_core_context_t *ctx,
                          polycall_memory_pool_t *pool, void *ptr) {
  if (!ctx || !pool || !ptr) {
    return;
  }
...
void *polycall_memory_realloc(polycall_core_context_t *ctx,
                              polycall_memory_pool_t *pool, void *ptr,
                              size_t size) {
  if (!ctx || !pool) {
    return NULL;
...
polycall_memory_region_t *polycall_memory_create_region(
    polycall_core_context_t *ctx, polycall_memory_pool_t *pool, size_t size,
    polycall_memory_permissions_t perms, polycall_memory_flags_t flags,
    const char *owner) {
  ...
  void *region_mem =
      polycall_memory_alloc(ctx, pool, sizeof(polycall_memory_region_t),
...
void polycall_memory_destroy_region(polycall_core_context_t *ctx,
                                    polycall_memory_pool_t *pool,
...
polycall_memory_share_region(polycall_core_context_t *ctx,
                             polycall_memory_region_t *region,
...
polycall_core_error_t
polycall_memory_unshare_region(polycall_core_context_t *ctx,
                               polycall_memory_region_t *region) {
  if (!ctx || !region) {
    return POLYCALL_CORE_ERROR_INVALID_PARAMETERS;
  }
...
polycall_core_error_t
polycall_memory_get_stats(polycall_core_context_t *ctx,
                          polycall_memory_pool_t *pool,
...
bool polycall_memory_verify_permissions(
    polycall_core_context_t *ctx, const polycall_memory_region_t *region,
    const char *component, polycall_memory_permissions_t required_perms) {
  if (!ctx || !region || !component) {
    return false;
  }
...
polycall_core_error_t polycall_memory_reset_pool(polycall_core_context_t *ctx,
                                                 polycall_memory_pool_t *pool) {
  if (!ctx || !pool) {
    return POLYCALL_CORE_ERROR_INVALID_PARAMETERS;
  }
