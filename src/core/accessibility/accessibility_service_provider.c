...
polycall_accessibility_register_services(polycall_cli_container_t *container) {
  if (!container) {
  ...
  polycall_core_error_t result =
      polycall_accessibility_init(core_ctx, &access_config, &access_ctx);
...
void polycall_accessibility_cleanup_services(
  ...
  polycall_accessibility_context_t *access_ctx =
      container->resolve_service(container, "accessibility_context");
  if (!access_ctx) {
    return;
  }
