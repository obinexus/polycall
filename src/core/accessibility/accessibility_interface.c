...
static bool format_table_row(char *buffer, size_t buffer_size,
                             const char **cells, size_t cell_count,
                             const int *widths, bool is_header,
...
polycall_accessibility_init(polycall_core_context_t *core_ctx,
  ...
  // Allocate context
  polycall_accessibility_context_t *ctx =
      (polycall_accessibility_context_t *)malloc(
          sizeof(polycall_accessibility_context_t));
  if (!ctx) {
...
bool polycall_accessibility_format_text(
    polycall_core_context_t *core_ctx,
    polycall_accessibility_context_t *access_ctx, const char *text,
    polycall_text_type_t type, polycall_text_style_t style, char *buffer,
    size_t buffer_size) {
  ...
  if (access_ctx->screen_reader_active) {
    ...
    if (strlen(sr_prefix) > 0) {
      ...
      return polycall_format_colored_text(temp, type, style, buffer,
                                          buffer_size);
    }
  }
...
bool polycall_accessibility_format_command_help(
    polycall_core_context_t *core_ctx,
    polycall_accessibility_context_t *access_ctx, const char *command,
    const char *description, const char *usage, char *buffer,
    size_t buffer_size) {
  ...
  if (description) {
    polycall_accessibility_format_text(
        core_ctx, access_ctx, description, POLYCALL_TEXT_NORMAL,
        POLYCALL_STYLE_NORMAL, description_buf, sizeof(description_buf));
  }
  ...
  if (usage) {
    ...
    polycall_accessibility_format_text(
        core_ctx, access_ctx, "Usage:", POLYCALL_TEXT_HEADING,
        POLYCALL_STYLE_BOLD, usage_label, sizeof(usage_label));

    char usage_text[896];
    polycall_accessibility_format_text(
        core_ctx, access_ctx, usage, POLYCALL_TEXT_NORMAL,
        POLYCALL_STYLE_NORMAL, usage_text, sizeof(usage_text));
  ...
  if (access_ctx->screen_reader_active) {
    snprintf(buffer, buffer_size, "Command: %s\nDescription: %s%s\n", command,
...
bool polycall_accessibility_format_error(
    polycall_core_context_t *core_ctx,
    polycall_accessibility_context_t *access_ctx, int error_code,
    const char *error_message, char *buffer, size_t buffer_size) {
  ...
  if (error_message) {
    polycall_accessibility_format_text(
        core_ctx, access_ctx, error_message, POLYCALL_TEXT_ERROR,
        POLYCALL_STYLE_NORMAL, message_buf, sizeof(message_buf));
  } else {
    polycall_accessibility_format_text(
        core_ctx, access_ctx, "Unknown error", POLYCALL_TEXT_ERROR,
        POLYCALL_STYLE_NORMAL, message_buf, sizeof(message_buf));
  }
  ...
  if (access_ctx->screen_reader_active) {
    snprintf(buffer, buffer_size, "Error: %s. %s",
             error_message ? error_message : "Unknown error", code_buf);
  } else {
    // Visual formatting
    snprintf(buffer, buffer_size, "%s (%s)", message_buf, code_formatted);
  }
...
bool polycall_accessibility_format_success(
    polycall_core_context_t *core_ctx,
    polycall_accessibility_context_t *access_ctx, const char *message,
  ...
  return polycall_accessibility_format_text(
      core_ctx, access_ctx, message, POLYCALL_TEXT_SUCCESS,
      POLYCALL_STYLE_NORMAL, buffer, buffer_size);
}
...
bool polycall_accessibility_format_progress(
    polycall_core_context_t *core_ctx,
    polycall_accessibility_context_t *access_ctx, float progress,
...
bool polycall_accessibility_format_table(
    polycall_core_context_t *core_ctx,
    polycall_accessibility_context_t *access_ctx, const char **headers,
    size_t header_count, const char ***data, size_t row_count, size_t col_count,
    char *buffer, size_t buffer_size) {
  if (!core_ctx || !access_ctx || !headers || !data || !buffer ||
      buffer_size == 0 || header_count == 0 || row_count == 0 ||
      col_count == 0) {
    return false;
  }
  ...
  if (access_ctx->screen_reader_active) {
    ...
    offset += snprintf(buffer + offset, buffer_size - offset,
                       "Table with %zu rows and %zu columns.\n", row_count,
                       col_count);
  ...
  // Format header row
  if (!format_table_row(buffer + offset, buffer_size - offset, headers,
                        col_count, widths, true, access_ctx)) {
    return false;
  }
  ...
  // Format data rows
  for (size_t row = 0; row < row_count; row++) {
    if (!format_table_row(buffer + offset, buffer_size - offset, data[row],
                          col_count, widths, false, access_ctx)) {
      return false;
    }
...
bool polycall_accessibility_get_screen_reader_text(
    polycall_core_context_t *core_ctx,
    polycall_accessibility_context_t *access_ctx, const char *element_type,
    const char *element_name, const char *element_state, char *buffer,
    size_t buffer_size) {
  if (!core_ctx || !access_ctx || !element_type || !buffer ||
      buffer_size == 0) {
    return false;
  }
  ...
  if (strcmp(element_type, "button") == 0) {
    snprintf(buffer, buffer_size, "Button: %s%s%s",
             element_name ? element_name : "Unnamed", element_state ? ", " : "",
    ...
    // Generic element
    snprintf(buffer, buffer_size, "%s: %s%s%s", element_type,
             element_name ? element_name : "Unnamed", element_state ? ", " : "",
             element_state ? element_state : "");
  }
...
bool polycall_accessibility_format_prompt(
    polycall_core_context_t *core_ctx,
    polycall_accessibility_context_t *access_ctx, const char *prompt,
    char *buffer, size_t buffer_size) {
  ...
  if (access_ctx->screen_reader_active) {
    ...
    return polycall_accessibility_format_text(
        core_ctx, access_ctx, prompt, POLYCALL_TEXT_COMMAND,
        POLYCALL_STYLE_BOLD, buffer, buffer_size);
  }
...
polycall_core_error_t polycall_accessibility_apply_environment_settings(
  ...
  detect_terminal_dimensions(&access_ctx->terminal_width,
                             &access_ctx->terminal_height);
...
static bool detect_screen_reader(void) {
  ...
  if (getenv("NVDA_LAUNCHED") != NULL || getenv("JAWS_LAUNCHED") != NULL ||
...
static bool format_table_row(char *buffer, size_t buffer_size,
  ...
  for (size_t col = 0; col < cell_count; col++) {
    ...
    if (is_header) {
      polycall_accessibility_format_text(
          NULL, ctx, cell_text, POLYCALL_TEXT_HEADING, POLYCALL_STYLE_BOLD,
          cell_buffer, sizeof(cell_buffer));
    } else {
      polycall_accessibility_format_text(
          NULL, ctx, cell_text, POLYCALL_TEXT_NORMAL, POLYCALL_STYLE_NORMAL,
          cell_buffer, sizeof(cell_buffer));
    }
