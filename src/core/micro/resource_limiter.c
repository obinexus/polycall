 ...
 * @brief Resource limitation implementation for LibPolyCall micro command
system
...
static void check_threshold(polycall_core_context_t *ctx,
  ...
  for (size_t i = 0; i < limiter->threshold_callback_count; i++) {
    ...
    if (info->resource_type == resource_type &&
        usage_percent >= info->threshold && info->callback) {

      // Call the threshold callback
      info->callback(ctx, micro_ctx, component, resource_type, current_usage,
                     quota, info->user_data);
    }
  }
}
...
resource_limiter_init(polycall_core_context_t *ctx,
                      resource_limiter_t **limiter,
  ...
  resource_limiter_t *new_limiter =
      polycall_core_malloc(ctx, sizeof(resource_limiter_t));
  if (!new_limiter) {
    POLYCALL_ERROR_SET(
        ctx, POLYCALL_ERROR_SOURCE_MICRO, POLYCALL_CORE_ERROR_OUT_OF_MEMORY,
        POLYCALL_ERROR_SEVERITY_ERROR, "Failed to allocate resource limiter");
