 ...
 * command subsystem, providing registration, execution, and lifecycle
management
...
struct resource_limiter {
  ...
  threshold_callback_info_t
      threshold_callbacks[MAX_THRESHOLD_CALLBACKS]; // Threshold callbacks
...
typedef struct async_command_data {
  ...
  void (*callback)(                           // Completion callback
      ...
      polycall_micro_component_t *component, const char *command_name,
      void *result, polycall_core_error_t error, void *user_data);
  void *user_data; // User data for callback
...
static polycall_core_error_t find_command(polycall_core_context_t *ctx,
...
static void notify_component_state_change(polycall_core_context_t *ctx,
                                          polycall_micro_component_t *component,
...
queue_async_command(polycall_micro_context_t *micro_ctx,
                    async_command_data_t *cmd_data);

static void check_threshold(polycall_core_context_t *ctx,
                            polycall_micro_context_t *micro_ctx,
                            polycall_micro_component_t *component,
                            resource_limiter_t *limiter,
                            polycall_resource_type_t resource_type,
                            size_t current_usage, size_t quota);
 ...
 */
void resource_limiter_cleanup(polycall_core_context_t *ctx,
                              resource_limiter_t *limiter) {
  if (!ctx || !limiter) {
    return;
  }
...
polycall_core_error_t resource_limiter_set_quota(
    polycall_core_context_t *ctx, resource_limiter_t *limiter,
    polycall_resource_type_t resource_type, size_t quota) {
  if (!ctx || !limiter) {
...
polycall_core_error_t resource_limiter_allocate(
    polycall_core_context_t *ctx, resource_limiter_t *limiter,
    polycall_resource_type_t resource_type, size_t amount) {
  if (!ctx || !limiter) {
    return POLYCALL_CORE_ERROR_INVALID_PARAMETERS;
  }
  ...
  switch (resource_type) {
  case POLYCALL_RESOURCE_MEMORY:
    if (limiter->enforce_limits && limiter->memory_quota > 0 &&
      ...
      if (limiter->memory_quota > 0) {
        check_threshold(ctx, NULL, NULL, limiter, POLYCALL_RESOURCE_MEMORY,
                        limiter->memory_usage, limiter->memory_quota);
      }
    }
    break;

  case POLYCALL_RESOURCE_CPU:
    if (limiter->enforce_limits && limiter->cpu_quota > 0 &&
      ...
      if (limiter->cpu_quota > 0) {
        check_threshold(ctx, NULL, NULL, limiter, POLYCALL_RESOURCE_CPU,
                        limiter->cpu_usage, limiter->cpu_quota);
      }
    }
    break;

  case POLYCALL_RESOURCE_IO:
    if (limiter->enforce_limits && limiter->io_quota > 0 &&
      ...
      if (limiter->io_quota > 0) {
        check_threshold(ctx, NULL, NULL, limiter, POLYCALL_RESOURCE_IO,
                        limiter->io_usage, limiter->io_quota);
      }
    }
    break;
  ...
  if (result != POLYCALL_CORE_SUCCESS) {
    POLYCALL_ERROR_SET(ctx, POLYCALL_ERROR_SOURCE_MICRO, result,
                       POLYCALL_ERROR_SEVERITY_ERROR,
...
resource_limiter_get_usage(polycall_core_context_t *ctx,
                           resource_limiter_t *limiter,
 ...
 */
polycall_core_error_t
resource_limiter_reset_usage(polycall_core_context_t *ctx,
                             resource_limiter_t *limiter) {
  if (!ctx || !limiter) {
    return POLYCALL_CORE_ERROR_INVALID_PARAMETERS;
  }
...
polycall_core_error_t resource_limiter_register_threshold(
    polycall_core_context_t *ctx, resource_limiter_t *limiter,
    polycall_resource_type_t resource_type, uint8_t threshold,
    resource_threshold_callback_t callback, void *user_data) {
  ...
  // Add callback
  threshold_callback_info_t *info =
      &limiter->threshold_callbacks[limiter->threshold_callback_count];
 ...
 */
void *resource_limiter_malloc(polycall_core_context_t *ctx,
                              resource_limiter_t *limiter, size_t size) {
  ...
  if (!block) {
    ...
    resource_limiter_release(ctx, limiter, POLYCALL_RESOURCE_MEMORY,
                             total_size);
    POLYCALL_ERROR_SET(
        ctx, POLYCALL_ERROR_SOURCE_MICRO, POLYCALL_CORE_ERROR_OUT_OF_MEMORY,
        POLYCALL_ERROR_SEVERITY_ERROR, "Failed to allocate memory");
    return NULL;
  }
 ...
 */
void resource_limiter_free(polycall_core_context_t *ctx,
                           resource_limiter_t *limiter, void *ptr) {
  if (!ctx || !limiter || !ptr) {
    return;
  }
  ...
  memory_block_header_t *block =
      (memory_block_header_t *)((char *)ptr - sizeof(memory_block_header_t));
 ...
 */
void polycall_micro_cleanup(polycall_core_context_t *core_ctx,
                            polycall_micro_context_t *micro_ctx) {
...
polycall_core_error_t polycall_micro_create_component(
    polycall_core_context_t *core_ctx, polycall_micro_context_t *micro_ctx,
    polycall_micro_component_t **component, const char *name,
  ...
  if (result != POLYCALL_CORE_SUCCESS) {
    POLYCALL_ERROR_SET(core_ctx, POLYCALL_ERROR_SOURCE_MICRO, result,
                       POLYCALL_ERROR_SEVERITY_ERROR,
  ...
  if (micro_ctx->config.enable_resource_limits) {
    resource_limiter_config_t limiter_config =
        resource_limiter_create_default_config();
    ...
    if (result != POLYCALL_CORE_SUCCESS) {
      ...
      *component = NULL;
      ...
      POLYCALL_ERROR_SET(
          core_ctx, POLYCALL_ERROR_SOURCE_MICRO, result,
          POLYCALL_ERROR_SEVERITY_ERROR,
  ...
  if (micro_ctx->config.enable_security && micro_ctx->security_policy) {
    ...
    result = polycall_micro_component_init_security(core_ctx, *component,
                                                    &security_ctx);
  ...
  result = component_registry_register(core_ctx, micro_ctx->component_registry,
                                       *component);
 ...
 */
polycall_core_error_t
polycall_micro_destroy_component(polycall_core_context_t *core_ctx,
                                 polycall_micro_context_t *micro_ctx,
                                 polycall_micro_component_t *component) {
...
polycall_core_error_t polycall_micro_find_component(
    polycall_core_context_t *core_ctx, polycall_micro_context_t *micro_ctx,
    const char *name, polycall_micro_component_t **component) {
  if (!core_ctx || !micro_ctx || !name || !component) {
    return POLYCALL_CORE_ERROR_INVALID_PARAMETERS;
  }

  return component_registry_find(core_ctx, micro_ctx->component_registry, name,
                                 component);
}
...
polycall_core_error_t polycall_micro_register_command(
    polycall_core_context_t *core_ctx, polycall_micro_context_t *micro_ctx,
    polycall_micro_component_t *component, polycall_micro_command_t **command,
    const char *name, polycall_command_handler_t handler,
    polycall_command_flags_t flags, void *user_data) {
  ...
  polycall_micro_command_t *new_command =
      polycall_core_malloc(core_ctx, sizeof(polycall_micro_command_t));
  if (!new_command) {
    POLYCALL_ERROR_SET(core_ctx, POLYCALL_ERROR_SOURCE_MICRO,
  ...
  if (micro_ctx->config.enable_security) {
    ...
    if (flags & POLYCALL_COMMAND_FLAG_READONLY) {
      ...
      required_permissions |=
          (POLYCALL_PERMISSION_READ | POLYCALL_PERMISSION_WRITE);
    }
  ...
  if (component->command_count >= component->command_capacity) {
    ...
    polycall_micro_command_t **new_commands = polycall_core_realloc(
        core_ctx, component->commands,
        new_capacity * sizeof(polycall_micro_command_t *));
...
polycall_core_error_t polycall_micro_execute_command(
  ...
  polycall_core_error_t find_result =
      find_command(core_ctx, component, command_name, &command);

  if (find_result != POLYCALL_CORE_SUCCESS) {
    POLYCALL_ERROR_SET(core_ctx, POLYCALL_ERROR_SOURCE_MICRO, find_result,
                       POLYCALL_ERROR_SEVERITY_ERROR,
                       "Command '%s' not found in component '%s'", command_name,
                       component->name);
    return find_result;
  }
  ...
  if (state_result != POLYCALL_CORE_SUCCESS) {
    POLYCALL_ERROR_SET(core_ctx, POLYCALL_ERROR_SOURCE_MICRO, state_result,
                       POLYCALL_ERROR_SEVERITY_ERROR,
  ...
  if (state != POLYCALL_COMPONENT_STATE_RUNNING) {
    POLYCALL_ERROR_SET(
        core_ctx, POLYCALL_ERROR_SOURCE_MICRO,
        POLYCALL_CORE_ERROR_INVALID_STATE, POLYCALL_ERROR_SEVERITY_ERROR,
  ...
  if (micro_ctx->config.enable_security) {
    polycall_core_error_t verify_result =
        verify_command_execution(core_ctx, micro_ctx, component, command);

    if (verify_result != POLYCALL_CORE_SUCCESS) {
      POLYCALL_ERROR_SET(core_ctx, POLYCALL_ERROR_SOURCE_MICRO, verify_result,
                         POLYCALL_ERROR_SEVERITY_ERROR,
  ...
  // Execute command
  return command->handler(core_ctx, micro_ctx, component, params, result,
                          command->user_data);
}
...
polycall_core_error_t polycall_micro_execute_command_async(
    ...
    polycall_micro_component_t *component, const char *command_name,
    void *params,
    void (*callback)(polycall_core_context_t *ctx,
                     polycall_micro_context_t *micro_ctx,
                     polycall_micro_component_t *component,
                     const char *command_name, void *result,
                     polycall_core_error_t error, void *user_data),
    void *user_data) {
  ...
  cmd_data.result = polycall_core_malloc(
      core_ctx, 1024); // Allocate result buffer (size depends on command)
...
polycall_core_error_t polycall_micro_set_resource_limits(
    polycall_core_context_t *core_ctx, polycall_micro_context_t *micro_ctx,
    polycall_micro_component_t *component, size_t memory_quota,
    uint32_t cpu_quota, uint32_t io_quota) {
  if (!core_ctx || !micro_ctx || !component) {
    return POLYCALL_CORE_ERROR_INVALID_PARAMETERS;
  }
  ...
  polycall_core_error_t polycall_micro_set_component_data(
      polycall_core_context_t * core_ctx, polycall_micro_context_t * micro_ctx,
      polycall_micro_component_t * component, void *user_data) {
    if (!core_ctx || !micro_ctx || !component) {
      return POLYCALL_CORE_ERROR_INVALID_PARAMETERS;
    }
  ...
  polycall_core_error_t command_handler_init(
    ...
    if (micro_ctx->config.enable_async_commands) {
      ...
      if (pthread_create(&micro_ctx->async_thread, NULL, async_command_thread,
                         micro_ctx) != 0) {
       ...
       */
      void command_handler_cleanup(polycall_core_context_t * ctx,
                                   polycall_micro_context_t * micro_ctx) {
        if (!ctx || !micro_ctx) {
          return;
        }
      ...
      // Set CPU quota
      result = resource_limiter_set_quota(core_ctx, component->resource_limiter,
                                          POLYCALL_RESOURCE_CPU, cpu_quota);
  ...
  polycall_core_error_t polycall_micro_start_component(
    ...
    // Notify state change
    pthread_mutex_unlock(&component->lock);
    notify_component_state_change(core_ctx, component, old_state,
                                  POLYCALL_COMPONENT_STATE_STARTING);
    ...
    // Notify state change
    notify_component_state_change(core_ctx, component, old_state,
                                  POLYCALL_COMPONENT_STATE_RUNNING);
  ...
  polycall_core_error_t polycall_micro_stop_component(
    ...
    pthread_mutex_unlock(&component->lock);
    notify_component_state_change(core_ctx, component, old_state,
                                  POLYCALL_COMPONENT_STATE_STOPPING);
    ...
    // Notify state change
    notify_component_state_change(core_ctx, component, old_state,
                                  POLYCALL_COMPONENT_STATE_STOPPED);
