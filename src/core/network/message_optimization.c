 ...
 */
static bool validate_optimization_context(
...
static polycall_core_error_t compress_message_data(
    ...
    polycall_message_optimization_context_t *opt_ctx, const void *data,
    size_t data_size, void *compressed_buffer, size_t buffer_size,
    size_t *compressed_size, polycall_msg_compression_level_t level) {
  if (!data || data_size == 0 || !compressed_buffer || buffer_size == 0 ||
      !compressed_size) {
...
decompress_message_data(polycall_core_context_t *core_ctx,
                        polycall_message_optimization_context_t *opt_ctx,
                        const void *compressed_data, size_t compressed_size,
                        void *decompressed_buffer, size_t buffer_size,
...
polycall_core_error_t polycall_message_optimization_init(
  ...
  // Allocate context
  polycall_message_optimization_context_t *new_ctx = polycall_core_malloc(
      core_ctx, sizeof(polycall_message_optimization_context_t));
  ...
  memcpy(&new_ctx->config, config,
         sizeof(polycall_message_optimization_config_t));
    ...
    for (uint32_t i = 0; i < opt_ctx->priority_queues[q].count; i++) {
      if (opt_ctx->priority_queues[q].entries[i].data) {
        polycall_core_free(core_ctx,
                           opt_ctx->priority_queues[q].entries[i].data);
      }
    }
  }
...
polycall_core_error_t polycall_message_optimize(
    polycall_core_context_t *core_ctx,
    polycall_message_optimization_context_t *opt_ctx, const void *message,
    size_t message_size, void *optimized_buffer, size_t buffer_size,
    size_t *optimized_size, polycall_msg_priority_t priority) {
  if (!core_ctx || !validate_optimization_context(opt_ctx) || !message ||
      message_size == 0 || !optimized_buffer || buffer_size == 0 ||
      !optimized_size) {
    return POLYCALL_CORE_ERROR_INVALID_PARAMETERS;
  }
  ...
  if (opt_ctx->config.compression_level != POLYCALL_MSG_COMPRESSION_NONE &&
    ...
    polycall_core_error_t result = compress_message_data(
        core_ctx, opt_ctx, message, message_size, optimized_buffer, buffer_size,
        optimized_size, opt_ctx->config.compression_level);
  ...
  float total_ratio = (float)opt_ctx->stats.total_original_bytes /
                      opt_ctx->stats.total_optimized_bytes;

  // Update average compression ratio (using a weighted average)
  opt_ctx->stats.average_compression_ratio =
      (opt_ctx->stats.average_compression_ratio *
           (opt_ctx->stats.total_messages - 1) +
       current_ratio) /
      opt_ctx->stats.total_messages;
...
polycall_message_restore(polycall_core_context_t *core_ctx,
                         polycall_message_optimization_context_t *opt_ctx,
                         const void *optimized_message, size_t optimized_size,
                         void *original_buffer, size_t buffer_size,
                         size_t *original_size) {
  if (!core_ctx || !validate_optimization_context(opt_ctx) ||
      !optimized_message || optimized_size == 0 || !original_buffer ||
      buffer_size == 0 || !original_size) {
    return POLYCALL_CORE_ERROR_INVALID_PARAMETERS;
  }
  ...
  if (opt_ctx->config.compression_level != POLYCALL_MSG_COMPRESSION_NONE) {
    ...
    polycall_core_error_t result = decompress_message_data(
        core_ctx, opt_ctx, optimized_message, optimized_size, original_buffer,
 ...
 */
polycall_core_error_t
polycall_message_batch_add(polycall_core_context_t *core_ctx,
                           polycall_message_optimization_context_t *opt_ctx,
  ...
  if (opt_ctx->config.enable_prioritization) {
    ...
    polycall_batch_entry_t *entry =
        &opt_ctx->priority_queues[queue_idx]
             .entries[opt_ctx->priority_queues[queue_idx].count++];
    ...
    // Add to batch queue
    polycall_batch_entry_t *entry =
        &opt_ctx->batch_queue[opt_ctx->batch_queue_count++];
 ...
 */
polycall_core_error_t
polycall_message_batch_process(polycall_core_context_t *core_ctx,
                               polycall_message_optimization_context_t *opt_ctx,
                               bool force_flush, void *batch_buffer,
                               size_t buffer_size, size_t *batch_size) {
  if (!core_ctx || !validate_optimization_context(opt_ctx) || !batch_buffer ||
      buffer_size == 0 || !batch_size) {
    return POLYCALL_CORE_ERROR_INVALID_PARAMETERS;
  }
  ...
  if (!should_process) {
    ...
    if (opt_ctx->batch_strategy == POLYCALL_BATCH_STRATEGY_SIZE) {
      should_process =
          (opt_ctx->batch_queue_count >= opt_ctx->config.batch_size);
    }
...
polycall_core_error_t polycall_message_unbatch(
    polycall_core_context_t *core_ctx,
    polycall_message_optimization_context_t *opt_ctx, const void *batch_data,
    size_t batch_size,
    void (*message_callback)(const void *message, size_t message_size,
                             polycall_msg_priority_t priority,
                             polycall_protocol_msg_type_t message_type,
                             void *user_data),
    void *user_data) {
  if (!core_ctx || !validate_optimization_context(opt_ctx) || !batch_data ||
      batch_size < sizeof(polycall_batch_header_t) || !message_callback) {
    return POLYCALL_CORE_ERROR_INVALID_PARAMETERS;
  }

  // Parse batch header
  const polycall_batch_header_t *header =
      (const polycall_batch_header_t *)batch_data;
  ...
  const uint8_t *ptr =
      (const uint8_t *)batch_data + sizeof(polycall_batch_header_t);
  ...
  for (uint32_t i = 0; i < header->message_count; i++) {
    ...
    polycall_protocol_msg_type_t message_type =
        (polycall_protocol_msg_type_t)type_value;
    ptr += 4;
...
polycall_core_error_t polycall_message_set_batch_strategy(
    polycall_core_context_t *core_ctx,
    polycall_message_optimization_context_t *opt_ctx,
    polycall_msg_batch_strategy_t strategy, void *params) {
  if (!core_ctx || !validate_optimization_context(opt_ctx)) {
    return POLYCALL_CORE_ERROR_INVALID_PARAMETERS;
  }
 ...
 */
polycall_core_error_t
polycall_message_get_stats(polycall_core_context_t *core_ctx,
                           polycall_message_optimization_context_t *opt_ctx,
 ...
 */
polycall_core_error_t
polycall_message_reset_stats(polycall_core_context_t *core_ctx,
                             polycall_message_optimization_context_t *opt_ctx) {
  if (!core_ctx || !validate_optimization_context(opt_ctx)) {
    return POLYCALL_CORE_ERROR_INVALID_PARAMETERS;
  }
