...
polycall_core_error_t polycall_validate_component_configuration(
    polycall_core_context_t *ctx, polycall_schema_context_t *schema_ctx,
  ...
  if (!ctx || !schema_ctx || !security_ctx || !component_config ||
  ...
  polycall_core_error_t result = polycall_schema_validate_component(
      ctx, schema_ctx, component_type, component_config, error_message,
      error_message_size);
  ...
  result = polycall_security_validate_component(security_ctx, component_type,
                                                component_config);

  if (result != POLYCALL_CORE_SUCCESS) {
    // Copy error message from security validation
    const char *security_error =
        polycall_security_validation_get_error(security_ctx);
    if (security_error) {
...
polycall_core_error_t polycall_create_validation_contexts(
    polycall_core_context_t *ctx, polycall_schema_context_t **schema_ctx,
    polycall_security_validation_context_t **security_ctx,
    bool strict_validation, uint32_t security_flags) {
  ...
  polycall_core_error_t result =
      polycall_schema_context_create(ctx, schema_ctx, strict_validation);
  ...
  result =
      polycall_security_validation_create(ctx, security_ctx, security_flags);
...
polycall_core_error_t polycall_register_config_validation_hook(
    ...
    polycall_config_validation_hook_t validation_hook, void *hook_ctx) {
  if (!config_ctx || !validation_hook) {
    return POLYCALL_CORE_ERROR_INVALID_PARAMETERS;
  }

  return polycall_config_register_validation_hook(config_ctx, validation_hook,
                                                  hook_ctx);
}
 ...
 */
bool default_validation_hook(polycall_config_context_t *config_ctx,
                             polycall_component_type_t component_type,
                             const void *component_config, void *hook_ctx,
                             char *error_message, size_t error_message_size) {
  ...
  polycall_core_error_t result = polycall_validate_component_configuration(
      contexts->core_ctx, contexts->schema_ctx, contexts->security_ctx,
      component_type, component_config, error_message, error_message_size);
...
polycall_core_error_t polycall_setup_integrated_validation(
  ...
  validation_context_pair_t *contexts =
      malloc(sizeof(validation_context_pair_t));
  if (!contexts) {
  ...
  polycall_core_error_t result = polycall_create_validation_contexts(
      ctx, &contexts->schema_ctx, &contexts->security_ctx, strict_validation,
      security_flags);
  ...
  result = polycall_register_config_validation_hook(
      config_ctx, default_validation_hook, contexts);

  if (result != POLYCALL_CORE_SUCCESS) {
    polycall_destroy_validation_contexts(ctx, contexts->schema_ctx,
 ...
 */
void polycall_cleanup_integrated_validation(polycall_core_context_t *ctx,
                                            void *validation_contexts) {
  ...
  validation_context_pair_t *contexts =
      (validation_context_pair_t *)validation_contexts;
