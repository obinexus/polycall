...
/**
 * @file repl.c
 * @brief Enhancements to the REPL implementation to support Biafran UI/UX
 * design system
 *
 ...
 */
polycall_core_error_t polycall_repl_init(polycall_core_context_t *core_ctx,
  ...
  if (config->enable_accessibility) {
    polycall_accessibility_config_t access_config =
        polycall_accessibility_default_config();
    ...
    polycall_core_error_t result = polycall_accessibility_init(
        core_ctx, &access_config, &(ctx->access_ctx));
...
static void print_prompt(polycall_repl_context_t *repl_ctx) {
  ...
  if (repl_ctx->access_ctx) {
    ...
    if (polycall_accessibility_format_prompt(
            repl_ctx->core_ctx, repl_ctx->access_ctx, repl_ctx->prompt,
            formatted_prompt, sizeof(formatted_prompt))) {
...
static void print_error(const char *message,
  ...
  if (repl_ctx && repl_ctx->access_ctx) {
    ...
    if (polycall_accessibility_format_text(
            repl_ctx->core_ctx, repl_ctx->access_ctx, message,
            POLYCALL_TEXT_ERROR, POLYCALL_STYLE_NORMAL, formatted_error,
...
static void print_success(const char *message,
  ...
  if (repl_ctx && repl_ctx->access_ctx) {
    ...
    if (polycall_accessibility_format_text(
            repl_ctx->core_ctx, repl_ctx->access_ctx, message,
            POLYCALL_TEXT_SUCCESS, POLYCALL_STYLE_NORMAL, formatted_success,
...
static void print_info(const char *message, polycall_repl_context_t *repl_ctx) {
  ...
  if (repl_ctx && repl_ctx->access_ctx) {
    ...
    if (polycall_accessibility_format_text(
            repl_ctx->core_ctx, repl_ctx->access_ctx, message,
            POLYCALL_TEXT_NORMAL, POLYCALL_STYLE_NORMAL, formatted_info,
...
static void print_help(polycall_repl_context_t *repl_ctx) {
  ...
  if (repl_ctx && repl_ctx->access_ctx) {
    ...
    polycall_accessibility_format_text(
        repl_ctx->core_ctx, repl_ctx->access_ctx, "LibPolyCall REPL Commands",
        POLYCALL_TEXT_HEADING, POLYCALL_STYLE_BOLD, heading, sizeof(heading));
    ...
    polycall_accessibility_format_text(
        repl_ctx->core_ctx, repl_ctx->access_ctx,
        "Built-in Commands:", POLYCALL_TEXT_SUBCOMMAND, POLYCALL_STYLE_BOLD,
        section_heading, sizeof(section_heading));
    ...
    for (size_t i = 0;
      ...
      char cmd_name[128];
      polycall_accessibility_format_text(
          repl_ctx->core_ctx, repl_ctx->access_ctx, built_in_commands[i].name,
          POLYCALL_TEXT_COMMAND, POLYCALL_STYLE_NORMAL, cmd_name,
          sizeof(cmd_name));

      char cmd_desc[256];
      polycall_accessibility_format_text(
          repl_ctx->core_ctx, repl_ctx->access_ctx, built_in_commands[i].desc,
          POLYCALL_TEXT_NORMAL, POLYCALL_STYLE_NORMAL, cmd_desc,
          sizeof(cmd_desc));
    ...
    if (count > 0) {
      ...
      polycall_accessibility_format_text(
          repl_ctx->core_ctx, repl_ctx->access_ctx,
          "Registered Commands:", POLYCALL_TEXT_SUBCOMMAND, POLYCALL_STYLE_BOLD,
          reg_heading, sizeof(reg_heading));
      ...
      for (int i = 0; i < count; i++) {
        char cmd_name[128];
        polycall_accessibility_format_text(
            repl_ctx->core_ctx, repl_ctx->access_ctx, commands[i].name,
            POLYCALL_TEXT_COMMAND, POLYCALL_STYLE_NORMAL, cmd_name,
            sizeof(cmd_name));

        char cmd_desc[256];
        polycall_accessibility_format_text(
            repl_ctx->core_ctx, repl_ctx->access_ctx, commands[i].description,
            POLYCALL_TEXT_NORMAL, POLYCALL_STYLE_NORMAL, cmd_desc,
            sizeof(cmd_desc));
 ...
 */
polycall_core_error_t polycall_repl_run(polycall_core_context_t *core_ctx,
                                        polycall_repl_context_t *repl_ctx) {
  ...
  if (repl_ctx->access_ctx) {
    char welcome[512];
    polycall_accessibility_format_text(
        core_ctx, repl_ctx->access_ctx, "LibPolyCall Interactive REPL",
    ...
    polycall_accessibility_format_text(
        ...
        POLYCALL_TEXT_NORMAL, POLYCALL_STYLE_NORMAL, instructions,
        sizeof(instructions));
  ...
  while (repl_ctx->running) {
    // Read command line (call print_prompt with repl_ctx to use enhanced
    // version)
 ...
 */
void polycall_repl_cleanup(polycall_core_context_t *core_ctx,
                           polycall_repl_context_t *repl_ctx) {
  if (!repl_ctx) {
    return;
  }
