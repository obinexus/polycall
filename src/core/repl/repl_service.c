...
polycall_repl_register_services(polycall_cli_container_t *container) {
  if (!container) {
  ...
  polycall_core_context_t *core_ctx =
      container->resolve_service(container, "core_context");
  if (!core_ctx) {
    return POLYCALL_CORE_ERROR_INVALID_PARAMETERS;
  }
  ...
  polycall_config_context_t *config_ctx =
      container->resolve_service(container, "config_context");

  // Create default REPL configuration
  polycall_repl_config_t repl_config = {.show_prompts = true,
                                        .echo_commands = true,
                                        .save_history = true,
  ...
  polycall_core_error_t result =
      polycall_repl_init(core_ctx, &repl_ctx, &repl_config);
...
void polycall_repl_cleanup_services(polycall_cli_container_t *container) {
  if (!container) {
    return;
  }
  ...
  polycall_repl_context_t *repl_ctx =
      container->resolve_service(container, "repl_context");
  if (!repl_ctx) {
    return;
  }
