...
static void format_identifier_string(polycall_identifier_t *identifier,
  ...
  case POLYCALL_ID_FORMAT_CRYPTONOMIC:
    ...
    sprintf(str,
            "C-%02x%02x%02x%02x-%02x%02x-%02x%02x-%02x%02x-%02x%02x%02x%02x%"
            "02x%02x",
            bytes[0], bytes[1], bytes[2], bytes[3], bytes[4], bytes[5],
            bytes[6], bytes[7], bytes[8], bytes[9], bytes[10], bytes[11],
            bytes[12], bytes[13], bytes[14], bytes[15]);
...
polycall_identifier_create(polycall_core_context_t *core_ctx,
...
polycall_core_error_t
polycall_identifier_from_string(polycall_core_context_t *core_ctx,
                                polycall_identifier_t *identifier,
...
polycall_core_error_t
polycall_identifier_to_string(polycall_core_context_t *core_ctx,
                              const polycall_identifier_t *identifier,
                              char *buffer, size_t buffer_size,
                              polycall_identifier_format_t output_format) {

  if (!core_ctx || !identifier || !buffer ||
      buffer_size < POLYCALL_MAX_ID_LEN) {
    return POLYCALL_CORE_ERROR_INVALID_PARAMETERS;
  }
...
polycall_core_error_t
polycall_identifier_validate(polycall_core_context_t *core_ctx,
                             const polycall_identifier_t *identifier) {
...
polycall_core_error_t polycall_identifier_generate_cryptonomic(
    polycall_core_context_t *core_ctx, polycall_identifier_t *identifier,
    const char *namespace_id, uint32_t state_id, const char *entity_id) {
...
polycall_core_error_t
polycall_identifier_update_state(polycall_core_context_t *core_ctx,
                                 polycall_identifier_t *identifier,
                                 const polycall_identifier_t *parent_id,
                                 uint32_t state_id, uint32_t event_id) {
