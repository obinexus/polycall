...
polycall_component_validator_t *
polycall_component_validator_create(const char *component_name) {
  if (!component_name) {
    return NULL;
  }
...
polycall_result_t polycall_component_validator_add_constraint(
    polycall_component_validator_t *validator, const char *prop_name,
    polycall_dop_data_type_t type, bool required,
    polycall_validation_func validate, const char *error_message) {
...
polycall_result_t
polycall_component_validator_validate(polycall_component_validator_t *validator,
  ...
  for (size_t i = 0; i < validator->constraint_count; i++) {
    ...
    const char *prop_name =
        validator->constraints[i].error_message; /*
                                                  *
                                                  *
    ...
    if (!constraint->validate(prop_value, props)) {
      if (error_out) {
        ...
        snprintf(error_out->message, sizeof(error_out->message),
                 "Validation failed for prop '%s': %s", prop_name,
...
polycall_dop_data_model_t *polycall_dop_data_model_create(
    void *data, void *(*clone)(void *data), void *(*to_object)(void *data),
    void *(*merge)(void *data, void *other),
    bool (*equals)(void *data, void *other), void (*free_func)(void *data)) {
...
polycall_dop_behavior_model_t *
polycall_dop_behavior_model_create(void *(*process)(void *data),
                                   const char *(*get_behavior_id)(void),
                                   const char *(*get_description)(void)) {
...
polycall_dop_adapter_create(polycall_dop_data_model_t *data_model,
                            polycall_dop_behavior_model_t *behavior_model,
                            polycall_component_validator_t *validator,
                            const char *adapter_name) {
...
void *polycall_dop_adapter_to_functional(polycall_dop_adapter_t *adapter) {
  ...
  polycall_logger_log(POLYCALL_LOG_INFO, "Converting %s to functional paradigm",
...
void *polycall_dop_adapter_to_oop(polycall_dop_adapter_t *adapter) {
  ...
  polycall_logger_log(POLYCALL_LOG_INFO, "Converting %s to OOP paradigm",
                      adapter->adapter_name);
