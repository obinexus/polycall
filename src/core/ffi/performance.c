...
polycall_core_error_t polycall_performance_init(
    ...
    performance_manager_t **perf_mgr, const performance_config_t *config) {
  if (!ctx || !ffi_ctx || !perf_mgr || !config) {
    return POLYCALL_CORE_ERROR_INVALID_PARAMETERS;
  }

  // Allocate performance manager
  performance_manager_t *mgr =
      polycall_core_malloc(ctx, sizeof(performance_manager_t));
  if (!mgr) {
  ...
  if (config->enable_call_caching) {
    polycall_core_error_t result = init_call_cache(
        ctx, &mgr->call_cache, config->cache_size, config->cache_ttl_ms);
  ...
  if (config->enable_type_caching) {
    polycall_core_error_t result =
        init_type_cache(ctx, &mgr->type_cache, config->cache_size);
    if (result != POLYCALL_CORE_SUCCESS) {
      if (mgr->call_cache) {
  ...
  if (config->enable_call_batching) {
    ...
    mgr->batch_queue =
        polycall_core_malloc(ctx, mgr->batch_capacity * sizeof(batch_entry_t));
    if (!mgr->batch_queue) {
      if (mgr->type_cache) {
      ...
      POLYCALL_ERROR_SET(
          ...
          POLYCALL_ERROR_SEVERITY_ERROR, "Failed to allocate batch queue");
      return POLYCALL_CORE_ERROR_OUT_OF_MEMORY;
    }
  ...
  mgr->trace_entries = polycall_core_malloc(
      ctx, mgr->trace_capacity * sizeof(performance_trace_entry_t));
  if (!mgr->trace_entries) {
    ...
    POLYCALL_ERROR_SET(
        ctx, POLYCALL_ERROR_SOURCE_FFI, POLYCALL_CORE_ERROR_OUT_OF_MEMORY,
        POLYCALL_ERROR_SEVERITY_ERROR, "Failed to allocate trace buffer");
    return POLYCALL_CORE_ERROR_OUT_OF_MEMORY;
  }
  memset(mgr->trace_entries, 0,
         mgr->trace_capacity * sizeof(performance_trace_entry_t));
...
polycall_core_error_t polycall_performance_get_metrics(
    polycall_core_context_t *ctx, polycall_ffi_context_t *ffi_ctx,
    performance_manager_t *perf_mgr, polycall_performance_metrics_t *metrics) {
...
polycall_core_error_t polycall_performance_register_hot_function(
    polycall_core_context_t *ctx, polycall_ffi_context_t *ffi_ctx,
    performance_manager_t *perf_mgr, const char *function_name,
  ...
  POLYCALL_ERROR_SET(ctx, POLYCALL_ERROR_SOURCE_FFI, POLYCALL_CORE_SUCCESS,
                     POLYCALL_ERROR_SEVERITY_INFO,
                     "Registered hot function: %s (level %d)", function_name,
                     opt_level);
...
polycall_core_error_t polycall_performance_set_optimization_level(
    polycall_core_context_t *ctx, polycall_ffi_context_t *ffi_ctx,
    performance_manager_t *perf_mgr, polycall_optimization_level_t opt_level) {
  if (!ctx || !ffi_ctx || !perf_mgr) {
    return POLYCALL_CORE_ERROR_INVALID_PARAMETERS;
  }
...
polycall_core_error_t polycall_performance_set_feature(
    polycall_core_context_t *ctx, polycall_ffi_context_t *ffi_ctx,
    performance_manager_t *perf_mgr, const char *feature_name, bool enabled) {
...
polycall_core_error_t polycall_performance_get_traces(
    polycall_core_context_t *ctx, polycall_ffi_context_t *ffi_ctx,
    performance_manager_t *perf_mgr, performance_trace_entry_t ***traces,
    size_t *trace_count) {
  ...
  performance_trace_entry_t **trace_array = polycall_core_malloc(
      ctx, perf_mgr->trace_count * sizeof(performance_trace_entry_t *));
  if (!trace_array) {
    pthread_mutex_unlock(&perf_mgr->trace_mutex);
    POLYCALL_ERROR_SET(ctx, POLYCALL_ERROR_SOURCE_FFI,
                       POLYCALL_CORE_ERROR_OUT_OF_MEMORY,
 ...
 */
polycall_core_error_t
polycall_performance_clear_traces(polycall_core_context_t *ctx,
                                  polycall_ffi_context_t *ffi_ctx,
                                  performance_manager_t *perf_mgr) {
  if (!ctx || !ffi_ctx || !perf_mgr) {
    return POLYCALL_CORE_ERROR_INVALID_PARAMETERS;
  ...
  for (size_t i = 0; i < perf_mgr->trace_count; i++) {
    ...
    if (perf_mgr->trace_entries[i].source_language) {
      polycall_core_free(ctx,
                         (void *)perf_mgr->trace_entries[i].source_language);
    }
    if (perf_mgr->trace_entries[i].target_language) {
      polycall_core_free(ctx,
                         (void *)perf_mgr->trace_entries[i].target_language);
    }
  }
...
polycall_core_error_t polycall_performance_export_data(
    polycall_core_context_t *ctx, polycall_ffi_context_t *ffi_ctx,
    performance_manager_t *perf_mgr, const char *filename, const char *format) {
  ...
  if (strcmp(format, "json") == 0) {
    ...
    // Write traces as CSV
    fprintf(file, "\nfunction_name,source_language,target_language,start_time_"
                  "ns,end_time_ns,execution_time_ns,marshalling_time_ns,arg_"
                  "count,cached,batched,sequence\n");
    ...
    for (size_t i = 0; i < perf_mgr->trace_count; i++) {
      ...
      fprintf(file, "\"%s\",\"%s\",\"%s\",%llu,%llu,%llu,%llu,%zu,%s,%s,%u\n",
              entry->function_name, entry->source_language,
              entry->target_language, (unsigned long long)entry->start_time_ns,
              (unsigned long long)entry->end_time_ns,
              (unsigned long long)entry->execution_time_ns,
              (unsigned long long)entry->marshalling_time_ns, entry->arg_count,
              entry->cached ? "true" : "false",
              entry->batched ? "true" : "false", entry->sequence);
    }
    ...
    fprintf(file, "------------------------------------------------------------"
                  "--------------------\n");
    fprintf(file,
            "%-30s %-15s %-15s %-15s %-15s %-15s %-15s %-10s %-8s %-8s %-10s\n",
            "Function", "Source", "Target", "Start (ns)", "End (ns)",
            "Exec (ns)", "Marshal (ns)", "Args", "Cached", "Batched",
            "Sequence");
    fprintf(file, "------------------------------------------------------------"
                  "--------------------\n");
    ...
    for (size_t i = 0; i < perf_mgr->trace_count; i++) {
      ...
      fprintf(file,
              "%-30.30s %-15.15s %-15.15s %-15llu %-15llu %-15llu %-15llu "
              "%-10zu %-8s %-8s %-10u\n",
              entry->function_name, entry->source_language,
              entry->target_language, (unsigned long long)entry->start_time_ns,
              (unsigned long long)entry->end_time_ns,
              (unsigned long long)entry->execution_time_ns,
              (unsigned long long)entry->marshalling_time_ns, entry->arg_count,
              entry->cached ? "Yes" : "No", entry->batched ? "Yes" : "No",
              entry->sequence);
    }
  ...
  polycall_core_error_t polycall_performance_execute_batch(
      polycall_core_context_t * ctx, polycall_ffi_context_t * ffi_ctx,
      performance_manager_t * perf_mgr, ffi_value_t * **results,
      size_t * result_count) {
    ...
    ffi_value_t **result_array = polycall_core_malloc(
        ctx, perf_mgr->batch_queue_count * sizeof(ffi_value_t *));
    if (!result_array) {
      pthread_mutex_unlock(&perf_mgr->batch_mutex);
      POLYCALL_ERROR_SET(ctx, POLYCALL_ERROR_SOURCE_FFI,
                         POLYCALL_CORE_ERROR_OUT_OF_MEMORY,
    ...
    for (size_t i = 0; i < perf_mgr->batch_queue_count; i++) {
      ...
      polycall_performance_trace_begin(ctx, ffi_ctx, perf_mgr,
                                       entry->function_name, "batch",
                                       entry->target_language, &trace_entry);
      ...
      polycall_core_error_t call_result = polycall_ffi_call_function(
          ctx, ffi_ctx, entry->function_name, entry->args, entry->arg_count,
          result, entry->target_language);
      ...
      if (call_result != POLYCALL_CORE_SUCCESS) {
        ...
        POLYCALL_ERROR_SET(ctx, POLYCALL_ERROR_SOURCE_FFI, call_result,
                           POLYCALL_ERROR_SEVERITY_ERROR,
                           "Batch call %zu failed: %s", i,
                           entry->function_name);
        return call_result;
      }
  ...
  void polycall_performance_cleanup(polycall_core_context_t * ctx,
    ...
    if (perf_mgr->trace_entries) {
      ...
      for (size_t i = 0; i < perf_mgr->trace_count; i++) {
        if (perf_mgr->trace_entries[i].function_name) {
          polycall_core_free(ctx,
                             (void *)perf_mgr->trace_entries[i].function_name);
        }
        if (perf_mgr->trace_entries[i].source_language) {
          polycall_core_free(
              ctx, (void *)perf_mgr->trace_entries[i].source_language);
        }
        if (perf_mgr->trace_entries[i].target_language) {
          polycall_core_free(
              ctx, (void *)perf_mgr->trace_entries[i].target_language);
        }
      }
  ...
  polycall_core_error_t polycall_performance_trace_begin(
    ...
    if (!ctx || !ffi_ctx || !perf_mgr || !function_name || !source_language ||
        !target_language || !trace_entry) {
      return POLYCALL_CORE_ERROR_INVALID_PARAMETERS;
    }
    ...
    if (perf_mgr->trace_count >= perf_mgr->trace_capacity) {
      ...
      if (!new_entries) {
        ...
        POLYCALL_ERROR_SET(
            ctx, POLYCALL_ERROR_SOURCE_FFI, POLYCALL_CORE_ERROR_OUT_OF_MEMORY,
            POLYCALL_ERROR_SEVERITY_ERROR, "Failed to expand trace buffer");
        return POLYCALL_CORE_ERROR_OUT_OF_MEMORY;
      }
      ...
      memset(&perf_mgr->trace_entries[perf_mgr->trace_capacity], 0,
             (new_capacity - perf_mgr->trace_capacity) *
                 sizeof(performance_trace_entry_t));
      perf_mgr->trace_capacity = new_capacity;
    }

    // Create new trace entry
    performance_trace_entry_t *entry =
        &perf_mgr->trace_entries[perf_mgr->trace_count++];
    ...
    if (!fn_copy || !src_copy || !tgt_copy) {
      if (fn_copy)
        polycall_core_free(ctx, fn_copy);
      if (src_copy)
        polycall_core_free(ctx, src_copy);
      if (tgt_copy)
        polycall_core_free(ctx, tgt_copy);
    ...
    entry->start_time_ns =
        get_current_time_ms() * 1000000; // Convert to nanoseconds
    entry->end_time_ns = 0;
  ...
  polycall_core_error_t polycall_performance_trace_end(
    ...
    // Record end time
    trace_entry->end_time_ns =
        get_current_time_ms() * 1000000; // Convert to nanoseconds

    // Calculate execution time
    trace_entry->execution_time_ns =
        trace_entry->end_time_ns - trace_entry->start_time_ns;

    // Update metrics
    perf_mgr->metrics.total_execution_time_ns += trace_entry->execution_time_ns;
    perf_mgr->metrics.total_marshalling_time_ns +=
        trace_entry->marshalling_time_ns;
  ...
  bool polycall_performance_check_cache(
      polycall_core_context_t * ctx, polycall_ffi_context_t * ffi_ctx,
      performance_manager_t * perf_mgr, const char *function_name,
      ffi_value_t *args, size_t arg_count, ffi_value_t **cached_result) {
    if (!ctx || !ffi_ctx || !perf_mgr || !function_name ||
        (!args && arg_count > 0) || !cached_result) {
      return false;
    }
  ...
  polycall_core_error_t polycall_performance_cache_result(
    ...
    if (perf_mgr->call_cache->count >= perf_mgr->call_cache->capacity) {
      ...
      for (size_t i = 0; i < perf_mgr->call_cache->count; i++) {
        ...
        if (entry->access_count < lowest_access ||
            (entry->access_count == lowest_access &&
             entry->cache_time < oldest_time)) {
          lru_index = i;
      ...
      if (lru_index < perf_mgr->call_cache->count - 1) {
        memmove(&perf_mgr->call_cache->entries[lru_index],
                ...
                (perf_mgr->call_cache->count - lru_index - 1) *
    ...
    cache_entry_t *new_entry =
        &perf_mgr->call_cache->entries[perf_mgr->call_cache->count];

    // Copy function name
    new_entry->function_name =
        polycall_core_malloc(ctx, strlen(function_name) + 1);
  ...
  static uint64_t hash_function_call(const char *function_name,
    ...
    for (size_t i = 0; i < arg_count; i++) {
      ...
      case FFI_TYPE_STRING:
        if (args[i].value.string_val) {
          hash ^= calculate_hash(args[i].value.string_val,
                                 strlen(args[i].value.string_val));
        }
        break;
  ...
  static uint64_t hash_function_result(ffi_value_t * result) {
    ...
    case FFI_TYPE_STRING:
      if (result->value.string_val) {
        hash ^= calculate_hash(result->value.string_val,
                               strlen(result->value.string_val));
      }
      break;
    // Additional types would be handled here
    default:
  ...
  static int compare_trace_entries(const void *a, const void *b) {
    const performance_trace_entry_t *entry_a =
        (const performance_trace_entry_t *)a;
    const performance_trace_entry_t *entry_b =
        (const performance_trace_entry_t *)b;

    return (entry_a->sequence < entry_b->sequence)   ? -1
           : (entry_a->sequence > entry_b->sequence) ? 1
                                                     : 0;
  }

  static void process_cache_expiry(polycall_core_context_t * ctx,
                                   call_cache_t * cache) {
    ...
    while (i < cache->count) {
      ...
      if (cache->ttl_ms > 0 &&
          (current_time - entry->cache_time) > cache->ttl_ms) {
        // Free resources
        polycall_core_free(ctx, entry->function_name);
        free_ffi_value(ctx, entry->cached_result);
        polycall_core_free(ctx, entry->cached_result);
  ...
  static ffi_value_t *clone_ffi_value(polycall_core_context_t * ctx,
    ...
    case FFI_TYPE_STRING:
      if (src->value.string_val) {
        dest->value.string_val =
            polycall_core_malloc(ctx, strlen(src->value.string_val) + 1);
  ...
  static void free_ffi_value(polycall_core_context_t * ctx,
                             ffi_value_t * value) {
    if (!value) {
      return;
    }
  ...
  static polycall_core_error_t init_call_cache(
      polycall_core_context_t * ctx, call_cache_t * *cache, size_t capacity,
      uint32_t ttl_ms) {
    call_cache_t *new_cache = polycall_core_malloc(ctx, sizeof(call_cache_t));
    if (!new_cache) {
      POLYCALL_ERROR_SET(
          ctx, POLYCALL_ERROR_SOURCE_FFI, POLYCALL_CORE_ERROR_OUT_OF_MEMORY,
          POLYCALL_ERROR_SEVERITY_ERROR, "Failed to allocate call cache");
      return POLYCALL_CORE_ERROR_OUT_OF_MEMORY;
    }

    new_cache->entries =
        polycall_core_malloc(ctx, capacity * sizeof(cache_entry_t));
  ...
  static void cleanup_call_cache(polycall_core_context_t * ctx,
                                 call_cache_t * cache) {
    if (!cache) {
      return;
    }
  ...
  static polycall_core_error_t init_type_cache(
      polycall_core_context_t * ctx, type_cache_t * *cache, size_t capacity) {
    type_cache_t *new_cache = polycall_core_malloc(ctx, sizeof(type_cache_t));
    if (!new_cache) {
      POLYCALL_ERROR_SET(
          ctx, POLYCALL_ERROR_SOURCE_FFI, POLYCALL_CORE_ERROR_OUT_OF_MEMORY,
          POLYCALL_ERROR_SEVERITY_ERROR, "Failed to allocate type cache");
      return POLYCALL_CORE_ERROR_OUT_OF_MEMORY;
    }

    new_cache->entries =
        polycall_core_malloc(ctx, capacity * sizeof(type_cache_entry_t));
    if (!new_cache->entries) {
      polycall_core_free(ctx, new_cache);
      POLYCALL_ERROR_SET(ctx, POLYCALL_ERROR_SOURCE_FFI,
                         POLYCALL_CORE_ERROR_OUT_OF_MEMORY,
  ...
  static void cleanup_type_cache(polycall_core_context_t * ctx,
                                 type_cache_t * cache) {
    if (!cache) {
      return;
    }
