...
static const size_t type_info_count =
    sizeof(type_info_table) / sizeof(type_info_table[0]);
...
bool polycall_ffi_types_compatible(polycall_ffi_type_t type1,
  ...
  bool type1_numeric =
      (type1 >= POLYCALL_FFI_TYPE_INT8 && type1 <= POLYCALL_FFI_TYPE_DOUBLE);
  bool type2_numeric =
      (type2 >= POLYCALL_FFI_TYPE_INT8 && type2 <= POLYCALL_FFI_TYPE_DOUBLE);
  ...
  if (type1 == POLYCALL_FFI_TYPE_POINTER ||
      type2 == POLYCALL_FFI_TYPE_POINTER) {
    /* Pointers can convert to/from certain types */
    if (type1 == POLYCALL_FFI_TYPE_STRING ||
        type2 == POLYCALL_FFI_TYPE_STRING || type1 == POLYCALL_FFI_TYPE_ARRAY ||
        type2 == POLYCALL_FFI_TYPE_ARRAY ||
        type1 == POLYCALL_FFI_TYPE_FUNCTION ||
        type2 == POLYCALL_FFI_TYPE_FUNCTION) {
      return true;
    }
  }
...
polycall_ffi_error_t polycall_ffi_value_copy(polycall_ffi_value_t *dest,
  ...
  case POLYCALL_FFI_TYPE_ARRAY:
    if (src->value.array_val.data && src->value.array_val.size > 0) {
      ...
      memcpy(dest->value.array_val.data, src->value.array_val.data,
  ...
  case POLYCALL_FFI_TYPE_STRUCT:
    ...
    if (src->value.struct_val.fields && src->value.struct_val.count > 0) {
      ...
      if (struct_size > 0) {
        ...
        memcpy(dest->value.struct_val.fields, src->value.struct_val.fields,
               struct_size);
      }
    }
    break;
