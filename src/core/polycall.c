...
static int init_module_containers() {
  ...
  polycall_container_register_service(g_polycall_container, "auth_container",
                                      g_auth_container);
  polycall_container_register_service(g_polycall_container, "config_container",
                                      g_config_container);
  polycall_container_register_service(g_polycall_container, "edge_container",
                                      g_edge_container);
  polycall_container_register_service(g_polycall_container, "ffi_container",
                                      g_ffi_container);
  polycall_container_register_service(g_polycall_container, "micro_container",
                                      g_micro_container);
  polycall_container_register_service(g_polycall_container, "network_container",
                                      g_network_container);
  polycall_container_register_service(
      g_polycall_container, "protocol_container", g_protocol_container);
  polycall_container_register_service(
      g_polycall_container, "telemetry_container", g_telemetry_container);
  polycall_container_register_service(g_polycall_container,
                                      "accessibility_container",
                                      g_accessibility_container);

  // Register command handler
  polycall_container_register_service(g_polycall_container, "command_handler",
                                      polycall_command_handler);
...
static void cleanup_module_containers() {
  if (g_polycall_container)
    polycall_container_destroy(g_polycall_container);
  if (g_auth_container)
    auth_container_destroy(g_auth_container);
  if (g_config_container)
    config_container_destroy(g_config_container);
  if (g_edge_container)
    edge_container_destroy(g_edge_container);
  if (g_ffi_container)
    ffi_container_destroy(g_ffi_container);
  if (g_micro_container)
    micro_container_destroy(g_micro_container);
  if (g_network_container)
    network_container_destroy(g_network_container);
  if (g_protocol_container)
    protocol_container_destroy(g_protocol_container);
  if (g_telemetry_container)
    telemetry_container_destroy(g_telemetry_container);
  if (g_accessibility_container)
    accessibility_container_destroy(g_accessibility_container);
 ...
 */
void polycall_cleanup() { cleanup_module_containers(); }
...
int polycall_register_service(const char *service_name, void *service) {
  ...
  return polycall_container_register_service(g_polycall_container, service_name,
                                             service);
}
