...
int adapter_base_init(adapter_base_t *adapter,
...
int adapter_base_release(adapter_base_t *adapter) {
  int adapter_base_release(adapter_base_t * adapter) {
    if (!adapter)
      return -1;
    if (atomic_fetch_sub_explicit(&adapter->ref_count, 1,
  ...
  int adapter_execute_transition(adapter_base_t * adapter, uint64_t thread_id,
    ...
    if (!adapter || !adapter->vtable)
      return -1;
    if (adapter->vtable->validate_transition) {
      int res = adapter->vtable->validate_transition(
          adapter, adapter->adapter_layer_id, target_layer);
      if (res != 0)
        return res;
    }
    ...
    return 0;
                              ...
                              uint32_t target_layer) {
                                ...
                                int result =
                                    topology_manager_validate_transition(
                                        adapter->manager,
