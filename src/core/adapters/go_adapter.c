...
static int go_adapter_init(void *adapter, topology_manager_t *manager) {
  ...
  if (adapter_base_init(&go->base, manager) != 0)
    return -1;
...
static int go_adapter_enter_layer(void *adapter, uint64_t thread_id,
                                  uint32_t layer_id) {
  go_adapter_t *go = (go_adapter_t *)adapter;
...
adapter_base_t *create_go_adapter(topology_manager_t *manager,
                                  uintptr_t handle) {
  go_adapter_t *adapter = calloc(1, sizeof(go_adapter_t));
  if (!adapter)
    return NULL;
  adapter->go_handle = handle;
  static adapter_vtable_t vt = {.init = go_adapter_init,
