...
static int nodejs_adapter_init(void *adapter, topology_manager_t *manager) {
  ...
  if (adapter_base_init(&na->base, manager) != 0)
    return -1;
  na->base.adapter_layer_id = TOPOLOGY_LAYER_NODEJS;
  return 0;
}

static int nodejs_adapter_enter_layer(void *adapter, uint64_t thread_id,
                                      uint32_t layer_id) {
  nodejs_adapter_t *na = (nodejs_adapter_t *)adapter;
...
adapter_base_t *create_nodejs_adapter(topology_manager_t *manager,
                                      napi_env env) {
  nodejs_adapter_t *adapter = calloc(1, sizeof(nodejs_adapter_t));
  if (!adapter)
    return NULL;
  adapter->env = env;
  static adapter_vtable_t vt = {.init = nodejs_adapter_init,
