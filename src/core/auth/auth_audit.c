 ...
 *
 * This file implements the audit logging interfaces for LibPolyCall
 * authentication, providing functions to log, query, and export
...
polycall_auth_log_audit_event(polycall_core_context_t *core_ctx,
  ...
  if (auth_ctx->auth_audit->entry_count >=
      auth_ctx->auth_audit->entry_capacity) {
    size_t new_capacity = auth_ctx->auth_audit->entry_capacity * 2;
    if (new_capacity == 0) {
      new_capacity = 32; // Initial capacity if not yet initialized
    }

    audit_entry_t **new_entries =
        polycall_core_malloc(core_ctx, new_capacity * sizeof(audit_entry_t *));
    if (!new_entries) {
  ...
  POLYCALL_LOG(core_ctx, POLYCALL_LOG_INFO,
              "Audit event [%s]: identity=%s, resource=%s, action=%s,
  success=%s", polycall_audit_event_type_to_string(event->type),
...
polycall_core_error_t polycall_auth_query_audit_events(
    ...
    const audit_query_t *query, audit_event_t ***events, size_t *event_count) {
  // Validate parameters
  ...
  *event_count = 0;
  ...
  audit_event_t **result =
      polycall_core_malloc(core_ctx, result_count * sizeof(audit_event_t *));
  if (!result) {
    pthread_mutex_unlock(&auth_ctx->auth_audit->mutex);
    return POLYCALL_CORE_ERROR_OUT_OF_MEMORY;
  }
  ...
  for (size_t i = 0;
       i < auth_ctx->auth_audit->entry_count && result_index < result_count;
       i++) {
    ...
    if (match_audit_event(entry, query)) {
      ...
      audit_event_t *event_copy = polycall_auth_create_audit_event(
          core_ctx, entry->event.type, entry->event.identity_id,
          entry->event.resource, entry->event.action, entry->event.success,
...
polycall_core_error_t polycall_auth_export_audit_events(
    polycall_core_context_t *core_ctx, polycall_auth_context_t *auth_ctx,
    const audit_query_t *query, const char *filename, const char *format) {
  // Validate parameters
  ...
  polycall_core_error_t result = polycall_auth_query_audit_events(
      core_ctx, auth_ctx, query, &events, &event_count);
...
audit_event_t *polycall_auth_create_audit_event(
    polycall_core_context_t *core_ctx, polycall_audit_event_type_t type,
 ...
 */
void polycall_auth_free_audit_event(polycall_core_context_t *core_ctx,
                                    audit_event_t *event) {
  if (!core_ctx || !event) {
    return;
  }
 ...
 */
void polycall_auth_free_audit_events(polycall_core_context_t *core_ctx,
 ...
 */
const char *
polycall_audit_event_type_to_string(polycall_audit_event_type_t type) {
  switch (type) {
 ...
 */
auth_audit_t *init_auth_audit(polycall_core_context_t *ctx,
                              bool enable_logging) {
  if (!ctx) {
    return NULL;
  }
  ...
  audit->entries = polycall_core_malloc(ctx, audit->entry_capacity *
 ...
 */
static audit_entry_t *create_audit_entry(polycall_core_context_t *core_ctx,
                                         const audit_event_t *event) {
  if (!core_ctx || !event) {
    return NULL;
  }
 ...
 */
static void free_audit_entry(polycall_core_context_t *core_ctx,
                             audit_entry_t *entry) {
  if (!core_ctx || !entry) {
    return;
  }
...
static bool match_audit_event(const audit_entry_t *entry,
  ...
  // Check event type
  if (query->type != POLYCALL_AUDIT_EVENT_CUSTOM &&
      entry->event.type != query->type) {
    return false;
  }
 ...
 */
static char *export_events_to_json(polycall_core_context_t *core_ctx,
                                   audit_event_t **events, size_t event_count) {
  ...
  for (size_t i = 0; i < event_count; i++) {
    ...
    if (events[i]->identity_id) {
      buffer_size +=
          strlen(events[i]->identity_id) * 2; // Account for JSON escaping
    }
  ...
  for (size_t i = 0; i < event_count; i++) {
    ...
    if (events[i]->identity_id) {
      buffer_size +=
          strlen(events[i]->identity_id) * 2; // Account for CSV escaping
    }
 ...
 */
static char *export_events_to_csv(polycall_core_context_t *core_ctx,
                                  audit_event_t **events, size_t event_count) {
  if (!core_ctx || !events || event_count == 0) {
    return NULL;
  }
  ...
  // Write CSV header
  int pos =
      sprintf(buffer, "type,timestamp,identity_id,resource,action,success,"
