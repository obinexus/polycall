 ...
 *
 * This file implements the policy management interfaces for LibPolyCall
authentication,
 ...
 * @brief Add a role
 */
polycall_core_error_t polycall_auth_add_role(polycall_core_context_t *core_ctx,
  ...
  if (auth_ctx->policies->role_count >= auth_ctx->policies->role_capacity) {
    ...
    role_entry_t **new_roles =
        polycall_core_malloc(core_ctx, new_capacity * sizeof(role_entry_t *));
    if (!new_roles) {
      pthread_mutex_unlock(&auth_ctx->policies->mutex);
  ...
  audit_event_t *event = polycall_auth_create_audit_event(
      core_ctx, POLYCALL_AUDIT_EVENT_ROLE_ASSIGN, auth_ctx->current_identity,
      NULL, "create_role", true, NULL);
...
polycall_auth_assign_role(polycall_core_context_t *core_ctx,
                          polycall_auth_context_t *auth_ctx,
                          const char *identity_id, const char *role_name) {
  // Validate parameters
  ...
  // Get attributes
  identity_attributes_t *attrs =
      auth_ctx->identities->attributes[identity_index];
  ...
  char **new_roles =
      polycall_core_malloc(core_ctx, (attrs->role_count + 1) * sizeof(char *));
  if (!new_roles) {
    pthread_mutex_unlock(&auth_ctx->identities->mutex);
    return POLYCALL_CORE_ERROR_OUT_OF_MEMORY;
  }
  ...
  // Create audit event
  audit_event_t *event = polycall_auth_create_audit_event(
      core_ctx, POLYCALL_AUDIT_EVENT_ROLE_ASSIGN, identity_id, NULL,
      "assign_role", true, NULL);
 ...
 */
polycall_core_error_t
polycall_auth_remove_role(polycall_core_context_t *core_ctx,
                          polycall_auth_context_t *auth_ctx,
                          const char *identity_id, const char *role_name) {
  // Validate parameters
  if (!core_ctx || !auth_ctx || !identity_id || !role_name) {
  ...
  // Create audit event
  audit_event_t *event = polycall_auth_create_audit_event(
      core_ctx, POLYCALL_AUDIT_EVENT_ROLE_REMOVE, identity_id, NULL,
      "remove_role", true, NULL);
 ...
 */
polycall_core_error_t
polycall_auth_add_policy(polycall_core_context_t *core_ctx,
                         polycall_auth_context_t *auth_ctx,
  ...
  if (auth_ctx->policies->policy_count >= auth_ctx->policies->policy_capacity) {
    ...
    policy_entry_t **new_policies =
        polycall_core_malloc(core_ctx, new_capacity * sizeof(policy_entry_t *));
    if (!new_policies) {
      pthread_mutex_unlock(&auth_ctx->policies->mutex);
      return POLYCALL_CORE_ERROR_OUT_OF_MEMORY;
    }
  ...
  policy_entry_t *new_policy =
      polycall_core_malloc(core_ctx, sizeof(policy_entry_t));
  if (!new_policy) {
    pthread_mutex_unlock(&auth_ctx->policies->mutex);
    return POLYCALL_CORE_ERROR_OUT_OF_MEMORY;
  }
  ...
  if (policy->statements && policy->statement_count > 0) {
    new_policy->statements = polycall_core_malloc(
        core_ctx, policy->statement_count * sizeof(policy_statement_t *));
    if (!new_policy->statements) {
      if (new_policy->description)
        polycall_core_free(core_ctx, new_policy->description);
      polycall_core_free(core_ctx, new_policy->name);
      polycall_core_free(core_ctx, new_policy);
      pthread_mutex_unlock(&auth_ctx->policies->mutex);
      return POLYCALL_CORE_ERROR_OUT_OF_MEMORY;
    ...
    for (size_t i = 0; i < policy->statement_count; i++) {
      ...
      policy_statement_t *new_stmt =
          polycall_core_malloc(core_ctx, sizeof(policy_statement_t));
      if (!new_stmt) {
        for (size_t j = 0; j < i; j++) {
          if (new_policy->statements[j]) {
            // Clean up each statement
            if (new_policy->statements[j]->actions) {
              for (size_t k = 0; k < new_policy->statements[j]->action_count;
                   k++) {
                if (new_policy->statements[j]->actions[k]) {
                  polycall_core_free(core_ctx,
                                     new_policy->statements[j]->actions[k]);
                }
              }
            ...
            if (new_policy->statements[j]->resources) {
              for (size_t k = 0; k < new_policy->statements[j]->resource_count;
                   k++) {
                if (new_policy->statements[j]->resources[k]) {
                  polycall_core_free(core_ctx,
                                     new_policy->statements[j]->resources[k]);
                }
              }
              polycall_core_free(core_ctx,
                                 new_policy->statements[j]->resources);
            }

            if (new_policy->statements[j]->condition) {
              polycall_core_free(core_ctx,
                                 new_policy->statements[j]->condition);
            }
      ...
      if (src_stmt->actions && src_stmt->action_count > 0) {
        new_stmt->actions = polycall_core_malloc(
            core_ctx, src_stmt->action_count * sizeof(char *));
        if (!new_stmt->actions) {
          ...
          for (size_t j = 0; j < i; j++) {
            if (new_policy->statements[j]) {
              ...
              if (new_policy->statements[j]->actions) {
                ...
                polycall_core_free(core_ctx,
                                   new_policy->statements[j]->actions);
              }

              if (new_policy->statements[j]->resources) {
                for (size_t k = 0;
                     k < new_policy->statements[j]->resource_count; k++) {
                  if (new_policy->statements[j]->resources[k]) {
                    polycall_core_free(core_ctx,
                                       new_policy->statements[j]->resources[k]);
                  }
            ...
            if (!new_stmt->actions[k]) {
              ...
              for (size_t j = 0; j < i; j++) {
                if (new_policy->statements[j]) {
                  if (new_policy->statements[j]->actions) {
                    for (size_t m = 0;
                         m < new_policy->statements[j]->action_count; m++) {
                      if (new_policy->statements[j]->actions[m]) {
                        polycall_core_free(
                            core_ctx, new_policy->statements[j]->actions[m]);
                      }
                    }
                    polycall_core_free(core_ctx,
                                       new_policy->statements[j]->actions);
                  ...
                  if (new_policy->statements[j]->resources) {
                    for (size_t m = 0;
                         m < new_policy->statements[j]->resource_count; m++) {
                      if (new_policy->statements[j]->resources[m]) {
                        polycall_core_free(
                            core_ctx, new_policy->statements[j]->resources[m]);
                      }
                    }
                    polycall_core_free(core_ctx,
                                       new_policy->statements[j]->resources);
      ...
      if (src_stmt->resources && src_stmt->resource_count > 0) {
        new_stmt->resources = polycall_core_malloc(
            core_ctx, src_stmt->resource_count * sizeof(char *));
        ...
        for (size_t k = 0; k < src_stmt->resource_count; k++) {
          if (src_stmt->resources[k]) {
            ...
            new_stmt->resources[k] =
                polycall_core_malloc(core_ctx, resource_len);
            ...
            memcpy(new_stmt->resources[k], src_stmt->resources[k],
                   resource_len);
          } else {
  ...
  // Create audit event
  audit_event_t *event = polycall_auth_create_audit_event(
      core_ctx, POLYCALL_AUDIT_EVENT_POLICY_CREATE, auth_ctx->current_identity,
      NULL, NULL, true, NULL);

  if (event) {
    // Add policy name to details
    char details[256] = {0};
    snprintf(details, sizeof(details), "{\"policy_name\":\"%s\"}",
             policy->name);
    event->details = strdup(details);
 ...
 */
polycall_core_error_t
polycall_auth_attach_policy(polycall_core_context_t *core_ctx,
                            polycall_auth_context_t *auth_ctx,
                            const char *role_name, const char *policy_name) {
  // Validate parameters
  ...
  for (size_t i = 0; i < role->policy_count; i++) {
    if (role->policy_names[i] &&
        strcmp(role->policy_names[i], policy_name) == 0) {
      pthread_mutex_unlock(&auth_ctx->policies->mutex);
  ...
  // Add policy to role
  char **new_policies =
      polycall_core_malloc(core_ctx, (role->policy_count + 1) * sizeof(char *));
  if (!new_policies) {
    pthread_mutex_unlock(&auth_ctx->policies->mutex);
    return POLYCALL_CORE_ERROR_OUT_OF_MEMORY;
  }
  ...
  // Free old policy array (but not the strings, as they were copied by
  // reference)
  if (old_policies) {
  ...
  // Create audit event
  audit_event_t *event = polycall_auth_create_audit_event(
      core_ctx, POLYCALL_AUDIT_EVENT_POLICY_UPDATE, auth_ctx->current_identity,
      NULL, "attach_policy", true, NULL);

  if (event) {
    // Add details
    char details[512] = {0};
    snprintf(details, sizeof(details),
             "{\"role_name\":\"%s\",\"policy_name\":\"%s\"}", role_name,
             policy_name);
    event->details = strdup(details);
 ...
 */
polycall_core_error_t
polycall_auth_detach_policy(polycall_core_context_t *core_ctx,
                            polycall_auth_context_t *auth_ctx,
                            const char *role_name, const char *policy_name) {
  // Validate parameters
  if (!core_ctx || !auth_ctx || !role_name || !policy_name) {
...
polycall_core_error_t polycall_auth_evaluate_permission(
    ...
    const char *identity_id, const char *resource, const char *action,
    const char *context, bool *allowed) {
  // Validate parameters
  if (!core_ctx || !auth_ctx || !identity_id || !resource || !action ||
      !allowed) {
    return POLYCALL_CORE_ERROR_INVALID_PARAMETERS;
  }
  ...
  *allowed = false;
  ...
  polycall_core_error_t result =
      get_identity_roles(core_ctx, auth_ctx, identity_id, &roles, &role_count);
  ...
  for (size_t i = 0; i < role_count; i++) {
    ...
    for (size_t j = 0; j < role->policy_count; j++) {
      ...
      for (size_t k = 0; k < policy->statement_count; k++) {
        ...
        // Evaluate statement
        bool statement_match =
            evaluate_policy_statement(statement, resource, action, context);
  ...
  audit_event_t *event = polycall_auth_create_audit_event(
      core_ctx,
      *allowed ? POLYCALL_AUDIT_EVENT_ACCESS_GRANTED
               : POLYCALL_AUDIT_EVENT_ACCESS_DENIED,
      identity_id, resource, action, *allowed, NULL);
 ...
 */
polycall_core_error_t
polycall_auth_check_permission(polycall_core_context_t *core_ctx,
                               polycall_auth_context_t *auth_ctx,
                               const char *identity_id, const char *resource,
                               const char *action, bool *allowed) {
  // This is a simplified wrapper around evaluate_permission
  return polycall_auth_evaluate_permission(core_ctx, auth_ctx, identity_id,
                                           resource, action, NULL, allowed);
}

/**
 * @brief Check if a policy resource pattern matches a resource
 */
static bool policy_matches_resource(const char *policy_resource,
                                    const char *resource) {
  ...
  if (policy_len > 0 && policy_resource[policy_len - 1] == '*') {
    ...
    if (policy_len > 1 &&
        strncmp(policy_resource, resource, policy_len - 1) == 0) {
      return true;
    }
  }
 ...
 */
static bool policy_matches_action(const char *policy_action,
                                  const char *action) {
 ...
 */
static bool evaluate_policy_statement(policy_statement_t *statement,
  ...
  for (size_t i = 0; i < statement->resource_count; i++) {
    if (statement->resources[i] &&
        policy_matches_resource(statement->resources[i], resource)) {
      resource_match = true;
      break;
    }
  }
  ...
  for (size_t i = 0; i < statement->action_count; i++) {
    if (statement->actions[i] &&
        policy_matches_action(statement->actions[i], action)) {
      action_match = true;
      break;
    }
  }
...
get_identity_roles(polycall_core_context_t *core_ctx,
                   polycall_auth_context_t *auth_ctx, const char *identity_id,
