 ...
 *
 * This file implements the identity management interfaces for LibPolyCall
authentication,
...
polycall_core_error_t polycall_auth_register_identity(
    ...
    const char *identity_id, const identity_attributes_t *attributes,
  ...
  for (size_t i = 0; i < auth_ctx->identities->count; i++) {
    ...
    if (existing && existing->name &&
        strcmp(existing->name, attributes->name) == 0) {
      pthread_mutex_unlock(&auth_ctx->identities->mutex);
  ...
  if (auth_ctx->identities->count >= auth_ctx->identities->capacity) {
    ...
    char **new_ids =
        polycall_core_malloc(core_ctx, new_capacity * sizeof(char *));
    identity_attributes_t **new_attrs = polycall_core_malloc(
        core_ctx, new_capacity * sizeof(identity_attributes_t *));
    char **new_passwords =
        polycall_core_malloc(core_ctx, new_capacity * sizeof(char *));

    if (!new_ids || !new_attrs || !new_passwords) {
      if (new_ids)
        polycall_core_free(core_ctx, new_ids);
      if (new_attrs)
        polycall_core_free(core_ctx, new_attrs);
      if (new_passwords)
        polycall_core_free(core_ctx, new_passwords);
      pthread_mutex_unlock(&auth_ctx->identities->mutex);
      return POLYCALL_CORE_ERROR_OUT_OF_MEMORY;
    }

    // Copy existing entries
    memcpy(new_ids, auth_ctx->identities->identity_ids,
           auth_ctx->identities->count * sizeof(char *));
    memcpy(new_attrs, auth_ctx->identities->attributes,
           auth_ctx->identities->count * sizeof(identity_attributes_t *));
    memcpy(new_passwords, auth_ctx->identities->hashed_passwords,
           auth_ctx->identities->count * sizeof(char *));
  ...
  char *hashed_password =
      hash_password(auth_ctx->credentials, initial_password);
  if (!hashed_password) {
    pthread_mutex_unlock(&auth_ctx->identities->mutex);
    return POLYCALL_CORE_ERROR_OUT_OF_MEMORY;
  }
  ...
  identity_attributes_t *attr_copy =
      polycall_core_malloc(core_ctx, sizeof(identity_attributes_t));
  if (!attr_copy) {
    polycall_core_free(core_ctx, hashed_password);
  ...
  if (attributes->email) {
    ...
    if (!attr_copy->email) {
      ...
      polycall_core_free(core_ctx, id_copy);
      if (attr_copy->name)
        polycall_core_free(core_ctx, attr_copy->name);
      polycall_core_free(core_ctx, attr_copy);
      pthread_mutex_unlock(&auth_ctx->identities->mutex);
      return POLYCALL_CORE_ERROR_OUT_OF_MEMORY;
    }
  ...
  if (attributes->roles && attributes->role_count > 0) {
    attr_copy->roles =
        polycall_core_malloc(core_ctx, attributes->role_count * sizeof(char *));
    if (!attr_copy->roles) {
      polycall_core_free(core_ctx, hashed_password);
      polycall_core_free(core_ctx, id_copy);
      if (attr_copy->name)
        polycall_core_free(core_ctx, attr_copy->name);
      if (attr_copy->email)
        polycall_core_free(core_ctx, attr_copy->email);
      polycall_core_free(core_ctx, attr_copy);
      pthread_mutex_unlock(&auth_ctx->identities->mutex);
      return POLYCALL_CORE_ERROR_OUT_OF_MEMORY;
    }
  ...
  if (attributes->groups && attributes->group_count > 0) {
    attr_copy->groups = polycall_core_malloc(core_ctx, attributes->group_count *
                                                           sizeof(char *));
    if (!attr_copy->groups) {
      polycall_core_free(core_ctx, hashed_password);
      polycall_core_free(core_ctx, id_copy);
      ...
      if (attr_copy->roles) {
        for (size_t i = 0; i < attr_copy->role_count; i++) {
          if (attr_copy->roles[i])
            polycall_core_free(core_ctx, attr_copy->roles[i]);
        }
        polycall_core_free(core_ctx, attr_copy->roles);
      }
    ...
    if (!attr_copy->metadata) {
      ...
      if (attr_copy->groups) {
        for (size_t i = 0; i < attr_copy->group_count; i++) {
          if (attr_copy->groups[i])
            polycall_core_free(core_ctx, attr_copy->groups[i]);
        }
        polycall_core_free(core_ctx, attr_copy->groups);
      }
  ...
  auth_ctx->identities->hashed_passwords[auth_ctx->identities->count] =
      hashed_password;
  ...
  audit_event_t *event = polycall_auth_create_audit_event(
      core_ctx, POLYCALL_AUDIT_EVENT_IDENTITY_CREATE, identity_id, NULL, NULL,
      true, NULL);
...
polycall_core_error_t polycall_auth_get_identity_attributes(
    polycall_core_context_t *core_ctx, polycall_auth_context_t *auth_ctx,
    const char *identity_id, identity_attributes_t **attributes) {
  // Validate parameters
  if (!core_ctx || !auth_ctx || !identity_id || !attributes) {
    return POLYCALL_CORE_ERROR_INVALID_PARAMETERS;
  }
  ...
  identity_attributes_t *src_attrs =
  ...
  if (src_attrs->roles && src_attrs->role_count > 0) {
    attr_copy->roles =
        polycall_core_malloc(core_ctx, src_attrs->role_count * sizeof(char *));
    if (!attr_copy->roles) {
      if (attr_copy->name)
        polycall_core_free(core_ctx, attr_copy->name);
      if (attr_copy->email)
  ...
  if (src_attrs->groups && src_attrs->group_count > 0) {
    attr_copy->groups =
        polycall_core_malloc(core_ctx, src_attrs->group_count * sizeof(char *));
    if (!attr_copy->groups) {
      if (attr_copy->name)
        polycall_core_free(core_ctx, attr_copy->name);
      if (attr_copy->email)
...
polycall_core_error_t polycall_auth_update_identity(
  ...
  // Get existing attributes
  identity_attributes_t *existing =
      auth_ctx->identities->attributes[identity_index];

  // Create a new attributes structure
  identity_attributes_t *new_attrs =
      polycall_core_malloc(core_ctx, sizeof(identity_attributes_t));
  if (!new_attrs) {
    pthread_mutex_unlock(&auth_ctx->identities->mutex);
    return POLYCALL_CORE_ERROR_OUT_OF_MEMORY;
  ...
  if (attributes->email) {
    ...
    if (!new_attrs->email) {
      if (new_attrs->name)
        polycall_core_free(core_ctx, new_attrs->name);
      polycall_core_free(core_ctx, new_attrs);
      pthread_mutex_unlock(&auth_ctx->identities->mutex);
      return POLYCALL_CORE_ERROR_OUT_OF_MEMORY;
    }
  ...
  if (attributes->roles && attributes->role_count > 0) {
    ...
    if (!new_attrs->roles) {
      if (new_attrs->name)
        polycall_core_free(core_ctx, new_attrs->name);
      if (new_attrs->email)
        polycall_core_free(core_ctx, new_attrs->email);
      polycall_core_free(core_ctx, new_attrs);
      pthread_mutex_unlock(&auth_ctx->identities->mutex);
      return POLYCALL_CORE_ERROR_OUT_OF_MEMORY;
    }
    ...
    for (size_t i = 0; i < attributes->role_count; i++) {
      if (attributes->roles[i]) {
        ...
        if (!new_attrs->roles[i]) {
          ...
          for (size_t j = 0; j < i; j++) {
            if (new_attrs->roles[j])
              polycall_core_free(core_ctx, new_attrs->roles[j]);
          }
    ...
    // Keep existing roles
    new_attrs->roles =
        polycall_core_malloc(core_ctx, existing->role_count * sizeof(char *));
    if (!new_attrs->roles) {
      if (new_attrs->name)
        polycall_core_free(core_ctx, new_attrs->name);
      if (new_attrs->email)
  ...
  if (attributes->groups && attributes->group_count > 0) {
    new_attrs->groups = polycall_core_malloc(core_ctx, attributes->group_count *
                                                           sizeof(char *));
    if (!new_attrs->groups) {
      if (new_attrs->name)
        polycall_core_free(core_ctx, new_attrs->name);
      ...
      if (new_attrs->roles) {
        for (size_t i = 0; i < new_attrs->role_count; i++) {
          if (new_attrs->roles[i])
            polycall_core_free(core_ctx, new_attrs->roles[i]);
        }
        polycall_core_free(core_ctx, new_attrs->roles);
      }
    ...
    for (size_t i = 0; i < attributes->group_count; i++) {
      if (attributes->groups[i]) {
        ...
        if (!new_attrs->groups[i]) {
          ...
          for (size_t j = 0; j < i; j++) {
            if (new_attrs->groups[j])
              polycall_core_free(core_ctx, new_attrs->groups[j]);
          }
    ...
    new_attrs->groups =
        polycall_core_malloc(core_ctx, existing->group_count * sizeof(char *));
    if (!new_attrs->groups) {
      if (new_attrs->name)
        polycall_core_free(core_ctx, new_attrs->name);
      if (new_attrs->email)
    ...
    if (!new_attrs->metadata) {
      ...
      if (new_attrs->groups) {
        for (size_t i = 0; i < new_attrs->group_count; i++) {
          if (new_attrs->groups[i])
            polycall_core_free(core_ctx, new_attrs->groups[i]);
        }
        polycall_core_free(core_ctx, new_attrs->groups);
      }
  ...
  // Create audit event
  audit_event_t *event = polycall_auth_create_audit_event(
      core_ctx, POLYCALL_AUDIT_EVENT_IDENTITY_UPDATE, identity_id, NULL, NULL,
      true, NULL);
...
polycall_core_error_t polycall_auth_change_password(
    polycall_core_context_t *core_ctx, polycall_auth_context_t *auth_ctx,
    const char *identity_id, const char *current_password,
    const char *new_password) {
  // Validate parameters
  if (!core_ctx || !auth_ctx || !identity_id || !current_password ||
      !new_password) {
    return POLYCALL_CORE_ERROR_INVALID_PARAMETERS;
  }
  ...
  // Verify current password
  bool password_valid =
      verify_password(auth_ctx->credentials, current_password, hash_copy);
  ...
  if (!password_valid) {
    ...
    audit_event_t *event = polycall_auth_create_audit_event(
        core_ctx, POLYCALL_AUDIT_EVENT_PASSWORD_CHANGE, identity_id, NULL, NULL,
        false, "Invalid current password");
  ...
  if (identity_index >= (int)auth_ctx->identities->count ||
      strcmp(auth_ctx->identities->identity_ids[identity_index], identity_id) !=
          0) {
  ...
  if (auth_ctx->identities->hashed_passwords[identity_index]) {
    polycall_core_free(core_ctx,
                       auth_ctx->identities->hashed_passwords[identity_index]);
  }
...
polycall_core_error_t polycall_auth_reset_password(
    polycall_core_context_t *core_ctx, polycall_auth_context_t *auth_ctx,
    const char *identity_id, const char *new_password) {
  // Validate parameters
  ...
  // Create audit event
  audit_event_t *event = polycall_auth_create_audit_event(
      core_ctx, POLYCALL_AUDIT_EVENT_PASSWORD_RESET, identity_id, NULL, NULL,
      true, NULL);
...
polycall_auth_deactivate_identity(polycall_core_context_t *core_ctx,
  ...
  // Create audit event
  audit_event_t *event = polycall_auth_create_audit_event(
      core_ctx, POLYCALL_AUDIT_EVENT_IDENTITY_UPDATE, identity_id, NULL,
      "deactivate", true, NULL);
 ...
 */
polycall_core_error_t
polycall_auth_reactivate_identity(polycall_core_context_t *core_ctx,
                                  polycall_auth_context_t *auth_ctx,
                                  const char *identity_id) {
  // Validate parameters
  if (!core_ctx || !auth_ctx || !identity_id) {
 ...
 */
void polycall_auth_free_identity_attributes(polycall_core_context_t *core_ctx,
