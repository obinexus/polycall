 ...
 *
 * This file implements the authentication context that manages identity,
credentials,
...
polycall_core_error_t polycall_auth_init(polycall_core_context_t *core_ctx,
  ...
  polycall_auth_context_t *new_ctx =
      polycall_core_malloc(core_ctx, sizeof(polycall_auth_context_t));
  if (!new_ctx) {
  ...
  new_ctx->config.token_signing_secret =
      polycall_core_malloc(core_ctx, secret_len);
  if (!new_ctx->config.token_signing_secret) {
    polycall_core_free(core_ctx, new_ctx);
    return POLYCALL_CORE_ERROR_OUT_OF_MEMORY;
  }
  memcpy(new_ctx->config.token_signing_secret, config->token_signing_secret,
         secret_len);
  ...
  new_ctx->credentials =
      init_credential_store(core_ctx, config->enable_credential_hashing);
  ...
  new_ctx->token_service = init_token_service(
      core_ctx, config->token_signing_secret, config->token_validity_period_sec,
...
void polycall_auth_cleanup(polycall_core_context_t *core_ctx,
                           polycall_auth_context_t *auth_ctx) {
...
polycall_auth_get_current_identity(polycall_core_context_t *core_ctx,
                                   polycall_auth_context_t *auth_ctx,
                                   char **identity_id) {
  // Validate parameters
...
polycall_core_error_t
polycall_auth_authenticate(polycall_core_context_t *core_ctx,
                           polycall_auth_context_t *auth_ctx,
                           const char *username, const char *password,
                           char **access_token, char **refresh_token) {
  // Validate parameters
  if (!core_ctx || !auth_ctx || !username || !password || !access_token ||
      !refresh_token) {
    return POLYCALL_CORE_ERROR_INVALID_PARAMETERS;
  }
  ...
  for (size_t i = 0; i < auth_ctx->identities->count; i++) {
    ...
    if (attributes && attributes->name &&
        strcmp(attributes->name, username) == 0) {
  ...
  // Verify password
  bool password_valid =
      verify_password(auth_ctx->credentials, password, stored_hash);
  if (!password_valid) {
    return POLYCALL_CORE_ERROR_ACCESS_DENIED;
  }
  ...
  uint64_t refresh_expiry =
      now + auth_ctx->token_service->refresh_token_validity;

  // Generate tokens
  char *new_access_token =
      generate_token(auth_ctx->token_service, identity_id,
  ...
  // Create audit event
  audit_event_t *event =
      polycall_auth_create_audit_event(core_ctx, POLYCALL_AUDIT_EVENT_LOGIN,
                                       identity_id, NULL, NULL, true, NULL);
...
polycall_core_error_t
polycall_auth_validate_token(polycall_core_context_t *core_ctx,
                             polycall_auth_context_t *auth_ctx,
                             const char *token, char **identity_id) {
  // Validate parameters
  ...
  if (!result->is_valid) {
    ...
    audit_event_t *event = polycall_auth_create_audit_event(
        ...
        NULL, // No identity
        NULL, NULL, false, result->error_message);
  ...
  *identity_id = polycall_core_malloc(core_ctx, id_len);
  ...
  audit_event_t *event = polycall_auth_create_audit_event(
      core_ctx, POLYCALL_AUDIT_EVENT_TOKEN_VALIDATE, subject, NULL, NULL, true,
      NULL);
...
polycall_core_error_t
polycall_auth_refresh_token(polycall_core_context_t *core_ctx,
                            polycall_auth_context_t *auth_ctx,
  ...
  token_validation_result_t *result =
      validate_token_internal(auth_ctx->token_service, refresh_token);
  if (!result) {
    return POLYCALL_CORE_ERROR_OUT_OF_MEMORY;
  }
  ...
  if (!result->is_valid) {
    // Create audit event for invalid token
    audit_event_t *event = polycall_auth_create_audit_event(
        core_ctx, POLYCALL_AUDIT_EVENT_TOKEN_REFRESH,
        NULL, // No identity
        NULL, NULL, false, result->error_message);
  ...
  if (result->claims->token_id[0] != 'R') {
    ...
    audit_event_t *event = polycall_auth_create_audit_event(
        core_ctx, POLYCALL_AUDIT_EVENT_TOKEN_REFRESH, result->claims->subject,
        NULL, NULL, false, "Not a refresh token");
  ...
  char *new_access_token =
      generate_token(auth_ctx->token_service, subject,
                     POLYCALL_TOKEN_TYPE_ACCESS, access_expiry);
  if (!new_access_token) {
    free(result->claims);
    free(result);
  ...
  audit_event_t *event = polycall_auth_create_audit_event(
      core_ctx, POLYCALL_AUDIT_EVENT_TOKEN_REFRESH, subject, NULL, NULL, true,
      NULL);
