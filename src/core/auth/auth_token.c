 ...
 *
 * This file implements the token management interfaces for LibPolyCall
authentication,
...
static void free_token_entry(polycall_core_context_t *core_ctx,
                             token_entry_t *entry);
...
polycall_core_error_t polycall_auth_issue_token(
    ...
    const char *identity_id, polycall_token_type_t token_type,
    const char **scopes, size_t scope_count, const char *custom_claims,
    char **token) {
  // Validate parameters
  ...
  if (scopes && scope_count > 0) {
    claims.scopes =
        polycall_core_malloc(core_ctx, scope_count * sizeof(char *));
    if (!claims.scopes) {
      polycall_core_free(core_ctx, claims.subject);
      polycall_core_free(core_ctx, claims.issuer);
      polycall_core_free(core_ctx, claims.audience);
  ...
  // Generate token
  char *new_token = generate_token(auth_ctx->token_service, identity_id,
                                   token_type, claims.expires_at);
  if (!new_token) {
    polycall_core_free(core_ctx, claims.subject);
    polycall_core_free(core_ctx, claims.issuer);
    polycall_core_free(core_ctx, claims.audience);
  ...
  audit_event_t *event = polycall_auth_create_audit_event(
      core_ctx, POLYCALL_AUDIT_EVENT_TOKEN_ISSUE, identity_id, NULL,
      token_type == POLYCALL_TOKEN_TYPE_ACCESS
          ? "access"
...
polycall_core_error_t polycall_auth_validate_token_ex(
    polycall_core_context_t *core_ctx, polycall_auth_context_t *auth_ctx,
    const char *token, token_validation_result_t **result) {
  // Validate parameters
  ...
  token_validation_result_t *validation_result =
      polycall_core_malloc(core_ctx, sizeof(token_validation_result_t));
  ...
  // Validate token
  token_validation_result_t *internal_result =
      validate_token_internal(auth_ctx->token_service, token);
  if (!internal_result) {
    // Create error message
    const char *error_message = "Memory allocation failed during validation";
    size_t error_len = strlen(error_message) + 1;
    validation_result->error_message =
        polycall_core_malloc(core_ctx, error_len);
  ...
  if (!internal_result->is_valid) {
    ...
    if (internal_result->error_message) {
      ...
      validation_result->error_message =
          polycall_core_malloc(core_ctx, error_len);
      if (validation_result->error_message) {
        memcpy(validation_result->error_message, internal_result->error_message,
               error_len);
      }
    } else {
  ...
  validation_result->claims =
      polycall_core_malloc(core_ctx, sizeof(token_claims_t));
  ...
  if (internal_result->claims->subject) {
    ...
    validation_result->claims->subject =
        polycall_core_malloc(core_ctx, subject_len);
    if (validation_result->claims->subject) {
      memcpy(validation_result->claims->subject,
             internal_result->claims->subject, subject_len);
    }
  }
  ...
  if (internal_result->claims->issuer) {
    ...
    validation_result->claims->issuer =
        polycall_core_malloc(core_ctx, issuer_len);
    if (validation_result->claims->issuer) {
      memcpy(validation_result->claims->issuer, internal_result->claims->issuer,
             issuer_len);
    }
  }
  ...
  if (internal_result->claims->audience) {
    ...
    validation_result->claims->audience =
        polycall_core_malloc(core_ctx, audience_len);
    if (validation_result->claims->audience) {
      memcpy(validation_result->claims->audience,
             internal_result->claims->audience, audience_len);
    }
  }
  ...
  if (internal_result->claims->token_id) {
    ...
    validation_result->claims->token_id =
        polycall_core_malloc(core_ctx, token_id_len);
    if (validation_result->claims->token_id) {
      memcpy(validation_result->claims->token_id,
             internal_result->claims->token_id, token_id_len);
    }
  }
  ...
  if (internal_result->claims->roles &&
      internal_result->claims->role_count > 0) {
    validation_result->claims->roles = polycall_core_malloc(
        core_ctx, internal_result->claims->role_count * sizeof(char *));
    if (validation_result->claims->roles) {
      validation_result->claims->role_count =
          internal_result->claims->role_count;

      for (size_t i = 0; i < internal_result->claims->role_count; i++) {
        if (internal_result->claims->roles[i]) {
          size_t role_len = strlen(internal_result->claims->roles[i]) + 1;
          validation_result->claims->roles[i] =
              polycall_core_malloc(core_ctx, role_len);
  ...
  if (internal_result->claims->scopes &&
      internal_result->claims->scope_count > 0) {
    validation_result->claims->scopes = polycall_core_malloc(
        core_ctx, internal_result->claims->scope_count * sizeof(char *));
    if (validation_result->claims->scopes) {
      validation_result->claims->scope_count =
          internal_result->claims->scope_count;

      for (size_t i = 0; i < internal_result->claims->scope_count; i++) {
        if (internal_result->claims->scopes[i]) {
          size_t scope_len = strlen(internal_result->claims->scopes[i]) + 1;
          validation_result->claims->scopes[i] =
              polycall_core_malloc(core_ctx, scope_len);
  ...
  if (internal_result->claims->device_info) {
    ...
    validation_result->claims->device_info =
        polycall_core_malloc(core_ctx, device_info_len);
    if (validation_result->claims->device_info) {
      memcpy(validation_result->claims->device_info,
             internal_result->claims->device_info, device_info_len);
    }
  }
  ...
  if (internal_result->claims->custom_claims) {
    size_t custom_claims_len =
        strlen(internal_result->claims->custom_claims) + 1;
    validation_result->claims->custom_claims =
        polycall_core_malloc(core_ctx, custom_claims_len);
    if (validation_result->claims->custom_claims) {
      memcpy(validation_result->claims->custom_claims,
             internal_result->claims->custom_claims, custom_claims_len);
    }
  }
  ...
  if (validation_result->claims && validation_result->claims->subject) {
    audit_event_t *event = polycall_auth_create_audit_event(
        core_ctx, POLYCALL_AUDIT_EVENT_TOKEN_VALIDATE,
        validation_result->claims->subject, NULL, NULL, true, NULL);
...
polycall_auth_introspect_token(polycall_core_context_t *core_ctx,
                               polycall_auth_context_t *auth_ctx,
                               const char *token, token_claims_t **claims) {
  // Validate parameters
  ...
  polycall_core_error_t error =
      polycall_auth_validate_token_ex(core_ctx, auth_ctx, token, &result);
...
polycall_core_error_t polycall_auth_generate_api_key(
    polycall_core_context_t *core_ctx, polycall_auth_context_t *auth_ctx,
    const char *identity_id, const char *name, const char **scopes,
    size_t scope_count, uint32_t expiry_days, char **api_key) {
  // Validate parameters
  ...
  if (name) {
    snprintf(custom_claims, sizeof(custom_claims),
             "{\"name\":\"%s\",\"type\":\"api_key\"}", name);
  } else {
  ...
  // Generate token
  char *token = generate_token(auth_ctx->token_service, identity_id,
                               POLYCALL_TOKEN_TYPE_API_KEY, expiry_time);
  if (!token) {
    return POLYCALL_CORE_ERROR_OUT_OF_MEMORY;
  }
  ...
  // Create audit event
  audit_event_t *event = polycall_auth_create_audit_event(
      core_ctx, POLYCALL_AUDIT_EVENT_TOKEN_ISSUE, identity_id, NULL, "api_key",
      true, NULL);
 ...
 */
polycall_core_error_t
polycall_auth_revoke_token(polycall_core_context_t *core_ctx,
                           polycall_auth_context_t *auth_ctx,
                           const char *token) {
  // Validate parameters
  ...
  // Create audit event
  audit_event_t *event = polycall_auth_create_audit_event(
      core_ctx, POLYCALL_AUDIT_EVENT_TOKEN_REVOKE, identity_id, NULL, NULL,
      found, found ? NULL : "Token not found in registry");
...
void polycall_auth_free_token_validation_result(
    polycall_core_context_t *core_ctx, token_validation_result_t *result) {
  if (!core_ctx || !result) {
    return;
  }
 ...
 */
void polycall_auth_free_token_claims(polycall_core_context_t *core_ctx,
