...
static void load_history_from_file(command_history_t *history,
                                   const char *filename);
static void save_history_to_file(command_history_t *history,
                                 const char *filename);
static char **tokenize_command(const char *command, int *argc);
static void free_tokens(char **tokens, int count);
static char *read_line(const char *prompt, command_history_t *history,
                       bool enable_completion);
...
static void process_command(polycall_repl_context_t *repl_ctx,
                            const char *command);
static void inspect_log(polycall_repl_context_t *repl_ctx, const char *filter);
static void inspect_security(polycall_repl_context_t *repl_ctx,
                             const char *target);
  ...
  while (running) {
    ...
    if (strcmp(args[0], "exit") == 0 || strcmp(args[0], "quit") == 0) {
      ...
      if (argc == 1) {
        ...
        if (polycall_command_registry_list_modules(
                container->command_registry, &modules, &module_count) == 0) {
      ...
      container->execute_command(container, module, command, argc - 2,
                                 &args[2]);
    } else {
...
polycall_core_error_t polycall_repl_init(polycall_core_context_t *core_ctx,
  ...
  polycall_repl_context_t *ctx =
      (polycall_repl_context_t *)malloc(sizeof(polycall_repl_context_t));
  if (!ctx) {
...
polycall_core_error_t polycall_repl_run(polycall_core_context_t *core_ctx,
  ...
  while (repl_ctx->running) {
    // Read command line
    line = read_line(repl_ctx->prompt, repl_ctx->history,
                     repl_ctx->enable_completion);
    if (!line) {
...
polycall_repl_enable_log_inspection(polycall_core_context_t *core_ctx,
                                    polycall_repl_context_t *repl_ctx) {
  if (!core_ctx || !repl_ctx) {
    return POLYCALL_CORE_ERROR_INVALID_PARAMETERS;
  }
 ...
 */
polycall_core_error_t
polycall_repl_enable_zero_trust_inspection(polycall_core_context_t *core_ctx,
                                           polycall_repl_context_t *repl_ctx) {
  if (!core_ctx || !repl_ctx) {
    return POLYCALL_CORE_ERROR_INVALID_PARAMETERS;
  }
...
static command_history_t *create_command_history(int max_entries) {
  command_history_t *history =
      (command_history_t *)malloc(sizeof(command_history_t));
  if (!history) {
    return NULL;
  }
...
static void process_command(polycall_repl_context_t *repl_ctx,
  ...
  if (access_ctx && access_ctx->config.enable_audio_notifications) {
    polycall_accessibility_play_notification(
        POLYCALL_AUDIO_NOTIFICATION_PROMPT);
  }
  ...
  if (strcmp(command, "exit") == 0 || strcmp(command, "quit") == 0) {
    ...
    if (access_ctx && access_ctx->config.enable_audio_notifications) {
      polycall_accessibility_play_notification(
          POLYCALL_AUDIO_NOTIFICATION_INFO);
    }
    return;
    ...
    if (!repl_ctx->enable_log_inspection) {
      ...
      if (access_ctx && access_ctx->config.enable_audio_notifications) {
        polycall_accessibility_play_notification(
            POLYCALL_AUDIO_NOTIFICATION_ERROR);
      }
      return;
    }
    ...
    if (access_ctx && access_ctx->config.enable_audio_notifications) {
      polycall_accessibility_play_notification(
          POLYCALL_AUDIO_NOTIFICATION_SUCCESS);
    }
    return;
  ...
  case COMMAND_ERROR_INVALID_ARGUMENTS:
    print_error("Invalid command arguments. Type 'help' for usage information.",
                repl_ctx);
    break;
  ...
  case COMMAND_ERROR_NOT_FOUND:
    print_error("Command not found. Type 'help' to see available commands.",
                repl_ctx);
    break;
...
static void inspect_security(polycall_repl_context_t *repl_ctx,
  ...
  print_info(
      "This would display security information for the specified target.");
}
...
static void print_help(void) {
  ...
  if (g_repl_ctx && g_repl_ctx->enable_log_inspection) {
    printf("  %-20s %s\n", "inspect log [filter]",
           "Inspect logs with optional filter");
  }

  if (g_repl_ctx && g_repl_ctx->enable_zero_trust_inspection) {
    printf("  %-20s %s\n", "inspect security [target]",
           "Inspect security with optional target");
  }
