 ...
 */
int command_registry_register(command_registry_t *registry,
  ...
  if (registry->command_count >= registry->capacity) {
    ...
    command_t *new_commands =
        realloc(registry->commands, new_capacity * sizeof(command_t));
    if (!new_commands) {
      return -1;
    }
 ...
 */
const command_t *command_registry_get_help(command_registry_t *registry,
 ...
 * Get all commands
 */
int command_registry_get_all(command_registry_t *registry, command_t *commands,
                             int max_count) {
  ...
  int count =
      registry->command_count < max_count ? registry->command_count : max_count;
...
int register_all_command_handlers(void) {
  ...
  polycall_register_service(NULL, "config_command_handler",
  ...
  polycall_register_service(NULL, "micro_command_handler",
                            micro_command_handler);
  polycall_register_service(NULL, "network_command_handler",
                            network_command_handler);
  polycall_register_service(NULL, "protocol_command_handler",
                            protocol_command_handler);
  polycall_register_service(NULL, "telemetry_command_handler",
                            telemetry_command_handler);
  polycall_register_service(NULL, "accessibility_command_handler",
                            accessibility_command_handler);
