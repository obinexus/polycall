...
dop_cli_parse_arguments(int argc, char **argv,
                        dop_adapter_cli_options_t *options);

static polycall_core_error_t
dop_cli_create_component(polycall_core_context_t *core_ctx,
                         polycall_dop_adapter_context_t *adapter_ctx,
                         const dop_adapter_cli_options_t *options);

static polycall_core_error_t
dop_cli_list_components(polycall_dop_adapter_context_t *adapter_ctx);

static polycall_core_error_t
dop_cli_show_statistics(polycall_dop_adapter_context_t *adapter_ctx);

static polycall_core_error_t
dop_cli_cleanup_components(polycall_dop_adapter_context_t *adapter_ctx);

static polycall_core_error_t
dop_cli_load_config_file(const char *config_file_path,
                         dop_adapter_cli_options_t *options);
...
static const char *
dop_cli_isolation_level_to_string(polycall_dop_isolation_level_t level);
static polycall_dop_isolation_level_t
dop_cli_string_to_isolation_level(const char *str);
...
polycall_cli_dop_adapter_command(polycall_core_context_t *core_ctx, int argc,
                                 char **argv) {
  if (!core_ctx) {
  ...
  polycall_core_error_t parse_result =
      dop_cli_parse_arguments(argc, argv, &options);
  if (parse_result != POLYCALL_CORE_SUCCESS) {
    if (parse_result !=
        POLYCALL_CORE_ERROR_INVALID_PARAMETER) { // Don't show usage for help
  ...
  if (strlen(options.config_file) > 0) {
    polycall_core_error_t config_result =
        dop_cli_load_config_file(options.config_file, &options);
  ...
  polycall_dop_error_t policy_result =
      polycall_dop_security_policy_create_default(options.isolation_level,
                                                  &security_policy);
  ...
  security_policy.max_memory_usage =
      options.max_memory_mb * 1024 * 1024; // Convert MB to bytes
  ...
  polycall_dop_error_t adapter_result =
      polycall_dop_adapter_initialize(core_ctx, &adapter_ctx, &security_policy);
  ...
  // Cleanup DOP Adapter
  polycall_dop_error_t cleanup_result =
...
dop_cli_parse_arguments(int argc, char **argv,
  ...
  while ((c = getopt_long(argc, argv, "hc:i:v:l:I:p:m:t:C:r:LSXA", long_options,
                          &option_index)) != -1) {
    switch (c) {
    case 'h':
      dop_cli_print_help();
      return POLYCALL_CORE_ERROR_INVALID_PARAMETER; // Signal to exit without
                                                    // error

    case 'c':
      strncpy(options->component_name, optarg,
              POLYCALL_DOP_MAX_COMPONENT_NAME_LENGTH - 1);
      options->component_name[POLYCALL_DOP_MAX_COMPONENT_NAME_LENGTH - 1] =
          '\0';
      break;

    case 'i':
      strncpy(options->component_id, optarg,
  ...
  if (optind < argc && strlen(options->component_name) == 0) {
    strncpy(options->component_name, argv[optind],
            POLYCALL_DOP_MAX_COMPONENT_NAME_LENGTH - 1);
    options->component_name[POLYCALL_DOP_MAX_COMPONENT_NAME_LENGTH - 1] = '\0';
  }

  // Generate component ID if not provided
  if (strlen(options->component_name) > 0 &&
      strlen(options->component_id) == 0) {
...
dop_cli_create_component(polycall_core_context_t *core_ctx,
  ...
  polycall_dop_error_t config_result =
      polycall_dop_component_config_create_default(options->component_id,
                                                   options->component_name,
  ...
  strncpy((char *)config.version, options->version,
          POLYCALL_DOP_MAX_VERSION_LENGTH - 1);
  ...
  config.security_policy.max_memory_usage =
      options->max_memory_mb * 1024 * 1024;
  ...
  polycall_dop_error_t register_result =
  ...
  polycall_dop_error_t stats_result =
...
dop_cli_list_components(polycall_dop_adapter_context_t *adapter_ctx) {
  ...
  printf("  Use './polycall micro <component_name> --dop-adapter' to register "
         "a component.\n");
...
dop_cli_load_config_file(const char *config_file_path,
                         dop_adapter_cli_options_t *options) {
  // Simple configuration file loading (JSON or INI format would be implemented
  // here)
...
static void dop_cli_print_help(void) {
  ...
  printf("  -i, --id ID                  Component identifier (auto-generated "
         "if not provided)\n");
  printf("  -v, --version VERSION        Component version (default: 1.0.0)\n");
  printf("  -l, --language LANG          Programming language (javascript, "
         "python, c, java)\n");
  printf("  -I, --isolation LEVEL        Isolation level (none, basic, "
         "standard, strict, paranoid)\n");
  printf("  -p, --permissions PERMS      Comma-separated permissions list\n");
  printf("  -m, --memory MB              Maximum memory usage in MB (default: "
         "16)\n");
  printf("  -t, --timeout MS             Maximum execution time in "
...
static void dop_cli_print_examples(void) {
  ...
  printf("  polycall micro bankcard_component --dop-adapter "
         "--language=javascript --isolation=strict\n\n");

  printf("  # Register an ads service component with limited permissions\n");
  printf("  polycall micro ads_service --dop-adapter --permissions=memory_read "
         "--memory=8\n\n");

  printf("  # Register a Python component with custom timeout\n");
  printf("  polycall micro data_processor --dop-adapter --language=python "
         "--timeout=10000\n\n");
