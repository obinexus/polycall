...
static command_result_t handle_log_inspection(int argc, char **argv,
                                              void *context);
...
static subcommand_t repl_subcommands[] = {
    {.name = "start",
     ...
     .screen_reader_desc =
         "Start an interactive REPL session with optional configuration"},
    {.name = "history",
    ...
    {.name = "log-inspection",
     ...
     .usage = "polycall repl log-inspection [--enable] [--disable] [--filter "
              "<pattern>]",
 ...
 */
static void format_output(polycall_core_context_t *core_ctx, const char *text,
                          polycall_text_type_t type,
                          polycall_text_style_t style) {
  polycall_accessibility_context_t *access_ctx =
      get_accessibility_context(core_ctx);

  if (access_ctx) {
    char formatted_text[1024];
    polycall_accessibility_format_text(core_ctx, access_ctx, text, type, style,
                                       formatted_text, sizeof(formatted_text));
...
static int cmd_repl_exec(void *container, int argc, char **argv,
                         void *context) {
...
static int cmd_repl_script(void *container, int argc, char **argv,
                           void *context) {
  polycall_cli_container_t *cli = (polycall_cli_container_t *)container;
  ...
  polycall_core_error_t result =
      polycall_repl_execute_script(core_ctx, repl_ctx, argv[0]);
...
static void format_error(polycall_core_context_t *core_ctx, const char *text) {
  ...
  if (access_ctx) {
    ...
    polycall_accessibility_format_text(
        core_ctx, access_ctx, text, POLYCALL_TEXT_ERROR, POLYCALL_STYLE_NORMAL,
        formatted_text, sizeof(formatted_text));
    fprintf(stderr, "%s\n", formatted_text);
  } else {
    fprintf(stderr, "%s\n", text);
 ...
 */
static command_result_t init_repl_subsystem(polycall_core_context_t *core_ctx,
  ...
  polycall_core_error_t result =
      polycall_repl_init(core_ctx, config, &g_repl_ctx);
...
static command_result_t handle_repl(int argc, char **argv, void *context) {
  ...
  if (argc < 2) {
    if (access_ctx) {
      char title[256];
      polycall_accessibility_format_text(
          core_ctx, access_ctx, "REPL Commands", POLYCALL_TEXT_HEADING,
          POLYCALL_STYLE_BOLD, title, sizeof(title));
      printf("\n%s\n\n", title);
    } else {
      printf("\nREPL Commands\n\n");
    }
    ...
    for (int i = 0; i < repl_command.subcommand_count; i++) {
      ...
      if (access_ctx) {
        ...
        polycall_accessibility_format_text(
            core_ctx, access_ctx, subcmd->name, POLYCALL_TEXT_SUBCOMMAND,
            POLYCALL_STYLE_NORMAL, subcmd_name, sizeof(subcmd_name));

        char subcmd_desc[256];
        polycall_accessibility_format_text(
            core_ctx, access_ctx, subcmd->description, POLYCALL_TEXT_NORMAL,
            POLYCALL_STYLE_NORMAL, subcmd_desc, sizeof(subcmd_desc));
    ...
    printf("\nUse 'polycall help repl <subcommand>' for more information about "
           "a specific subcommand.\n");
    printf("Run 'polycall repl start' without arguments to launch an "
           "interactive REPL with default settings.\n");
    return COMMAND_SUCCESS;
  }
  ...
  if (access_ctx) {
    ...
    snprintf(unknown_cmd, sizeof(unknown_cmd), "Unknown repl subcommand: %s",
             subcommand_name);

    polycall_accessibility_format_text(
        core_ctx, access_ctx, unknown_cmd, POLYCALL_TEXT_ERROR,
        POLYCALL_STYLE_NORMAL, error_msg, sizeof(error_msg));
    fprintf(stderr, "%s\n", error_msg);
  } else {
...
static command_result_t handle_start(int argc, char **argv, void *context) {
  ...
  if (access_ctx) {
    char welcome[512];
    polycall_accessibility_format_text(
        core_ctx, access_ctx, "LibPolyCall Interactive Shell",
        POLYCALL_TEXT_HEADING, POLYCALL_STYLE_BOLD, welcome, sizeof(welcome));
    printf("\n%s\n\n", welcome);
  } else {
  ...
  // Run REPL
  polycall_core_error_t result = polycall_repl_run(core_ctx, g_repl_ctx);
...
static command_result_t handle_history(int argc, char **argv, void *context) {
  ...
  if (clear_history) {
    polycall_core_error_t result =
        polycall_repl_clear_history(core_ctx, g_repl_ctx);
    ...
    format_output(core_ctx, "Command history cleared", POLYCALL_TEXT_SUCCESS,
  ...
  if (load_file) {
    polycall_core_error_t result =
        polycall_repl_load_history(core_ctx, g_repl_ctx, load_file);

    if (result != POLYCALL_CORE_SUCCESS) {
      char error_msg[256];
      snprintf(error_msg, sizeof(error_msg), "Failed to load history from %s",
               load_file);
    ...
    snprintf(success_msg, sizeof(success_msg), "Command history loaded from %s",
             load_file);
    format_output(core_ctx, success_msg, POLYCALL_TEXT_SUCCESS,
                  POLYCALL_STYLE_NORMAL);
    printf("\n");
  }
  ...
  if (save_file) {
    polycall_core_error_t result =
        polycall_repl_save_history(core_ctx, g_repl_ctx, save_file);

    if (result != POLYCALL_CORE_SUCCESS) {
      char error_msg[256];
      snprintf(error_msg, sizeof(error_msg), "Failed to save history to %s",
               save_file);
      format_error(core_ctx, error_msg);
      return COMMAND_ERROR_EXECUTION_FAILED;
    }
    ...
    char success_msg[256];
    snprintf(success_msg, sizeof(success_msg), "Command history saved to %s",
             save_file);
    format_output(core_ctx, success_msg, POLYCALL_TEXT_SUCCESS,
                  POLYCALL_STYLE_NORMAL);
    printf("\n");
  ...
  if (!clear_history && !load_file && !save_file) {
    ...
    polycall_core_error_t result = polycall_repl_get_history(
        core_ctx, g_repl_ctx, &history_entries, &entry_count);
    ...
    // Display history
    format_output(core_ctx, "Command History", POLYCALL_TEXT_HEADING,
                  POLYCALL_STYLE_BOLD);
    printf("\n\n");

    if (entry_count == 0) {
      format_output(core_ctx, "No history entries", POLYCALL_TEXT_NORMAL,
                    POLYCALL_STYLE_ITALIC);
      printf("\n");
    } else {
      for (size_t i = 0; i < entry_count; i++) {
        char entry_str[1024];
        snprintf(entry_str, sizeof(entry_str), "%3zu  %s", i + 1,
                 history_entries[i]);
        format_output(core_ctx, entry_str, POLYCALL_TEXT_NORMAL,
                      POLYCALL_STYLE_NORMAL);
        printf("\n");
      }
    }
...
static command_result_t handle_log_inspection(int argc, char **argv,
  ...
  if (enable) {
    polycall_core_error_t result =
        polycall_repl_enable_log_inspection(core_ctx, g_repl_ctx, filter);
    ...
    if (filter) {
      snprintf(success_msg, sizeof(success_msg),
               "Log inspection enabled with filter: %s", filter);
    } else {
    ...
    polycall_core_error_t result =
        polycall_repl_disable_log_inspection(core_ctx, g_repl_ctx);
    ...
    format_output(core_ctx, "Log inspection disabled", POLYCALL_TEXT_SUCCESS,
                  POLYCALL_STYLE_NORMAL);
    printf("\n");
  } else if (filter) {
    // Just update the filter
    polycall_core_error_t result =
        polycall_repl_set_log_filter(core_ctx, g_repl_ctx, filter);
    ...
    polycall_core_error_t result = polycall_repl_get_log_inspection_status(
        core_ctx, g_repl_ctx, &is_enabled, current_filter,
    ...
    format_output(core_ctx, "Log Inspection Status", POLYCALL_TEXT_HEADING,
                  POLYCALL_STYLE_BOLD);
    printf("\n\n");

    char status_str[32];
    snprintf(status_str, sizeof(status_str), "Enabled: %s",
             is_enabled ? "Yes" : "No");
    format_output(core_ctx, status_str, POLYCALL_TEXT_NORMAL,
                  POLYCALL_STYLE_NORMAL);
    printf("\n");

    if (is_enabled && current_filter[0] != '\0') {
      char filter_str[256 + 16];
      snprintf(filter_str, sizeof(filter_str), "Current filter: %s",
               current_filter);
      format_output(core_ctx, filter_str, POLYCALL_TEXT_NORMAL,
                    POLYCALL_STYLE_NORMAL);
      printf("\n");
    }
  }
...
static command_result_t handle_settings(int argc, char **argv, void *context) {
  ...
  if (reset_settings) {
    polycall_core_error_t result =
        polycall_repl_reset_settings(core_ctx, g_repl_ctx);
    ...
    format_output(core_ctx, "REPL settings reset to defaults",
    ...
    polycall_core_error_t result = polycall_repl_set_setting(
        core_ctx, g_repl_ctx, setting_key, setting_value);

    if (result != POLYCALL_CORE_SUCCESS) {
      char error_msg[256];
      snprintf(error_msg, sizeof(error_msg), "Failed to set %s=%s", setting_key,
               setting_value);
      format_error(core_ctx, error_msg);
      return COMMAND_ERROR_EXECUTION_FAILED;
    }
    ...
    char success_msg[256];
    snprintf(success_msg, sizeof(success_msg), "Setting updated: %s=%s",
  ...
  if (list_settings) {
    ...
    polycall_core_error_t result =
        polycall_repl_get_config(core_ctx, g_repl_ctx, &current_config);
    ...
    format_output(core_ctx, "REPL Settings", POLYCALL_TEXT_HEADING,
                  POLYCALL_STYLE_BOLD);
    printf("\n\n");
    ...
    format_output(core_ctx, setting_str, POLYCALL_TEXT_NORMAL,
                  POLYCALL_STYLE_NORMAL);
    printf("\n");
    ...
    snprintf(setting_str, sizeof(setting_str), "Syntax highlighting: %s",
             current_config.enable_syntax_highlighting ? "Enabled"
                                                       : "Disabled");
    format_output(core_ctx, setting_str, POLYCALL_TEXT_NORMAL,
                  POLYCALL_STYLE_NORMAL);
    printf("\n");
    ...
    snprintf(setting_str, sizeof(setting_str), "Zero-trust inspection: %s",
             current_config.enable_zero_trust_inspection ? "Enabled"
                                                         : "Disabled");
    format_output(core_ctx, setting_str, POLYCALL_TEXT_NORMAL,
                  POLYCALL_STYLE_NORMAL);
    printf("\n");
    ...
    snprintf(setting_str, sizeof(setting_str), "History file: %s",
             current_config.history_file ? current_config.history_file
                                         : "Not set");
    format_output(core_ctx, setting_str, POLYCALL_TEXT_NORMAL,
                  POLYCALL_STYLE_NORMAL);
    printf("\n");
...
static int cmd_repl_init(void *container, int argc, char **argv,
                         void *context) {
  polycall_cli_container_t *cli = (polycall_cli_container_t *)container;
  ...
  polycall_repl_context_t *existing_repl =
      cli->resolve_service(cli, "repl_context");
  if (existing_repl) {
    printf("REPL already initialized. Use 'repl cleanup' before "
           "re-initializing.\n");
    return 1;
  }
  ...
  polycall_core_error_t result =
      polycall_repl_init(core_ctx, &repl_ctx, &config);
  if (result != POLYCALL_CORE_SUCCESS) {
...
static int cmd_repl_cleanup(void *container, int argc, char **argv,
                            void *context) {
  polycall_cli_container_t *cli = (polycall_cli_container_t *)container;
...
static int cmd_repl_register_handler(void *container, int argc, char **argv,
                                     void *context) {
  printf(
      "Error: Custom handler registration can only be done programmatically\n");
  return 1;
}
...
void polycall_register_repl_commands(void *registry) {
  ...
  static const polycall_command_t interactive_cmd = {
      ...
      .usage = "repl interactive [--no-color] [--verbose] [--history-file "
               "<file>] [--no-history]",
  ...
  static const polycall_command_t init_cmd = {
      ...
      .usage = "repl init [--no-color] [--verbose] [--history-file <file>] "
               "[--no-history]",
      .dependencies = (const char *[]){"core_context", NULL}};
  ...
  static const polycall_command_t register_handler_cmd = {
      ...
      .description =
          "Register a custom REPL command handler (programmatic only)",
...
static int cmd_repl_interactive(void *container, int argc, char **argv,
                                void *context) {
  polycall_cli_container_t *cli = (polycall_cli_container_t *)container;
  ...
  polycall_core_error_t result =
      polycall_repl_run_interactive(core_ctx, repl_ctx);
