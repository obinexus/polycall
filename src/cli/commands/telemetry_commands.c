...
// Define subcommands
static command_result_t handle_telemetry_help(int argc, char **argv,
                                              void *context);
static command_result_t handle_telemetry_status(int argc, char **argv,
                                                void *context);
static command_result_t handle_telemetry_configure(int argc, char **argv,
                                                   void *context);
static command_result_t handle_telemetry_analytics(int argc, char **argv,
                                                   void *context);
static command_result_t handle_telemetry_debug(int argc, char **argv,
                                               void *context);

// Helper for recording command state transitions
static void record_command_state(polycall_core_context_t *core_ctx,
                                 const char *cmd_name, uint32_t state_id,
                                 const char *parent_guid, char **current_guid);
...
static command_t telemetry_command = {
    ...
    .subcommand_count =
        sizeof(telemetry_subcommands) / sizeof(telemetry_subcommands[0]),
    .requires_context = true};
...
static void record_command_state(polycall_core_context_t *core_ctx,
                                 const char *cmd_name, uint32_t state_id,
                                 const char *parent_guid, char **current_guid) {
  telemetry_container_t *telemetry_container =
      polycall_get_service(core_ctx, "telemetry_container");
  if (!telemetry_container) {
    fprintf(stderr,
            "Warning: Telemetry container not available for state tracking\n");
    return;
  }

  // Get telemetry context from container
  polycall_telemetry_context_t *telemetry_ctx =
      telemetry_container->telemetry_ctx;
  if (!telemetry_ctx) {
    fprintf(stderr,
            "Warning: Telemetry context not available for state tracking\n");
    return;
  }
  ...
  if (parent_guid) {
    ...
    *current_guid =
        polycall_update_guid_state(core_ctx, parent_guid, state_id, event_id);
  } else {
    // Generate new GUID for new command
    *current_guid =
        polycall_generate_cryptonomic_guid(core_ctx, cmd_name, state_id, NULL);
  }
  ...
  if (state_id == CMD_STATE_ERROR) {
    ...
    if (security_ctx) {
      ...
      // Copy event GUID
      if (*current_guid) {
        strncpy(sec_event.event_guid, *current_guid,
                sizeof(sec_event.event_guid) - 1);
...
static command_result_t handle_telemetry_status(int argc, char **argv,
  ...
  record_command_state(core_ctx, "telemetry.status", CMD_STATE_INITIATED, NULL,
                       &cmd_guid);
  ...
  if (!container) {
    ...
    // Record error state
    record_command_state(core_ctx, "telemetry.status", CMD_STATE_ERROR,
                         cmd_guid, &cmd_guid);
    free(cmd_guid);
  ...
  record_command_state(core_ctx, "telemetry.status", CMD_STATE_EXECUTING,
                       cmd_guid, &cmd_guid);
  ...
  record_command_state(core_ctx, "telemetry.status", CMD_STATE_COMPLETED,
                       cmd_guid, &cmd_guid);
  free(cmd_guid);
...
static command_result_t handle_telemetry_configure(int argc, char **argv,
  ...
  // Record command initiation
  record_command_state(core_ctx, "telemetry.configure", CMD_STATE_INITIATED,
                       NULL, &cmd_guid);
  ...
  if (!parse_flags(argc - 1, &argv[1], flags, flag_count, remaining_args,
    ...
    // Record error state
    record_command_state(core_ctx, "telemetry.configure", CMD_STATE_ERROR,
                         cmd_guid, &cmd_guid);
    free(cmd_guid);
  ...
  // Record executing state
  record_command_state(core_ctx, "telemetry.configure", CMD_STATE_EXECUTING,
                       cmd_guid, &cmd_guid);
  ...
  // Record successful completion
  record_command_state(core_ctx, "telemetry.configure", CMD_STATE_COMPLETED,
                       cmd_guid, &cmd_guid);
  free(cmd_guid);
...
static command_result_t handle_telemetry_analytics(int argc, char **argv,
  ...
  // Record command initiation
  record_command_state(core_ctx, "telemetry.analytics", CMD_STATE_INITIATED,
                       NULL, &cmd_guid);
  ...
  if (!container) {
    ...
    // Record error state
    record_command_state(core_ctx, "telemetry.analytics", CMD_STATE_ERROR,
                         cmd_guid, &cmd_guid);
    free(cmd_guid);
  ...
  // Record executing state
  record_command_state(core_ctx, "telemetry.analytics", CMD_STATE_EXECUTING,
                       cmd_guid, &cmd_guid);
  ...
  // Record successful completion
  record_command_state(core_ctx, "telemetry.analytics", CMD_STATE_COMPLETED,
                       cmd_guid, &cmd_guid);
  free(cmd_guid);
...
static command_result_t handle_telemetry_debug(int argc, char **argv,
  ...
  // Record command initiation
  record_command_state(core_ctx, "telemetry.debug", CMD_STATE_INITIATED, NULL,
                       &cmd_guid);
  ...
  if (!container) {
    ...
    // Record error state
    record_command_state(core_ctx, "telemetry.debug", CMD_STATE_ERROR, cmd_guid,
                         &cmd_guid);
    free(cmd_guid);
  ...
  // Record executing state
  record_command_state(core_ctx, "telemetry.debug", CMD_STATE_EXECUTING,
                       cmd_guid, &cmd_guid);
  ...
  if (argc > 1) {
    ...
    printf("\nUse 'polycall telemetry debug <guid>' to see details for a "
           "specific event.\n");
  }

  // Record successful completion
  record_command_state(core_ctx, "telemetry.debug", CMD_STATE_COMPLETED,
                       cmd_guid, &cmd_guid);
  free(cmd_guid);
