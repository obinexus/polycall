...
// Define subcommands
static command_result_t handle_protocol_help(int argc, char **argv,
                                             void *context);
static command_result_t handle_protocol_status(int argc, char **argv,
                                               void *context);
static command_result_t handle_protocol_configure(int argc, char **argv,
                                                  void *context);
...
// protocol command
static command_t protocol_command = {.name = "protocol",
                                     .description = "protocol module commands",
                                     .usage = "polycall protocol <subcommand>",
                                     .handler = NULL,
                                     .subcommands = protocol_subcommands,
                                     .subcommand_count =
                                         sizeof(protocol_subcommands) /
                                         sizeof(protocol_subcommands[0]),
                                     .requires_context = true};
...
static command_result_t handle_protocol_status(int argc, char **argv,
  ...
  protocol_container_t *container =
      polycall_get_service(core_ctx, "protocol_container");
  if (!container) {
