...
bool cli_register_command(const command_t *command) {
  ...
  if (command_count >= MAX_COMMANDS) {
    fprintf(stderr,
            "Cannot register command: maximum number of commands reached\n");
    return false;
  }
 ...
 */
const subcommand_t *find_subcommand(const command_t *command,
                                    const char *name) {
  if (!command || !name) {
    return NULL;
  }
...
int process_command_line(int argc, char **argv,
  ...
  if (!command) {
    polycall_accessibility_context_t *access_ctx =
        get_accessibility_context(context);
    ...
    if (access_ctx) {
      ...
      polycall_accessibility_format_text(
          context, access_ctx, error_msg, POLYCALL_TEXT_ERROR,
          POLYCALL_STYLE_NORMAL, error_buffer, sizeof(error_buffer));
    } else {
...
command_result_t cli_execute_command(int argc, char **argv, void *context) {
  ...
  if (!command) {
    ...
    if (access_ctx) {
      ...
      snprintf(unknown_cmd, sizeof(unknown_cmd), "Unknown command: %s",
               command_name);

      polycall_accessibility_format_text(
          context, access_ctx, unknown_cmd, POLYCALL_TEXT_ERROR,
  ...
  if (command->requires_context && !context) {
    ...
    if (access_ctx) {
      ...
      polycall_accessibility_format_error(
          context, access_ctx, POLYCALL_CORE_ERROR_INVALID_PARAMETERS,
          "Command requires an initialized context", error_msg,
          sizeof(error_msg));
      fprintf(stderr, "%s\n", error_msg);
    } else {
      fprintf(stderr, "Command '%s' requires an initialized context\n",
      ...
      if (subcommand->requires_context && !context) {
        ...
        if (access_ctx) {
          ...
          snprintf(error_text, sizeof(error_text),
                   "Subcommand '%s' requires an initialized context",
                   subcommand_name);

          polycall_accessibility_format_text(
              context, access_ctx, error_text, POLYCALL_TEXT_ERROR,
              POLYCALL_STYLE_NORMAL, error_msg, sizeof(error_msg));
          fprintf(stderr, "%s\n", error_msg);
        } else {
          fprintf(stderr, "Subcommand '%s' requires an initialized context\n",
 ...
 */
int show_subcommand_help(const command_t *command,
  ...
  if (access_ctx) {
    char title[256];
    char cmd_subcmd[128];
    snprintf(cmd_subcmd, sizeof(cmd_subcmd), "%s %s", command->name,
             subcommand->name);

    polycall_accessibility_format_text(
        context, access_ctx, cmd_subcmd, POLYCALL_TEXT_HEADING,
        POLYCALL_STYLE_BOLD, title, sizeof(title));
    printf("%s\n\n", title);

    char description[512];
    polycall_accessibility_format_text(
        context, access_ctx, subcommand->description, POLYCALL_TEXT_NORMAL,
        POLYCALL_STYLE_NORMAL, description, sizeof(description));
    printf("%s\n\n", description);

    char usage_label[64];
    polycall_accessibility_format_text(
        context, access_ctx, "Usage:", POLYCALL_TEXT_COMMAND,
        POLYCALL_STYLE_BOLD, usage_label, sizeof(usage_label));

    char usage_text[512];
    polycall_accessibility_format_text(
        context, access_ctx, subcommand->usage, POLYCALL_TEXT_NORMAL,
...
int show_help(const char *command_name, polycall_core_context_t *context) {
  ...
  if (!command_name) {
    ...
    if (access_ctx) {
      polycall_accessibility_format_text(
          context, access_ctx, "LibPolyCall Command-Line Interface",
          POLYCALL_TEXT_HEADING, POLYCALL_STYLE_BOLD, title, sizeof(title));
      printf("%s\n\n", title);
    } else {
    ...
    for (int i = 0; i < command_count; i++) {
      if (access_ctx) {
        char cmd_name[128];
        polycall_accessibility_format_text(
            context, access_ctx, commands[i].name, POLYCALL_TEXT_COMMAND,
            POLYCALL_STYLE_NORMAL, cmd_name, sizeof(cmd_name));

        char cmd_desc[256];
        polycall_accessibility_format_text(
            context, access_ctx, commands[i].description, POLYCALL_TEXT_NORMAL,
            POLYCALL_STYLE_NORMAL, cmd_desc, sizeof(cmd_desc));
    ...
    printf("\nFor more information on a specific command, run: polycall help "
           "[command]\n");
    return COMMAND_SUCCESS;
  }
  ...
  if (access_ctx) {
    char title[256];
    polycall_accessibility_format_text(
        context, access_ctx, command->name, POLYCALL_TEXT_HEADING,
        POLYCALL_STYLE_BOLD, title, sizeof(title));
    printf("%s\n\n", title);

    char description[512];
    polycall_accessibility_format_text(
        context, access_ctx, command->description, POLYCALL_TEXT_NORMAL,
        POLYCALL_STYLE_NORMAL, description, sizeof(description));
    printf("%s\n\n", description);
    ...
    char usage_text[512];
    polycall_accessibility_format_text(
        context, access_ctx, command->usage, POLYCALL_TEXT_NORMAL,
        POLYCALL_STYLE_NORMAL, usage_text, sizeof(usage_text));
    printf("%s %s\n", usage_label, usage_text);
  } else {
    printf("%s\n\n", command->name);
  ...
  if (command->subcommand_count > 0) {
    ...
    for (int i = 0; i < command->subcommand_count; i++) {
      if (access_ctx) {
        ...
        polycall_accessibility_format_text(
            context, access_ctx, command->subcommands[i].name,
            POLYCALL_TEXT_SUBCOMMAND, POLYCALL_STYLE_NORMAL, subcmd_name,
            sizeof(subcmd_name));

        char subcmd_desc[256];
        polycall_accessibility_format_text(
            context, access_ctx, command->subcommands[i].description,
            POLYCALL_TEXT_NORMAL, POLYCALL_STYLE_NORMAL, subcmd_desc,
            sizeof(subcmd_desc));
    ...
    printf("\nFor more information on a subcommand, run: polycall help %s "
           "[subcommand]\n",
           command->name);
  }
            ...
            if (flags[j].short_name && flags[j].short_name[0] == short_flag) {
              ...
              if (flags[j].requires_value) {
                if (c == len - 1 && i + 1 < argc && argv[i + 1][0] != '-') {
                  ...
                  fprintf(stderr,
                          "Flag -%c requires a value and cannot be combined\n",
                          short_flag);
                  return false;
                }
              }
 ...
 */
void cli_cleanup_commands(void) { command_count = 0; }
